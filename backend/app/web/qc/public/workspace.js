(function () {
	...
	async function runWithTimeout(task, options, timeoutMs) {
		...
		const canAbort =
			typeof AbortController === "function" && Number.isFinite(ms) && ms > 0;
			...
			if (isAbortError(err)) {
				throw new Error(
					"Request timed out after " +
						String(Math.max(1, Math.round(ms / 1000))) +
						"s",
				);
			}
			throw err;
		...
		if (type === "claim-status-updated") {
			...
			if (status === "completed" && page === "case-detail") {
				...
				if (
					!payloadClaimUuid ||
					!currentClaimUuid ||
					payloadClaimUuid === currentClaimUuid
				) {
					window.location.href =
						"/qc/" +
						encodeURIComponent(role || "user") +
						"/" +
						targetPageAfterCompletion(role);
					return;
				}
			}
		}
	...
	function attachClaimSyncListeners() {
		...
		window.addEventListener("storage", function (event) {
			try {
				if (!event || event.key !== CLAIM_SYNC_STORAGE_KEY || !event.newValue)
					return;
	...
	function renderError(message) {
		contentPanel.innerHTML =
			'<p class="msg err">' +
			escapeHtml(message || "Unexpected error") +
			"</p>";
	}
	...
	function statusChip(status) {
		...
		if (
			...
			cls = "success";
		if (
			["withdrawn", "reject", "rejected", "failed", "hard_reject"].includes(s)
		)
			cls = "warn";
	...
	function sortClaimsByAllotmentDateFirst(items, ascending) {
		...
		list.sort(function (a, b) {
			...
			if (aHasAllot && bHasAllot && aAllot !== bAllot)
				return isAsc ? aAllot - bAllot : bAllot - aAllot;

			const aFallback = parseAllotmentDateValue(
				(a &&
					(a.assigned_at || a.updated_at || a.created_at || a.last_upload)) ||
					"",
			);
			const bFallback = parseAllotmentDateValue(
				(b &&
					(b.assigned_at || b.updated_at || b.created_at || b.last_upload)) ||
					"",
			);
			if (
				Number.isFinite(aFallback) &&
				Number.isFinite(bFallback) &&
				aFallback !== bFallback
			) {
	...
	function formatTreatmentType(value) {
		...
		if (
			normalized === "non_surgical" ||
			normalized === "non-surgical" ||
			normalized === "nonsurgical"
		)
			return "Non-Surgical";
		return raw;
	}
	...
	function resolveTreatmentType(item) {
		const payload =
			item && item.legacy_payload && typeof item.legacy_payload === "object"
				? item.legacy_payload
				: {};
		const raw =
			(item && (item.treatment_type || item.claim_type)) ||
	...
	async function apiFetch(path, options) {
		...
		const timeoutMs = Number(
			opts && opts.timeoutMs ? opts.timeoutMs : DEFAULT_API_TIMEOUT_MS,
		);
		...
		if (!resp.ok) {
			...
			if (resp.status === 413) {
				throw new Error(
					"Upload size too large. Please upload smaller files or fewer files at once.",
				);
			}
	...
	async function apiFetchFile(path, options) {
		...
		const timeoutMs = Number(
			opts && opts.timeoutMs
				? opts.timeoutMs
				: Math.max(DEFAULT_API_TIMEOUT_MS, 600000),
		); // Default 10min timeout for AI operations
		const headers = new Headers(opts.headers || {});
		headers.set("Authorization", "Bearer " + token);
		const fetchOpts = { ...opts, headers };
		delete fetchOpts.timeoutMs;
		const fileResult = await runWithTimeout(
			async function (signal) {
				const resp = await fetch(path, {
					...fetchOpts,
					signal: signal || undefined,
	...
	function parseRoute() {
		...
		if (parts.length === 0 || parts[0] === "login")
			return { routeRole: null, page: null };
		return {
			routeRole: parts[0],
			page: parts.slice(1).join("-") || "dashboard",
		};
	}
	...
	function renderHeader(user, activeRole, page) {
		...
		const roleSwitch = canSwitchRole
			? '<form class="role-switch-form" onsubmit="return false;">' +
				...
				(activeRole === "super_admin" ? " selected" : "") +
				">Super Admin</option>" +
				'<option value="doctor"' +
				(activeRole === "doctor" ? " selected" : "") +
				">Doctor</option>" +
				'<option value="user"' +
				(activeRole === "user" ? " selected" : "") +
				">User</option>" +
				'<option value="auditor"' +
				(activeRole === "auditor" ? " selected" : "") +
				">Auditor</option>" +
				"</select></form>"
			: "";
	...
	function renderClaimsTable(items) {
		if (!items || items.length === 0)
			return "<p class='muted'>No claims found.</p>";
		const rows = items
			.map((item) => {
				return (
					"<tr>" +
					"<td><code>" +
					escapeHtml(item.external_claim_id) +
					"</code></td>" +
					"<td>" +
					...
					"</td>" +
					"<td>" +
					escapeHtml(item.assigned_doctor_id || "-") +
					"</td>" +
					"<td>" +
			...
			.join("");
		return (
			'<div class="table-wrap"><table><thead><tr><th>Claim ID</th><th>Patient</th><th>Status</th><th>Doctor</th><th>Created</th></tr></thead><tbody>' +
			rows +
			"</tbody></table></div>"
		);
	}
		...
		try {
			...
			try {
				...
				const fallback = await apiFetch(
					"/api/v1/user-tools/completed-reports?status_filter=all&qc_filter=all&limit=500&offset=0",
				);
	...
	async function renderSuperAdminDashboard() {
		...
		const doctors = (users.items || []).filter(
			(u) => u.role === "doctor",
		).length;
		const operations = (users.items || []).filter(
			(u) => u.role === "user",
		).length;

		contentPanel.innerHTML =
			'<h2>Dashboard Overview</h2><p class="muted">Legacy QC admin controls in one screen.</p>' +
			'<div class="stats-grid">' +
			'<article class="stat-card"><div class="muted">Total Users</div><div class="value">' +
			users.total +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Doctor Users</div><div class="value">' +
			doctors +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Operations Users</div><div class="value">' +
			operations +
			"</div></article>" +
			"</div>" +
			'<div class="stats-grid" style="margin-top:12px;">' +
			'<article class="stat-card"><div class="muted">Claim Rules</div><div class="value">' +
			rules.total +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Diagnosis Criteria</div><div class="value">' +
			criteria.total +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Pending Suggestions</div><div class="value">' +
			suggestions.total +
			"</div></article>" +
			"</div>" +
			...
			importForm.addEventListener("submit", async function (e) {
				...
				try {
					const result = await apiFetch(
						"/api/v1/admin/analysis/import-sql" + qs,
						{ method: "POST", body: fd },
					);
					const msg =
						"Import complete. Processed: " +
						String(result.processed || 0) +
						", matched claim: " +
						String(result.matched_claim || 0) +
						", inserted reports: " +
						String(result.reports_inserted || 0) +
						", updated reports: " +
						String(result.reports_updated || 0) +
						", missing claim: " +
						String(result.no_claim_match || 0) +
						", no report html: " +
						String(result.no_report_html || 0);
	...
	async function renderDoctorDashboard() {
		...
		const doctorFromQuery = String(
			dashboardUrl.searchParams.get("doctor_username") || "",
		).trim();
		const doctorId =
			doctorFromQuery || String((me && me.username) || "").trim();
		const doctorParam = doctorId
			? "&assigned_doctor_id=" + encodeURIComponent(doctorId)
			: "";
		...
		const [
			...
			apiFetch(
				"/api/v1/user-tools/claim-document-status?" + openParams.toString(),
			),
			apiFetch(
				"/api/v1/user-tools/doctor-completion-stats?doctor_username=" +
		...
		const withdrawnCount = Number(
			(withdrawnSummary && withdrawnSummary.total) || 0,
		);
		const totalAssigned = Math.max(0, rawAssigned - withdrawnCount);
		const pendingCount = Math.max(0, totalAssigned - doneCount);

		const monthWiseClosed = Array.isArray(
			completionStats && completionStats.month_wise_closed,
		)
			? completionStats.month_wise_closed
			: [];
		let activeMonth = String(
			(completionStats && completionStats.selected_month) || "",
		).trim();
		...
		const visibleItems = (function () {
			...
			const orderedOpenItems = sortClaimsByAllotmentDateFirst(
				(openClaimsPayload && openClaimsPayload.items) || [],
				true,
			);
			if (orderedOpenItems.length > 0) {
				return orderedOpenItems.map(function (item) {
					...
					return {
						...
						status:
							String(
								item.status_display || item.status || base.status || "",
							).trim() || "-",
						assigned_doctor_id:
							String(
								item.assigned_doctor_id || base.assigned_doctor_id || "",
							).trim() || "-",
						created_at:
							item.allotment_date ||
							item.assigned_at ||
			...
			return monthWiseClosed
				.map(function (item) {
					...
					return (
						...
						"</td>" +
						'<td><button type="button" class="btn-soft doctor-month-drill-btn' +
						(isActive ? " is-active" : "") +
						'" data-month="' +
						escapeHtml(monthKey) +
						'">View Day-wise</button></td>' +
						"</tr>"
					);
				})
				.join("");
		...
		function renderDayWiseTable(dayItems) {
			const rows = (Array.isArray(dayItems) ? dayItems : [])
				.map(function (item) {
					return (
						"<tr>" +
						"<td>" +
						escapeHtml(String((item && item.date) || "-")) +
						"</td>" +
						"<td>" +
						escapeHtml(String((item && item.closed) || 0)) +
						"</td>" +
						"</tr>"
					);
				})
				.join("");
			return (
				"<table><thead><tr><th>Date</th><th>Closed Cases</th></tr></thead><tbody>" +
				(rows ||
					'<tr><td colspan="2">No day-wise completed cases found.</td></tr>') +
				"</tbody></table>"
			);
		}

		function readDayWise(payload) {
			return Array.isArray(payload && payload.day_wise_closed)
				? payload.day_wise_closed
				: [];
		}

		const initialDayWise = readDayWise(completionStats);
		contentPanel.innerHTML =
			'<h2>Dashboard Overview</h2><p class="muted">Quick summary of your assigned and completed workload.</p>' +
			'<div class="stats-grid">' +
			'<article class="stat-card"><div class="muted">Assigned Cases</div><div class="value">' +
			totalAssigned +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Pending Cases</div><div class="value">' +
			pendingCount +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Done Cases</div><div class="value">' +
			doneCount +
			"</div></article>" +
			"</div>" +
			...
			(renderMonthRows(activeMonth) ||
				'<tr><td colspan="3">No closed cases found.</td></tr>') +
			"</tbody></table></div>" +
			'<h3 style="margin-top:18px" id="doctor-daywise-title">Day-wise Closed Cases' +
			(activeMonth ? " (" + escapeHtml(activeMonth) + ")" : "") +
			"</h3>" +
			'<div class="table-wrap" id="doctor-daywise-wrap">' +
			renderDayWiseTable(initialDayWise) +
			"</div>" +
		...
		const monthButtons = Array.from(
			contentPanel.querySelectorAll(".doctor-month-drill-btn"),
		);
		...
		monthButtons.forEach(function (btn) {
			btn.addEventListener("click", async function () {
				...
				if (dayWiseTitleEl)
					dayWiseTitleEl.textContent =
						"Day-wise Closed Cases (" + monthKey + ")";
				if (dayWiseWrapEl) {
					dayWiseWrapEl.innerHTML =
						'<table><thead><tr><th>Date</th><th>Closed Cases</th></tr></thead><tbody><tr><td colspan="2">Loading...</td></tr></tbody></table>';
				}
				try {
					const drilldown = await apiFetch(
						"/api/v1/user-tools/doctor-completion-stats?month=" +
					...
					if (dayWiseWrapEl)
						dayWiseWrapEl.innerHTML = renderDayWiseTable(dayItems);
				} catch (err) {
					if (dayWiseWrapEl) {
						dayWiseWrapEl.innerHTML =
							'<table><thead><tr><th>Date</th><th>Closed Cases</th></tr></thead><tbody><tr><td colspan="2">Failed to load day-wise closed cases.</td></tr></tbody></table>';
					}
				} finally {
					monthButtons.forEach(function (b) {
						b.disabled = false;
	...
	async function renderUserDashboard() {
		...
		const dayWiseItems = Array.isArray(
			dashboardData && dashboardData.day_wise_completed,
		)
			? dashboardData.day_wise_completed
			: [];
		const assigneeItems = Array.isArray(
			dashboardData && dashboardData.assignee_wise,
		)
		...
		const dayWiseRows = dayWiseCurrentMonthItems
			.map(function (item) {
				return (
					"<tr>" +
					"<td>" +
					escapeHtml(String(item && item.date ? item.date : "-")) +
					"</td>" +
					"<td>" +
					escapeHtml(String(item && item.completed ? item.completed : 0)) +
					"</td>" +
					"</tr>"
				);
			})
		...
		const assigneeRows = assigneeItems
			.map(function (item) {
				return (
					"<tr>" +
					"<td>" +
					escapeHtml(String(item && item.username ? item.username : "-")) +
					"</td>" +
					"<td>" +
					escapeHtml(
						formatRoleLabelGlobal(item && item.role ? item.role : ""),
					) +
					"</td>" +
					"<td>" +
					escapeHtml(String(item && item.completed ? item.completed : 0)) +
					"</td>" +
					"<td>" +
					escapeHtml(String(item && item.pending ? item.pending : 0)) +
					"</td>" +
					"<td>" +
					escapeHtml(String(item && item.total ? item.total : 0)) +
					"</td>" +
					"</tr>"
				);
			})
		...
		contentPanel.innerHTML =
			'<h2>Dashboard Overview</h2><p class="muted">Day-wise completed and pending status across assigned cases.</p>' +
			'<div class="stats-grid">' +
			'<article class="stat-card"><div class="muted">Ready for Assignment</div><div class="value">' +
			ready.total +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Waiting for Documents</div><div class="value">' +
			waiting.total +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">In Review</div><div class="value">' +
			inReview.total +
			"</div></article>" +
			'</div><div class="stats-grid" style="margin-top:12px;">' +
			'<article class="stat-card"><div class="muted">Completed</div><div class="value">' +
			completed.total +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Withdrawn</div><div class="value">' +
			withdrawn.total +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Total Claims</div><div class="value">' +
			allClaims.total +
			"</div></article>" +
			"</div>" +
			...
			(dayWiseRows ||
				'<tr><td colspan="2">No completed cases found.</td></tr>') +
			"</tbody></table></div>" +
			...
			(assigneeRows ||
				'<tr><td colspan="5">No assignee data found.</td></tr>') +
			"</tbody></table></div>";
	}
	...
	async function renderPaymentSheet() {
		...
		contentPanel.innerHTML =
			'<h2>Payment Sheet</h2><p class="muted">Previous month payment sheet based on completed cases and configured rate.</p>' +
		...
		async function loadSheet() {
			...
			if (!/^\d{4}-\d{2}$/.test(monthVal)) {
				setMessage(
					"payment-sheet-msg",
					"err",
					"Please select month in YYYY-MM format.",
				);
				return;
			}
			const includeZero =
				String((includeZeroEl && includeZeroEl.value) || "1") === "1";
			const qs =
				"month=" +
				encodeURIComponent(monthVal) +
				"&include_zero_cases=" +
				encodeURIComponent(includeZero ? "true" : "false");

			try {
				setMessage("payment-sheet-msg", "", "Loading payment sheet...");
				const payload = await apiFetch(
					"/api/v1/user-tools/payment-sheet?" + qs,
				);
				...
				const rows = items
					.map(function (item) {
						return (
							...
							"</td>" +
							"<td>" +
							escapeHtml(String(item && item.rate_raw ? item.rate_raw : "0")) +
							"</td>" +
							"<td>" +
							escapeHtml(
								String(
									item && item.completed_cases != null
							...
							escapeHtml(
								formatMoney(
									item && item.amount_total != null ? item.amount_total : 0,
								),
							) +
							"</td>" +
							"</tr>"
				...
				tbodyEl.innerHTML =
					rows ||
					'<tr><td colspan="5">No payment rows found for selected month.</td></tr>';
				summaryEl.textContent =
					"Month: " +
					String(
						payload && payload.month_label ? payload.month_label : monthVal,
					) +
					" | Total Users: " +
					String(
						payload && payload.total_users != null ? payload.total_users : 0,
					) +
					" | Total Cases: " +
					String(
						payload && payload.total_cases != null ? payload.total_cases : 0,
					) +
					" | Total Amount: " +
					formatMoney(
						payload && payload.total_amount != null ? payload.total_amount : 0,
					);
				setMessage("payment-sheet-msg", "ok", "Payment sheet loaded.");
			} catch (err) {
				tbodyEl.innerHTML =
					'<tr><td colspan="5">Failed to load payment sheet.</td></tr>';
				summaryEl.textContent = "-";
				setMessage(
					"payment-sheet-msg",
					"err",
					err && err.message ? err.message : "Failed to load payment sheet.",
				);
			}
		}
	...
	function renderCreateUser() {
		contentPanel.innerHTML =
			'<h2>Create User</h2><p class="muted">Create doctor/user/super_admin/auditor accounts.</p>' +
	...
	async function renderUserBankDetails() {
		...
		contentPanel.innerHTML =
			'<h2>Bank Details</h2><p class="muted">Manage bank details for super admins and doctors.</p>' +
		...
		function setFormValues(item) {
			...
			document.getElementById("bank-account-holder").value = String(
				row.account_holder_name || "",
			);
			document.getElementById("bank-name").value = String(row.bank_name || "");
			document.getElementById("bank-branch").value = String(
				row.branch_name || "",
			);
			document.getElementById("bank-account-number").value = String(
				row.account_number || "",
			);
			document.getElementById("bank-rate").value = String(
				row.payment_rate || "",
			);
			...
			document.getElementById("bank-is-active").checked =
				row.bank_is_active !== false;
			setIfscStatus("", false);
		}

		function resetFormToCurrentItem() {
			const userId = String(state.editingUserId || "").trim();
			const target = state.items.find(function (it) {
				return String((it && it.user_id) || "") === userId;
			});
			if (target) {
		...
		function renderRows(items) {
			const rows = (items || [])
				.map(function (item) {
					...
					return (
						...
						"</td>" +
						"<td>" +
						escapeHtml(String(item && item.bank_name ? item.bank_name : "-")) +
						"</td>" +
						"<td>" +
						escapeHtml(
							String(item && item.account_number ? item.account_number : "-"),
						) +
						"</td>" +
						"<td>" +
						escapeHtml(
							String(item && item.payment_rate ? item.payment_rate : "-"),
						) +
						"</td>" +
						"<td>" +
						escapeHtml(String(item && item.ifsc_code ? item.ifsc_code : "-")) +
						"</td>" +
						"<td>" +
						escapeHtml(String(item && item.upi_id ? item.upi_id : "-")) +
						"</td>" +
						"<td>" +
						statusChip(
							item && item.bank_is_active === false ? "inactive" : "active",
						) +
						"</td>" +
						"<td>" +
						escapeHtml(
							String(item && item.updated_by ? item.updated_by : "-"),
						) +
						"</td>" +
						"<td>" +
						escapeHtml(
							formatDateTime(item && item.updated_at ? item.updated_at : ""),
						) +
						"</td>" +
						'<td><button type="button" class="btn-soft bank-edit-btn" data-user-id="' +
						escapeHtml(id) +
						'">Edit</button></td>' +
						"</tr>"
					);
			...
			Array.from(contentPanel.querySelectorAll(".bank-edit-btn")).forEach(
				function (btn) {
		...
		async function loadData(resetOffset) {
			...
			try {
				...
				const payload = await apiFetch(
					"/api/v1/auth/user-bank-details?" + params.join("&"),
				);
				state.items = Array.isArray(payload && payload.items)
					? payload.items
					: [];
				state.total = Number(payload && payload.total ? payload.total : 0);
				state.limit = Number(
					payload && payload.limit ? payload.limit : state.limit,
				);
				state.offset = Number(
					payload && payload.offset ? payload.offset : state.offset,
				);
		...
		pagePrevBtn.addEventListener("click", function () {
			state.offset = Math.max(
				0,
				Number(state.offset || 0) - Number(state.limit || 25),
			);
			loadData(false);
		});
		...
		if (ifscVerifyBtn) {
			ifscVerifyBtn.addEventListener("click", async function () {
				...
				const rawIfsc = String((ifscInput && ifscInput.value) || "")
					.toUpperCase()
					.replace(/\s+/g, "")
					.trim();
				if (!rawIfsc) {
				...
				try {
					const verify = await apiFetch(
						"/api/v1/auth/ifsc/verify/" + encodeURIComponent(rawIfsc),
					);
					if (ifscInput)
						ifscInput.value = String(
							(verify && verify.ifsc_code) || rawIfsc,
						).trim();
					const bankName = String((verify && verify.bank_name) || "").trim();
					const branchName = String(
						(verify && verify.branch_name) || "",
					).trim();
					if (bankName) document.getElementById("bank-name").value = bankName;
					if (branchName)
						document.getElementById("bank-branch").value = branchName;
					setIfscStatus(
						"IFSC verified: " + (bankName || "-") + " / " + (branchName || "-"),
						false,
					);
				} catch (err) {
					setIfscStatus(
						err && err.message ? err.message : "IFSC verification failed.",
						true,
					);
				} finally {
		...
		form.addEventListener("submit", async function (e) {
			...
			try {
				...
				await apiFetch(
					"/api/v1/auth/user-bank-details/" + encodeURIComponent(userId),
					{
						method: "PUT",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify(payload),
				...
				setModalMessage(
					"err",
					err && err.message ? err.message : "Failed to save bank details.",
				);
			} finally {
	...
	function renderChangePassword() {
		contentPanel.innerHTML =
			'<h2>Change My Password</h2><p class="muted">Policy: minimum 8 chars with uppercase, lowercase and number.</p>' +
	...
	function renderResetUserPassword() {
		contentPanel.innerHTML =
			'<h2>Reset Any User Password</h2><p class="muted">Super admin only.</p>' +
	...
	async function renderAssignCases() {
		...
		contentPanel.innerHTML =
			'<section class="claim-status-panel">' +
			...
			"</select></div>" +
			'<div class="claim-filter-group"><label for="claim-doctor-filter">Doctor Filter</label><select id="claim-doctor-filter" name="doctor_filter">' +
			doctorFilterOptions +
			"</select></div>" +
			'<div class="claim-filter-action"><button type="submit" class="claim-apply-btn">Apply</button></div>' +
			"</form>" +
			'<div class="claim-status-toolbar">' +
			'<div class="claim-filter-group"><label for="claim-assign-doctor">Assign Doctor</label><select id="claim-assign-doctor" name="assign_doctor">' +
			assignDoctorOptions +
			"</select></div>" +
		...
		const assignSortButtons = Array.from(
			contentPanel.querySelectorAll("button[data-assign-sort]"),
		);
		...
		function getSelectedClaimIds() {
			return Array.from(
				tbody.querySelectorAll("input[data-claim-select]:checked"),
			)
		...
		function updateAssignSortButtonsUi() {
			assignSortButtons.forEach(function (btn) {
				...
				const baseLabel = String(
					btn.getAttribute("data-sort-label") || "",
				).trim();
				const active = key && key === assignSortState.key;
				btn.classList.toggle("is-active", !!active);
				btn.textContent = active
					? baseLabel + (assignSortState.dir === "asc" ? " ^" : " v")
					: baseLabel;
			});
		}
		...
		function applyAssignSort() {
			...
			const decorated = rows.map(function (row, idx) {
				...
				const value = parseAssignSortValue(
			...
			decorated.sort(function (a, b) {
				...
				if (typeof a.value === "number" && typeof b.value === "number") {
					...
					cmp = String(a.value).localeCompare(String(b.value), undefined, {
						numeric: true,
						sensitivity: "base",
					});
				}
		...
		async function assignClaims(claimIds) {
			...
			for (const claimId of claimIds) {
				try {
					await apiFetch(
						"/api/v1/claims/" + encodeURIComponent(claimId) + "/assign",
						{
							method: "PATCH",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({
			...
			if (!failed.length) {
				setMessage(
					"assign-msg",
					"ok",
					successCount + " case(s) assigned to " + doctorId + ".",
				);
			} else {
				setMessage(
					"assign-msg",
					"err",
					successCount + " assigned, " + failed.length + " failed.",
				);
			}
		...
		async function loadRows(resetPage) {
			...
			const allotmentDate = String(
				document.getElementById("claim-allotment-date").value || "",
			).trim();
			const statusFilter = String(
				document.getElementById("claim-status-filter").value || "all",
			).trim();
			...
			const result = await apiFetch(
				"/api/v1/user-tools/claim-document-status?" + params.toString(),
			);
			...
			const rows = (result.items || [])
				.map((c) => {
					...
					const treatmentType = formatTreatmentType(
						c.treatment_type || c.claim_type,
					);
					return (
						"<tr>" +
						'<td class="claim-select-col"><input type="checkbox" data-claim-select value="' +
						escapeHtml(c.id) +
						'"></td>' +
						"<td>" +
						escapeHtml(c.external_claim_id || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(treatmentType || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(assignedDoctor || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(formatDateOnly(c.allotment_date)) +
						"</td>" +
						"<td>" +
						statusChip(formatStatusText(c.status_display || c.status)) +
						"</td>" +
						"<td>" +
						escapeHtml(String(c.documents || 0)) +
						"</td>" +
						"<td>" +
						escapeHtml(formatDateTime(c.last_upload)) +
						"</td>" +
						'<td><button type="button" class="btn-soft claim-status-action-btn" data-assign-claim="' +
						escapeHtml(c.id) +
						'">Assign</button></td>' +
						"</tr>"
					);
			...
			tbody.innerHTML =
				rows ||
				'<tr><td colspan="9">No claims found for selected filter.</td></tr>';
			selectAllEl.checked = false;

			tbody.querySelectorAll("button[data-assign-claim]").forEach((btn) => {
				btn.addEventListener("click", async function () {
					const claimId = String(
						this.getAttribute("data-assign-claim") || "",
					).trim();
					if (!claimId) return;
	...
	function isKycIdentityDocName(value) {
		...
		return /(aadhar|aadhaar|\bkyc\b|pan\s*card|\bvoter\s*id\b|passport|driving\s*license|\bid\s*proof\b|identity\s*proof|\be-?kyc\b|\bckyc\b|frm[-_ ]?id)/i.test(
			name,
		);
	}
	...
	function isLikelyOrgName(value) {
		...
		if (!v) return false;
		return /(hospital|clinic|diagnostic|laboratory|\blab\b|society|insurance|limited|\bltd\b|llp|plaza)/i.test(
			v,
		);
	}
	...
	async function runCasePreparationPipeline(claimUuid, actorId, options) {
		...
		const extractionProviderRaw = String(opts.extractionProvider || "auto")
			.trim()
			.toLowerCase();
		const extractionProvider = [
			...
			"aws_textract",
			"hybrid_local",
		].includes(extractionProviderRaw)
			? extractionProviderRaw
			: "auto";
		const extractionProviderLabel =
			extractionProvider === "openai"
				? "VerifAI"
				: extractionProvider === "aws_textract"
					? "AWS Textract"
					: extractionProvider;
		...
		const onLog =
			typeof opts.onLog === "function" ? opts.onLog : function () {};
		const batchThresholdBytes = Number(
			opts.batchThresholdBytes || 8 * 1024 * 1024,
		);
		...
		function shouldDisableOpenAIBatchFallback(err) {
			...
			if (msg.indexOf("unauthorized") >= 0 || msg.indexOf("forbidden") >= 0)
				return true;
			if (
				msg.indexOf("invalid api key") >= 0 ||
				msg.indexOf("invalid_api_key") >= 0
			)
				return true;
			...
			if (msg.indexOf("connection") >= 0 && msg.indexOf("openai") >= 0)
				return true;
			if (msg.indexOf("timed out") >= 0 && msg.indexOf("openai") >= 0)
				return true;
			if (msg.indexOf("openai extraction failed") >= 0) return true;
			return false;
		}

		const docsResult = await apiFetch(
			"/api/v1/claims/" +
				encodeURIComponent(claimUuid) +
				"/documents?limit=200&offset=0",
		);
		const docs = Array.isArray(docsResult && docsResult.items)
			? docsResult.items
			: [];
		...
		onLog(
			"Extraction mode: provider=" +
				...
				", files > " +
				formatBytes(batchThresholdBytes) +
				" run in single mode, others run in batches of " +
				String(batchSize) +
				", auto fallback=" +
				(allowAutoFallback ? "enabled" : "disabled") +
				".",
		);
		...
		async function processDoc(doc, forceSingleMode) {
			...
			const docName = String(
				row.file_name || docId || "Document " + String(processedCount + 1),
			);
			const sizeBytes = Number(row.file_size_bytes || 0);
			if (!docId) return;

			processedCount += 1;
			emitProgress("doc_start", docName, {
				document_id: docId,
			...
			try {
				if (!force) {
					const existing = await apiFetch(
						"/api/v1/documents/" +
							...
							"/extractions?limit=1&offset=0",
					);
					const alreadyExtracted =
						Number(existing && existing.total ? existing.total : 0) > 0;
					if (alreadyExtracted) {
						skippedCount += 1;
				...
				if (
					...
					!(
						allowAutoFallback &&
						openaiDisabledForBatch &&
						extractionProvider === "openai"
					)
				) {
					try {
						const extractionResult = await apiFetch(
							"/api/v1/documents/" + encodeURIComponent(docId) + "/extract",
							{
								method: "POST",
								headers: { "Content-Type": "application/json" },
								body: JSON.stringify({
						...
						const fallbackMeta =
							...
							(extractionResult.raw_response.openai_rate_limit_fallback ||
								extractionResult.raw_response.openai_fallback)
								? extractionResult.raw_response.openai_rate_limit_fallback ||
									extractionResult.raw_response.openai_fallback
								: null;
						if (fallbackMeta) {
							...
							const usedProvider = String(
								fallbackMeta.used_fallback_provider || "fallback",
							);
							if (reason) {
								onLog(
									"OpenAI unavailable (" +
								...
								onLog(
									"OpenAI fallback; extracted with " +
										usedProvider +
										": " +
										docName,
								);
							...
							onLog(
								"Extracted with " + extractionProviderLabel + ": " + docName,
							);
						}
					} catch (err) {
						lastError = err;
						...
						if (
							...
							extractionProvider === "openai" &&
							!openaiDisabledForBatch &&
							shouldDisableOpenAIBatchFallback(err)
						) {
							openaiDisabledForBatch = true;
							onLog(
								"VerifAI seems unavailable in this environment; switching remaining documents to auto extraction fallback.",
							);
						}
					}
				}
				...
				if (
					...
					try {
						const openaiExtract = await apiFetch(
							"/api/v1/documents/" + encodeURIComponent(docId) + "/extract",
							{
								method: "POST",
								headers: { "Content-Type": "application/json" },
						...
						extracted = true;
						extractedProvider = "hybrid_local";
						onLog("Extracted with hybrid_local: " + docName);
						if (openaiExtract && openaiExtract.raw_response) {
							const rawJson = JSON.stringify(openaiExtract.raw_response);
							onLog(
								"VerifAI JSON response: " +
									String(
										rawJson.length > 4000
											? rawJson.slice(0, 4000) + " ...[truncated]"
											: rawJson,
									),
							);
						}
						...
						if (strictOpenAI) {
							onLog(
								"VerifAI extraction failed (strict mode): " +
									docName +
									" (" +
									String(err && err.message ? err.message : err) +
									")",
							...
							onLog(
								"VerifAI extraction fallback to auto for: " +
									docName +
									" (" +
									String(err && err.message ? err.message : err) +
									")",
				...
				if (extracted) {
					...
					emitProgress("doc_done", docName, {
						document_id: docId,
						result: "extracted",
						provider: extractedProvider || extractionProvider,
					});
				} else {
					failedCount += 1;
					onLog(
		...
		for (let idx = 0; idx < docs.length; idx += 1) {
			...
			const isLarge =
				Number.isFinite(sizeBytes) && sizeBytes > batchThresholdBytes;
		...
		try {
			...
			try {
				await apiFetch(
					"/api/v1/claims/" +
						encodeURIComponent(claimUuid) +
						"/structured-data?auto_generate=true&use_llm=false",
				);
			...
			onLog(
				"Checklist evaluation completed. Recommendation: " +
					String((checklist && checklist.recommendation) || "unknown") +
					(failedCount > 0
						? " | extraction failures: " + String(failedCount)
						: ""),
			);
			emitProgress("checklist_done", "Checklist evaluation", {
				recommendation:
					checklist && checklist.recommendation
						? String(checklist.recommendation)
						: "",
			});
			const mergedReviewMeta =
				checklist &&
				checklist.source_summary &&
			...
			if (mergedReviewMeta && mergedReviewMeta.used) {
				onLog(
					"Merged AI medical-audit review applied. Admission required: " +
						...
						", confidence: " +
						String(mergedReviewMeta.confidence || 0),
				);
			} else if (mergedReviewMeta && mergedReviewMeta.error) {
				onLog(
					"Merged AI medical-audit review fallback: " +
						String(mergedReviewMeta.error),
				);
			}
		} catch (err) {
			checklistError = err;
			...
			emitProgress("checklist_failed", "Checklist evaluation", {
				error: String(err && err.message ? err.message : err),
			});
		}
	...
	async function renderDoctorAssignedCases() {
		...
		function renderFinalStatus(raw) {
			const normalized = String(raw || "Pending").replace(
				/<br\s*\/?\s*>/gi,
				"\n",
			);
		...
		async function openCaseDetail(
			...
			if (!claimKey) {
				setMessage(
					"doctor-assigned-msg",
					"err",
					"Claim key not available for opening detail page.",
				);
				return;
			}
			...
			return await new Promise((resolve) => {
				...
				if (saveBtn) {
					saveBtn.addEventListener("click", function () {
						closeWith(
							String(
								selectEl && selectEl.value ? selectEl.value : fallbackValue,
							),
						);
					});
				}
		...
		async function changeStatus(claimId, currentStatusRaw) {
			...
			const statusPayload =
				selectedStatus === "completed" ? "completed" : "in_review";
			try {
				await apiFetch(
					"/api/v1/claims/" + encodeURIComponent(claimId) + "/status",
					{
						method: "PATCH",
						headers: { "Content-Type": "application/json" },
				...
				setMessage(
					"doctor-assigned-msg",
					"ok",
					"Status updated to " + formatStatusText(selectedStatus) + ".",
				);
				await loadRows(false);
			} catch (err) {
		...
		async function loadRows(resetPage) {
			...
			const searchClaim = String(
				document.getElementById("doctor-claim-search").value || "",
			).trim();
			const allotmentDate = String(
				document.getElementById("doctor-allotment-date").value || "",
			).trim();
			...
			const sortedItems = sortClaimsByAllotmentDateFirst(
				result.items || [],
				true,
			);
			const rows = sortedItems
				.map((c) => {
					...
					return (
						...
						"</td>" +
						"<td>" +
						escapeHtml(formatDateTime(c.assigned_at)) +
						"</td>" +
						"<td>" +
						escapeHtml(formatDateOnly(c.allotment_date)) +
						"</td>" +
						'<td class="doctor-final-status">' +
						renderFinalStatus(c.final_status) +
						"</td>" +
						'<td><div class="doctor-case-actions">' +
						'<button type="button" class="btn-soft" data-open-case="' +
						escapeHtml(c.external_claim_id || "") +
						'" data-open-claim-id="' +
						escapeHtml(c.id || "") +
						'">Open Case</button>' +
						'<button type="button" class="btn-soft" data-change-status="' +
						escapeHtml(c.id) +
						'" data-current-status="' +
						escapeHtml(c.status || "") +
			...
			tbody.innerHTML =
				rows || '<tr><td colspan="9">No assigned claims found.</td></tr>';
		...
		try {
			...
			setMessage("doctor-assigned-msg", "err", err.message);
			tbody.innerHTML =
				'<tr><td colspan="8">Failed to load assigned cases.</td></tr>';
			updatePaginationUi();
		}
	}
	async function renderDoctorCaseDetail() {
		...
		const backSearchClaim = String(
			routeParams.get("search_claim") || "",
		).trim();
		const backAllotmentDate = String(
			routeParams.get("allotment_date") || "",
		).trim();
		const backPageParam = String(routeParams.get("back_page") || "").trim();
		const routeInfo = parseRoute();
		const activeRouteRole = String(
			(routeInfo && routeInfo.routeRole) || (me && me.role) || "doctor",
		).trim();
		const isAuditorRole =
			activeRouteRole === "auditor" || !!(me && me.role === "auditor");
		const canSubmitReview =
			activeRouteRole === "super_admin" ||
			activeRouteRole === "user" ||
			activeRouteRole === "auditor";
		const backPage =
			backPageParam ||
			(activeRouteRole === "user"
				? "upload-document"
				: isAuditorRole
					? "audit-claims"
					: "assigned-cases");
		const preferredReportSourceParam = String(
			routeParams.get("report_source") || "doctor",
		)
			.trim()
			.toLowerCase();
		let preferredReportSource =
			preferredReportSourceParam === "system" ||
			preferredReportSourceParam === "doctor"
				? preferredReportSourceParam
				: "doctor";
		...
		if (!claimUuid) {
			contentPanel.innerHTML =
				...
				'<div class="link-row"><a class="btn-soft" href="/qc/' +
				escapeHtml(activeRouteRole) +
				"/" +
				escapeHtml(backPage) +
				'">Back</a></div>' +
				"</section>";
		...
		contentPanel.innerHTML =
			'<section class="claim-status-panel case-detail-panel">' +
			...
			'<div><h3 class="brain-panel__title">AI Brain</h3><div class="muted-small" id="case-brain-status">Decision + risk + conflicts summary.</div></div>' +
			"</div>" +
			...
			'<div class="brain-conflicts" id="case-ai-ml-analysis"></div>' +
			(activeRouteRole === "super_admin"
				? '<div class="brain-conflicts" id="case-ai-usage-summary"></div>' +
					'<div class="brain-conflicts" id="case-ai-total-cost"></div>'
				: "") +
		...
		const diagnosisChecklistBtn = document.getElementById(
			"case-diagnosis-checklist",
		);
		const diagnosisChecklistResultEl = document.getElementById(
			"case-diagnosis-checklist-result",
		);
		...
		function renderCaseBrain(decideResp, checklistLatest) {
			...
			const finalDecisionEl = document.getElementById(
				"case-brain-final-decision",
			);
			...
			const aiConfidenceEl = document.getElementById(
				"case-brain-ai-confidence",
			);
			...
			const aiMlEl = document.getElementById("case-ai-ml-analysis");
			const usageEl = document.getElementById("case-ai-usage-summary");
			const totalCostEl = document.getElementById("case-ai-total-cost");

			if (!finalDecisionEl && !statusEl) return;

			const finalDecision =
				decideResp && decideResp.final_status
					? String(decideResp.final_status)
					: "";
			const route =
				decideResp && decideResp.route_target
					? String(decideResp.route_target)
					: "";
			const confidence =
				decideResp && typeof decideResp.confidence === "number"
					? decideResp.confidence
					: null;
			const riskScore =
				decideResp && typeof decideResp.risk_score === "number"
					? decideResp.risk_score
					: null;
			const conflicts =
				decideResp && Array.isArray(decideResp.conflicts)
					? decideResp.conflicts
					: [];

			const aiDecision =
				checklistLatest && checklistLatest.ai_decision
					? String(checklistLatest.ai_decision)
					: "";
			const aiConfidence =
				checklistLatest && typeof checklistLatest.ai_confidence === "number"
					? checklistLatest.ai_confidence
					: null;
			const mlPred =
				decideResp &&
				decideResp.ml_prediction &&
				typeof decideResp.ml_prediction === "object"
					? decideResp.ml_prediction
					: null;
			const mlAvailable = !!(mlPred && mlPred.available === true);
			const mlLabel = mlPred && mlPred.label ? String(mlPred.label) : "";
			const mlConf =
				mlPred && typeof mlPred.confidence === "number"
					? mlPred.confidence
					: null;
			...
			function normalizeDecisionLabel(value) {
				...
				if (v === "query" || v === "need_more_evidence" || v === "manual_review")
					return "QUERY";
			...
			if (statusEl) {
				const mapping =
					decideResp && decideResp.final_status_mapping
						? String(decideResp.final_status_mapping)
						: "";
				const mlText = mlAvailable
					? " | ML: " +
						(mlLabel || "available") +
						(mlConf != null ? " (" + String(mlConf) + ")" : "")
					: " | ML: not available";
				statusEl.textContent =
					(mapping ? "Latest mapping: " + mapping : "AI decide completed.") +
					mlText;
			}
			...
			if (aiConfidenceEl)
				aiConfidenceEl.textContent =
					aiConfidence != null ? String(aiConfidence) : "-";
			if (riskEl)
			...
			if (conflictsEl) {
				if (!conflicts.length) {
					conflictsEl.innerHTML =
						'<li class="muted-small">No conflicts detected.</li>';
				} else {
					conflictsEl.innerHTML = conflicts
						.slice(0, 12)
						.map(function (c) {
							const msg =
								c && c.message ? String(c.message) : JSON.stringify(c);
			...
			if (aiMlEl) {
				...
				const mlDecisionLabel = mlAvailable
					? normalizeDecisionLabel(mlLabel)
					: "-";
				const agreement =
				...
				aiMlEl.innerHTML =
					...
					'<div class="muted-small">AI Decision: ' +
					escapeHtml(aiDecisionLabel) +
					"<br>AI Confidence: " +
					escapeHtml(formatConfidence(aiConfidence != null ? aiConfidence : confidence)) +
					"<br><br>ML Decision: " +
					escapeHtml(mlDecisionLabel) +
					"<br>ML Confidence: " +
					escapeHtml(formatConfidence(mlConf)) +
					"<br><br>Agreement: " +
					escapeHtml(agreement) +
					"</div>";
			}

			const isAdminRole = activeRouteRole === "super_admin";
			if (usageEl) {
				if (!isAdminRole) {
			...
			if (totalCostEl) {
				if (!isAdminRole) {
					...
					const costs =
						(decideResp && typeof decideResp.ai_cost_breakdown === "object" && decideResp.ai_cost_breakdown) ||
					...
					totalCostEl.innerHTML =
						...
						'<div class="muted-small">Extraction: ' +
						escapeHtml(formatUsd(extractionCost)) +
						"<br>Structuring: " +
						escapeHtml(formatUsd(structuringCost)) +
						"<br>Reasoning: " +
						escapeHtml(formatUsd(reasoningCost)) +
						"<br>Report: " +
						escapeHtml(formatUsd(reportCost)) +
						"<br><br>Total: " +
						escapeHtml(formatUsd(totalCost)) +
						"</div>";
				}
			}
		}
		...
		async function runCaseAiDecide() {
			const confirmed = window.confirm(
				"Run AI decision for this claim now? This may take some time.",
			);
			if (!confirmed) return;
			...
			setMessage(
				"case-detail-msg",
				"",
				"Running AI decision (/decide)... this can take a few minutes.",
			);
			appendLog("AI decide started.");
			try {
				try {
					await apiFetch(
						"/api/v1/claims/" + encodeURIComponent(claimUuid) + "/prepare",
						{
							method: "POST",
							headers: { "Content-Type": "application/json" },
							timeoutMs: 20000,
					...
					appendLog(
						"Claim prepare skipped: " +
							String(
								prepareErr && prepareErr.message
									? prepareErr.message
									: prepareErr,
							),
					);
				...
				const decision = await apiFetch(
					"/api/v1/claims/" + encodeURIComponent(claimUuid) + "/decide",
					{
						method: "POST",
						headers: { "Content-Type": "application/json" },
				...
				try {
					checklistLatest = await apiFetch(
						"/api/v1/claims/" +
							encodeURIComponent(claimUuid) +
							"/checklist/latest",
					);
				...
				setMessage(
					...
					"AI decide done: " +
						String(
							(decision &&
								(decision.recommendation || decision.final_status)) ||
								"ok",
						) +
						".",
				);
				...
				if (String(msg).toLowerCase().indexOf("timed out") >= 0) {
					try {
						setMessage(
							"case-detail-msg",
							"",
							"AI decide still running on server. Waiting for result...",
						);
						...
						for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
							...
							const generatedAt =
								latest && latest.generated_at
									? Date.parse(String(latest.generated_at))
									: NaN;
							if (
								!Number.isNaN(generatedAt) &&
								generatedAt >= startedAt - 5000
							) {
								let checklistLatest = null;
								try {
									checklistLatest = await apiFetch(
								...
								setMessage(
									"case-detail-msg",
									"ok",
									"Loaded latest AI result from server (after timeout).",
								);
		...
		async function submitDoctorVerification() {
			...
			const act = String(doctorActionEl ? doctorActionEl.value : "")
				.trim()
				.toLowerCase();
			...
			try {
				...
				setMessage(
					"case-detail-msg",
					"err",
					err && err.message ? err.message : "Doctor decision submit failed.",
				);
				appendLog(
					"Doctor decision submit failed: " +
						String(err && err.message ? err.message : err),
				);
			} finally {
				setActionDisabled(false);
		...
		async function submitReviewerDecision() {
			...
			const action = String(reviewActionEl ? reviewActionEl.value : "")
				.trim()
				.toLowerCase();
			...
			try {
				const resp = await apiFetch(
					"/api/v1/claims/" + encodeURIComponent(claimUuid) + "/review",
					{
						method: "POST",
						headers: { "Content-Type": "application/json" },
				...
				setMessage(
					...
					"ok",
					"Review submitted: " +
						String((resp && resp.recommendation) || action) +
						".",
				);
				appendLog(
					"Review submitted. Recommendation: " +
						String((resp && resp.recommendation) || ""),
				);
				await loadDetail();
			} catch (err) {
				setMessage(
					"case-detail-msg",
					"err",
					err.message || "Review submit failed.",
				);
				appendLog(
		...
		function renderDiagnosisChecklistResult(payload) {
			...
			if (!payload || typeof payload !== "object") {
				diagnosisChecklistResultEl.className =
					"case-diagnosis-checklist-result muted";
				diagnosisChecklistResultEl.textContent =
					"Diagnosis checklist is not generated yet.";
				return;
			}

			const symptoms = Array.isArray(payload.symptoms)
				? payload.symptoms.filter(Boolean)
				: [];
			const drugChoices = Array.isArray(payload.drug_choices)
				? payload.drug_choices.filter(Boolean)
				: [];
			const investigations = Array.isArray(payload.investigation_findings)
				? payload.investigation_findings.filter(Boolean)
				: [];
			...
			const diagnosisLabel =
				String(payload.diagnosis_name || "-").trim() || "-";
			...
			function renderReferences(refs) {
				...
				return (
					refs
						...
						.filter(Boolean)
						.join("") || '<li class="muted">No references provided</li>'
				);
			}

			diagnosisChecklistResultEl.className = "case-diagnosis-checklist-result";
			diagnosisChecklistResultEl.innerHTML =
				'<div class="case-diagnosis-checklist-result__head">' +
				"<strong>Diagnosis Checklist: " +
				escapeHtml(diagnosisLabel) +
				"</strong>" +
				...
				")" +
				(modelText ? " | Model: " + escapeHtml(modelText) : "") +
				"</span>" +
				"</div>" +
				'<div class="case-diagnosis-checklist-result__grid">' +
				"<section><h4>Symptoms</h4><ul>" +
				renderList(symptoms, "No symptom points available") +
				"</ul></section>" +
				"<section><h4>Drug Choices</h4><ul>" +
				renderList(drugChoices, "No drug choices available") +
				"</ul></section>" +
				"<section><h4>Likely Investigation Findings</h4><ul>" +
				renderList(investigations, "No investigation points available") +
				"</ul></section>" +
				"<section><h4>References</h4><ul>" +
				renderReferences(references) +
				"</ul></section>" +
				"</div>" +
				(cautionNotes
					? '<p class="muted">' + escapeHtml(cautionNotes) + "</p>"
					: "");
		}

		async function resolveDiagnosisForChecklist() {
			const claimIdText = String(
				(currentClaim && currentClaim.id) || claimUuid || "",
			).trim();
			if (!claimIdText) return "";
			try {
				const structured = await apiFetch(
					"/api/v1/claims/" +
						encodeURIComponent(claimIdText) +
						"/structured-data?auto_generate=true&use_llm=false",
				);
				const diagnosis = sanitizeReportText(
					(structured && structured.diagnosis) || "",
				);
			...
			try {
				...
				if (!diagnosisText) {
					setMessage(
						"case-detail-msg",
						"err",
						"Diagnosis could not be auto-generated for checklist generation.",
					);
					appendLog(
						"Diagnosis checklist generation skipped: diagnosis missing after auto-generation.",
					);
					return;
				}
				...
				const cacheText =
					payload && payload.from_cache
						? "loaded from DB cache"
						: "generated and saved to DB";
				setMessage(
					"case-detail-msg",
					"ok",
					"Diagnosis checklist " + cacheText + ".",
				);
				appendLog(
					"Diagnosis checklist ready for " +
						String((payload && payload.diagnosis_name) || diagnosisText) +
						" (" +
						cacheText +
						").",
				);
				...
				setMessage(
					"case-detail-msg",
					"err",
					err.message || "Diagnosis checklist generation failed.",
				);
				appendLog(
					"Diagnosis checklist generation failed: " +
						String(err && err.message ? err.message : err),
				);
			} finally {
				setActionDisabled(false);
		...
		function isJsonLikeText(value) {
			...
			if (
				...
				return true;
			if (
				t.indexOf('\"extracted_entities\"') >= 0 ||
				t.indexOf('\"evidence_refs\"') >= 0 ||
				t.indexOf('\"raw_response\"') >= 0
			)
				return true;
			if (
		...
		function sanitizeReportText(value) {
			...
			const cleanedLines = raw
				...
				.map(function (line) {
					...
					if (
						/^\/[A-Za-z0-9._\/-]+\.(pdf|jpg|jpeg|png|tif|tiff)(?:-[0-9]+)?(?:-\d{1,2}[.\/-]\d{1,2}[.\/-]\d{2,4}\s+\d{1,2}:\d{2}(?::\d{2})?)?$/i.test(
							t,
						)
					)
						return "";
					...
					if (
						/^(?:https?:\/\/)?[^\s]+\.(pdf|jpg|jpeg|png|tif|tiff)(?:\?.*)?$/i.test(
							t,
						)
					)
						return "";
		...
		function sanitizeConclusionText(value) {
			...
			const lines = base
				...
				.map(function (line) {
					...
					if (
						/^(?:type|field|snippet|confidence|raw_response|analysis_snapshot|extracted_entities|evidence_refs)\s*:/i.test(
							t,
						)
					)
						return "";
					if (
						...
						return "";
					if (/[{}\[\]]/.test(t) && /"[^"]+"\s*:/.test(t)) return "";
					if (/bill\s*amount\s*:\s*.*\|\s*investigations\s*:/i.test(t))
						return "";
		...
		function isWeakConclusionText(value) {
			...
			if (
				/^(surgery\s*\/\s*procedure\s*performed|details\s+of\s+medication\s+administered|ipd\s+medicine\s+bill|patient\s+name|hospital\b|admitting\s+dr\b|total\s*[:-])/i.test(
					t,
				)
			)
				return true;
			if (
				/^(admission\s+date|discharge\s+date|claim\s+no\.?|company\s+name)\b/i.test(
					t,
				)
			)
				return true;
			if (
				/^(?:openai_(?:claim_rules|diagnosis_criteria)\s+(?:reject|query|approve|admissible|inadmissible)\s*)+$/i.test(
					t,
				)
			)
				return true;
			if (
				/^based on available records, inpatient admission from /i.test(t) &&
				/medicine evidence:|daily tpr summary:/i.test(t)
			)
				return true;
			if (
				/therefore, admission and treatment are not justified with current evidence\.?$/i.test(
					t,
				)
			)
				return true;
		...
		function mergeRuleWithLearning(ruleText, learningText, prefix, maxLen) {
			...
			if (ruleNorm === learningNorm) return rule;
			if (
				ruleNorm.indexOf(learningNorm) >= 0 ||
				learningNorm.indexOf(ruleNorm) >= 0
			)
				return rule;
			...
			if (!label) {
				const normalizedLearning = learningShort
					.replace(
						/^(?:auditor\s+learning|learning\s+update|learning\s+note)\s*[:\-]\s*/i,
						"",
					)
					.trim();
		...
		function normalizeDoctorNameCandidate(value) {
			...
			if (!text) return "";
			text = text.replace(
				/^(?:treating\s*doctor(?:\s*name)?|consult(?:ant|ing)\s*doctor|attending\s*doctor|doctor(?:\s*name)?)\s*[:\-]\s*/i,
				"",
			);
			...
			text = text
				.replace(
					/\s*\b(?:reg(?:istration)?(?:\s*no|\s*number)?|mci|nmc|dmc|medical council)\b[\s:.\-#]*[A-Za-z0-9\/\-]+.*$/i,
					"",
				)
				.replace(/\s{2,}/g, " ")
				.trim();
			...
			if (
				/\b(?:hospital|clinic|nursing\s*home|medical\s*centre|medical\s*center|diagnostic|laboratory)\b/i.test(
					text,
				)
			)
				return "";
		...
		function extractTreatingDoctorName(extractionPairs) {
			...
			(Array.isArray(extractionPairs) ? extractionPairs : []).forEach(
				function (pair) {
					...
					if (/[{}\[\]]/.test(firstLine) && /"[^"]+"\s*:/.test(firstLine))
						return;
		...
		function looksLikeHospitalAddressText(value) {
			...
			if (
				/\b(?:hospital\s*address|full\s*postal\s*address|address\s*of\s*hospital|hospital\s*addr(?:ess)?)\b/.test(
					low,
				)
			)
				return true;
			if (/^(?:add(?:ress)?\s*[:\-])/.test(low)) return true;
			const addressTokenHits = (
				low.match(
					/\b(?:road|rd|street|st|lane|ln|nagar|colony|city|district|state|pin|pincode|zip|plot|floor|building|plaza|near|opp|opposite|shop|apartment|apt|flat|unit|society|chsl|complex|tower|wing|sector|block|east|west|north|south|taluka|tehsil)\b/g,
				) || []
			).length;
			if (
				/\b(?:shop|apartment|apt|flat|unit)\s*no\.?\s*\d+\b/.test(low) &&
				/\b\d{6}\b/.test(low)
			)
				return true;
			if (
				addressTokenHits >= 2 &&
				(/\d{3,}/.test(low) || /\b(?:pin|pincode|zip)\b/.test(low))
			)
				return true;
			if (
				addressTokenHits >= 1 &&
		...
		function cleanHospitalDisplayName(value, fallbackHospitalName) {
			...
			const match = raw.match(
				/([A-Za-z][A-Za-z .&'-]{2,90}\b(?:Hospital|Clinic|Nursing Home|Medical Centre|Medical Center|Research Centre|Research Center))/i,
			);
			if (match && match[1]) return sanitizeReportText(match[1]);
			if (looksLikeHospitalAddressText(raw)) {
				if (fallback && fallback !== "-" && !isSameEntityText(fallback, raw))
					return fallback;
				return "-";
			}
			return raw;
		...
		function extractHospitalNameFromNarrative(extractionPairs, insuredName) {
			...
			const candidates = [];
			const pattern =
				/([A-Za-z][A-Za-z .&'-]{2,90}\b(?:Hospital|Clinic|Nursing Home|Medical Centre|Medical Center|Research Centre|Research Center))/gi;
		...
		function extractDoctorNameFromNarrative(
			extractionPairs,
			insuredName,
			hospitalName,
		) {
		...
		function normalizeDoctorRegNoCandidate(value) {
			...
			let m = out.match(
				/\b(?:reg(?:istration)?\.?\s*no\.?|mci(?:\s*reg\.?\s*no\.?)?|nmc(?:\s*reg\.?\s*no\.?)?|rmc(?:\s*reg\.?\s*no\.?)?|dmc(?:\s*reg\.?\s*no\.?)?)\s*[:\-]?\s*([A-Za-z0-9][A-Za-z0-9\/-]{2,40})\b/i,
			);
			if (m && m[1]) out = m[1];

			m = out.match(
				/\b(?:mci|nmc|rmc|dmc)\s*[:\-]?\s*([A-Za-z0-9\/-]{3,40})\b/i,
			);
			if (m && m[1]) out = m[1];
		...
		function isLikelyDoctorRegNo(value) {
			...
			if (/\b(reg|registration|mci|nmc|dmc|medical council)\b/i.test(t))
				return true;
		...
		function extractDoctorRegistrationFromNarrative(extractionPairs) {
			...
			const candidates = [];
			const patterns = [
				/\b(?:reg(?:istration)?\.?\s*no\.?|mci(?:\s*reg\.?\s*no\.?)?|nmc(?:\s*reg\.?\s*no\.?)?|rmc(?:\s*reg\.?\s*no\.?)?|dmc(?:\s*reg\.?\s*no\.?)?)\s*[:\-]?\s*([A-Za-z0-9][A-Za-z0-9\/-]{2,40})\b/gi,
				/\b(?:mci|nmc|rmc|dmc)\s*[:\-]?\s*([A-Za-z0-9\/-]{3,40})\b/gi,
		...
		function appendEntityPairsFromObject(entityObj, targetPairs, maxPairs) {
			if (
				!entityObj ||
				typeof entityObj !== "object" ||
				!Array.isArray(targetPairs)
			)
				return;
			...
			function pushPair(path, rawValue) {
				...
				const key = String(
					Array.isArray(path) && path.length ? path.join("_") : "value",
				).trim();
				const keyNorm = normalizeKey(key);
				if (!keyNorm) return;
				if (
					blockedKeyNormFragments.some(function (frag) {
						return keyNorm.indexOf(frag) >= 0;
					})
				)
					return;
		...
		function toIsoDateOrEmpty(value) {
			...
			const dmyMatch = firstLine.match(
		...
		function resolveReportDateForDisplay(
			primaryRawValue,
			extractionPairs,
			aliases,
		) {
			...
			const extractedIso = firstValidDateFromAliases(
				extractionPairs,
				aliases || [],
			);
			if (extractedIso) {
		...
		function isUnitFragment(value) {
			const t = String(value || "").trim();
			if (!t) return false;
			return /^(?:%|mg%|gm%|g\/dl|mg\/dl|mm\/hr|cells\/cu\s*mm|u\/l|iu\/l|mmol\/l|ng\/ml)$/i.test(
				t,
			);
		}
		...
		function isLikelyInvestigationTestName(value) {
			...
			if (isDateLikeFragment(t) || isNumericFragment(t) || isUnitFragment(t))
				return false;
		...
		function normalizeInvestigationRows(lines) {
			...
			function lineFingerprint(value) {
				...
				t = t.replace(
					/\bdate\s*:\s*\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4}\b/g,
					"",
				);
			...
			function isNoiseLine(value) {
				...
				if (
					/^(?:add\s*:|address\b|patient\s*name\b|hospital\b|consultant\s*name\b|admit(?:ting|ing)\s*dr\b|name\s+of\s+the\s+manager\b|signatures?\b)/i.test(
						t,
					)
				)
					return true;
				...
				if (
					/\b(?:details?\s+of\s+medication|follow\s*up\s+recommendation|follow\s*up(?:\s*to)?|review\s+after|in\s+case\s+of\s+emergency|call\s+us|contact\s*(?:us|no)?|helpline|ipd\s+medicine\s+bill|mobile(?:\s*number)?|phone(?:\s*number)?)\b/i.test(
						t,
					)
				)
					return true;
				if (/(?:\+?91[\s\-]*)?[6-9]\d{2}[\s\-]?\d{3}[\s\-]?\d{4}\b/.test(t))
					return true;
				return false;
			}

			function pushLine(text) {
				let cleaned = sanitizeReportText(text)
					.replace(/^[\-?]+\s*/, "")
					.trim();
				if (!cleaned) return;
			...
			source.forEach(function (raw) {
				let textLine = sanitizeReportText(raw)
					.replace(/^[\-?]+\s*/, "")
					.trim();
				if (!textLine) return;
			...
			return source
				.map(function (line) {
					return sanitizeReportText(line)
						.replace(/^[\-?]+\s*/, "")
						.replace(/^\s*test\s*:\s*/i, "")
						.trim();
				})
		...
		function extractVitalsFromLine(value) {
			...
			const tempRegex =
				/\b(?:temp(?:erature)?|t)\b\s*[:=\-]?\s*([0-9]{2,3}(?:\.[0-9]{1,2})?)\s*(?:ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â°?\s*[CF])?/gi;
			...
			const pulseRegex =
				/\b(?:pulse|pr|heart\s*rate|hr)\b\s*[:=\-]?\s*([0-9]{2,3})\b/gi;
			while ((m = pulseRegex.exec(text)) !== null) {
				const p = normalizeMeasurementValue(m[1], 20, 260);
				if (Number.isFinite(p)) out.pulses.push(p);
			}

			const labeledBpRegex =
				/\b(?:bp|b\/p|blood\s*pressure)\b\s*[:=\-]?\s*([0-9]{2,3})\s*[\/\-]\s*([0-9]{2,3})\b/gi;
			while ((m = labeledBpRegex.exec(text)) !== null) {
				...
				if (Number.isFinite(s) && Number.isFinite(d))
					out.bps.push({ s: s, d: d });
			}

			if (out.bps.length === 0) {
				const genericBpRegex = /\b([0-9]{2,3})\s*[\/\-]\s*([0-9]{2,3})\b/g;
				const isLikelyBpLine =
					/\b(?:mmhg|systolic|diastolic|tpr|vital|bp|b\/p|blood\s*pressure)\b/i.test(
						text,
		...
		function buildDailyTprSummary(extractionPairs, evidenceLines) {
			...
			(Array.isArray(evidenceLines) ? evidenceLines : [])
				.slice(0, 200)
			...
			allLines.forEach(function (line) {
				...
				const looksLikeVitals =
					/\b(?:temp|temperature|pulse|pr|heart\s*rate|hr|bp|b\/p|blood\s*pressure|tpr|mmhg)\b/i.test(
						line,
					) || /\b\d{2,3}\s*[\/\-]\s*\d{2,3}\b/.test(line);
				if (!looksLikeVitals) return;
				const vitals = extractVitalsFromLine(line);
				if (
					vitals.temps.length === 0 &&
					vitals.pulses.length === 0 &&
					vitals.bps.length === 0
				)
					return;
				...
				vitals.temps.forEach(function (n) {
					bucket.temps.push(n);
				});
				vitals.pulses.forEach(function (n) {
					bucket.pulses.push(n);
				});
			...
			if (dayMap.size === 0)
				return "No date-wise TPR (Temp/Pulse/BP) values extracted.";

			const rows = Array.from(dayMap.keys())
				.sort()
				.map(function (iso) {
					...
					const pulsePart = bucket.pulses.length
						...
						: "Pulse: -";
					const bpPart =
						bucket.bpSys.length && bucket.bpDia.length
							? "BP min/max: " +
								formatMinMax(bucket.bpSys, 0).split(" - ")[0] +
								"/" +
								formatMinMax(bucket.bpDia, 0).split(" - ")[0] +
								" to " +
								formatMinMax(bucket.bpSys, 0).split(" - ")[1] +
								"/" +
								formatMinMax(bucket.bpDia, 0).split(" - ")[1]
							: "BP: -";
					return (
						"- " +
						formatDdMmYyyy(iso) +
		...
		function buildMedicineEvidenceSummary(
			primaryMedicineText,
			extractionPairs,
		) {
			const allTexts = [];
			...
			function medicineFingerprint(value) {
				let t = String(value || "").toLowerCase();
				t = t.replace(
					/\b(?:inj|injection|tab|tablet|cap|capsule|syp|syrup)\b\.?/g,
					"",
				);
			...
			allTexts.forEach(function (block) {
				String(block || "")
					...
					.forEach(function (line) {
						const cleaned = sanitizeReportText(line)
							.replace(/^[\-ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢\d.()]+\s*/, "")
							.trim();
						if (!cleaned || cleaned === "-") return;
						if (!/[A-Za-z]/.test(cleaned)) return;
						if (
							/^(?:list\s+of\s+medicines|details?\s+of\s+medication|ipd\s+medicine\s+bill)/i.test(
								cleaned,
							)
						)
							return;
		...
		function buildCompactTreatmentSummary(value, maxItems) {
			const text = sanitizeReportText(value);
			if (!text || text === "-" || /no medicine list extracted/i.test(text))
				return "supportive in-patient treatment";
				...
				.forEach(function (chunk) {
					String(chunk || "")
						...
						.forEach(function (item) {
							const cleaned = sanitizeReportText(item)
								.replace(/^[-\d.()\s]+/, "")
								.trim();
							if (!cleaned || cleaned === "-") return;
							if (
			...
			return keep.length > short.length
				? summary + ", and other supportive medications"
				: summary;
		}
		...
		function parseGenderWord(value) {
			...
			if (text === "f" || /\b(?:female|woman|girl)\b/i.test(text))
				return "woman";
			return "";
		}
		...
		function isChecklistRuleSource(value) {
			const src = String(value || "")
				...
				.toLowerCase();
			return (
				src.indexOf("openai_claim_rules") === 0 ||
				src.indexOf("openai_diagnosis_criteria") === 0
			);
		}
		...
		function extractAntibioticNamesForConclusion(
			medicineText,
			highEndSignal,
			maxItems,
		) {
			...
			const antibioticPattern =
				/(meropenem|imipenem|ertapenem|doripenem|piperacillin|tazobactam|pip\s*-?\s*taz|ceftriaxone|cefotaxime|cefoperazone|sulbactam|cefepime|ceftazidime|amikacin|linezolid|colistin|teicoplanin|vancomycin|tigecycline|azithromycin|levofloxacin|ofloxacin|ciprofloxacin|amox(?:i|y)clav|amoxycillin|monocef)/i;
			const normalized = [];
			const seen = new Set();
			...
			String(combined)
				...
				.forEach(function (chunk) {
					const cleaned = sanitizeReportText(chunk)
						.replace(/^[-\d.()\s]+/, "")
						.replace(
							/\b(?:inj|injection|tab|tablet|cap|capsule|syp|syrup|iv|im|po|od|bd|tid|qid|hs|stat)\b\.?/gi,
							" ",
						)
			...
			if (!normalized.length) {
				if (/high\s*-?end\s*antibiotic|meropenem/i.test(combined))
		...
		function buildRuleStyleConclusion(decision, context) {
			...
			const complaints =
				summarizeForConclusion(info.complaints || "", 180) ||
				"unspecified complaints";
			const diagnosis =
				summarizeForConclusion(info.diagnosis || "", 130) ||
				"unspecified diagnosis";
			const treatments =
				summarizeForConclusion(info.treatments || "", 220) ||
				"standard in-patient treatment";
			const deranged =
				summarizeForConclusion(info.deranged || "", 220) ||
				"no significant deranged values documented";
			const reason =
				summarizeForConclusion(info.reason || "", 360) ||
				"clinical justification could not be derived from the available records";
			const decisionUpper = String(decision || "")
				.trim()
				.toUpperCase();
			const reasonLabel =
				decisionUpper === "ADMISSIBLE"
					? "approval"
					: decisionUpper === "QUERY"
						? "query"
						: "rejection";
			return sanitizeConclusionText(
				patientPhrase +
					...
					", diagnosis of " +
					diagnosis +
					", treated with following " +
					treatments +
					", and deranged investigation report of " +
					deranged +
					". " +
					"Reason for " +
					reasonLabel +
		...
		function hasFractureContextForConclusion(context) {
			...
			return /\b(?:fracture|orif|k[\s-]?wire|kwire|hairline|undisplaced|displaced|fixation)\b/i.test(
				corpus,
			);
		}
		...
		function hasProcedureContextForConclusion(context) {
			...
			const corpus = [
				...
				.join(" ");
			if (!corpus) return false;
			return /\b(?:surgery|surgical|procedure|operation|operative|post\s*op|postoperative|ablation|ligation|stripping|stent|angioplasty|lscs|orif|fixation)\b/i.test(
				corpus,
			);
		}
		...
		function isConclusionObservationRelevant(text, context) {
			...
			const fractureLike =
				/\b(?:fracture|orif|k[\s-]?wire|kwire|hairline|undisplaced|displaced|fixation)\b/i.test(
					line,
				);
			if (fractureLike && !hasFractureContextForConclusion(context))
				return false;
			const orphanSurgicalGap =
				/^surgical\s+indication\.?$/i.test(line) ||
				/^procedure\s+note\/?bill\.?$/i.test(line);
			const normalized = line
				.toLowerCase()
				.replace(/[^a-z0-9\s]/g, " ")
				.replace(/\s+/g, " ")
				.trim();
			const looseSurgicalGap =
				/\b(?:surgical\s+indication|fracture\s+description|procedure\s+note(?:\s*bill)?|orif|k\s*wire|kwire)\b/i.test(
					normalized,
				);
			if (
				(orphanSurgicalGap || looseSurgicalGap) &&
				!hasProcedureContextForConclusion(context) &&
				!hasFractureContextForConclusion(context)
			)
				return false;
			if (
				normalized.split(" ").filter(Boolean).length <= 2 &&
				/\b(?:indication|description)\b/i.test(normalized)
			)
				return false;
			return true;
		}
		...
		function filterTriggeredReasonTextForContext(reasonText, context) {
			const chunks = String(reasonText || "")
				...
				.map(function (x) {
					return sanitizeConclusionText(x)
						.replace(
							/^(?:key\s*gaps?|pending\s*clarifications?|checklist\s*observations?|missing\s*evidence)\s*:\s*/i,
							"",
						)
						.replace(/[\s.;:,-]+$/, "")
						.trim();
				...
				.filter(Boolean)
				.filter(function (x) {
					return isConclusionObservationRelevant(x, context);
				});
			return chunks.join("; ");
		}
		...
		function buildTriggeredRuleSummary(checklistRows, context) {
			...
			rows.forEach(function (entry) {
				...
				const name = sanitizeConclusionText(entry.name || entry.title || "")
					.replace(/\bR\d{3}\b\s*[-: ]*/gi, "")
					.replace(/\s+/g, " ")
					.trim();
				const decisionRaw = String(entry.decision || entry.status || "")
					.trim()
					.toUpperCase();
				const decisionLabel =
					decisionRaw === "APPROVE" ||
				...
				if (!base) return;
				const detail = (
					base + (decisionLabel ? " [" + decisionLabel + "]" : "")
				).trim();
		...
		function getRuleConclusionLineByCode(code, context) {
			...
			const antibioticNames = summarizeForConclusion(
				extractAntibioticNamesForConclusion(
					info.medicineEvidence || "",
					info.highEndSignal || "",
					3,
				),
			...
			const MAP = {
				R001:
					"Use of " +
					abxLabel +
					" is not supported by sepsis markers or objective evidence of severe infection; therefore, this rule is triggered.",
				...
				R003:
					"Pneumonia is not adequately established in view of negative/non-supportive imaging, absence of culture evidence, and unjustified use of " +
					abxLabel +
					".",
				...
				R006:
					abxLabel +
					" administration is not supported by abnormal vitals, laboratory markers, or other objective evidence of serious infection.",
				...
				R013:
					"UTI treatment with " +
					abxLabel +
					" is supported by culture and sensitivity evidence; therefore, the treatment is considered justified under this rule.",
		...
		function getDecisionOutcomeForConclusion(finalDecision) {
			const d = String(finalDecision || "")
				.trim()
				.toUpperCase();
			if (d === "ADMISSIBLE")
				return {
					verdict: "APPROVED",
					terminalSentence: "Therefore, the claim is admissible.",
				};
			if (d === "INADMISSIBLE")
				return {
					verdict: "REJECTED",
			...
			return {
				verdict: "QUERIED",
				terminalSentence: "Therefore, the claim is kept under query.",
			};
		}

		function shouldSuppressRuleDrivenConclusion(context) {
			const claimKey = String(
				(context && (context.externalClaimId || context.claimId || "")) || "",
			).trim();
		...
		function buildRuleDrivenConclusion(checklistRows, context) {
			...
			const diagnosis =
				...
				"unspecified diagnosis";
			const treatments =
				summarizeForConclusion(info.treatments || "", 180) ||
				"supportive in-patient treatment";
			const deranged =
				summarizeForConclusion(info.deranged || "", 180) ||
				"no significant deranged values documented";
			const reason =
				summarizeForConclusion(info.reason || "", 320) ||
				"clinical justification could not be derived from the available records";
			const decisionUpper = String(info.finalDecision || "")
				.trim()
				.toUpperCase();
			const reasonLabel =
				decisionUpper === "ADMISSIBLE"
			...
			rows.forEach(function (entry) {
				...
				const code = extractRuleCodeFromEntry(entry);
				if (/^R\d{3}$/.test(code) && !isRuleCodeRelevantForContext(code, info))
					return;
				if (/^R\d{3}$/.test(code)) {
					if (seenCodes.has(code)) return;
					seenCodes.add(code);
					const mappedLine = sanitizeConclusionText(
						getRuleConclusionLineByCode(code, info),
					)
				...
				const fallbackLine = sanitizeConclusionText(
					entry.note ||
						entry.why_triggered ||
						entry.summary ||
						entry.reason ||
					...
					.replace(/\bR\d{3}\b\s*[-:]\s*/gi, "")
					.trim();
				if (fallbackLine && isConclusionObservationRelevant(fallbackLine, info))
					ruleLinesOnly.push(fallbackLine);
			});

			if (ruleLinesOnly.length === 0) return introLine;

			const outcome = getDecisionOutcomeForConclusion(info.finalDecision);
			const rawGaps = filterTriggeredReasonTextForContext(
				info.triggeredReasonText || "",
				info,
			)
				.split(";")
				.map(function (x) {
			...
			const gapText =
				gapItems.length >= 3
					...
					: gapItems.length === 2
						? gapItems[0] + " and " + gapItems[1]
						: gapItems[0] || "key clinical documentation gaps";

			const finalParagraph = sanitizeConclusionText(
				"Conclusion: The claim has been reviewed against applicable clinical rules. " +
					"On examination of the available documents, it is noted that " +
					clinicalSummary +
					". " +
					"However, " +
					gapText +
		...
		function buildExactReasonText(decision, context) {
			const info = context || {};
			const diagnosis =
				summarizeForConclusion(info.diagnosis || "", 120) ||
				"the diagnosed condition";
			const deranged = summarizeForConclusion(info.deranged || "", 140);
			const filteredTriggerText = filterTriggeredReasonTextForContext(
				info.triggeredReasonText || "",
				info,
			);
			const triggerText = summarizeForConclusion(
				String(filteredTriggerText || "").replace(/\bR\d{3}\b\s*[-:]\s*/gi, ""),
				220,
			);
			const triggerTextClean = String(triggerText || "").replace(
				/[\s.;:,-]+$/,
				"",
			);
			const triggerTextFinal = isConclusionObservationRelevant(
				triggerTextClean,
				info,
			)
				? triggerTextClean
				: "";
			const triggeredRuleSummary = summarizeForConclusion(
				String(info.triggeredRuleSummary || "").replace(
					/\bR\d{3}\b\s*[-:]\s*/gi,
					"",
				),
				220,
			);
			const compactTreatment = buildCompactTreatmentSummary(
				info.medicineEvidence || "",
				4,
			);
			const medEvidence = summarizeForConclusion(compactTreatment, 140);
			const tprEvidence = summarizeForConclusion(
				String(info.tprSummary || "")
					.replace(/^-+\s*/gm, "")
					.replace(/\n+/g, "; "),
				180,
			);
			...
			if (
				deranged &&
				!/no deranged investigation values found/i.test(deranged)
			) {
				supportBits.push(
					"deranged investigations (" +
						summarizeForConclusion(deranged, 90) +
						")",
				);
			}
			if (
				medEvidence &&
				!/supportive in-patient treatment/i.test(medEvidence)
			) {
			...
			if (decision === "INADMISSIBLE") {
				reasons.push(
					"Admission during " +
						...
						") for " +
						diagnosis +
						" is not sufficiently justified on submitted records.",
				);
				if (triggerTextFinal) {
					reasons.push("Key gaps: " + triggerTextFinal + ".");
				} else if (triggeredRuleSummary) {
					reasons.push(
						"Triggered clinical checks indicate documentation gaps.",
					);
				}
				...
				reasons.push(
					"Admission during " +
						...
						" (LOS: " +
						losText +
						") is medically justified for " +
						diagnosis +
						" based on submitted records.",
				);
				if (supportText)
					reasons.push("Supportive evidence includes " + supportText + ".");
			} else {
				reasons.push(
					"Available records for " +
						...
						" during " +
						windowText +
						" are insufficient for final admissibility decision.",
				);
				if (triggerTextFinal) {
					reasons.push("Pending clarifications: " + triggerTextFinal + ".");
				} else if (triggeredRuleSummary) {
					reasons.push(
						"Triggered clinical checks require additional documentation.",
					);
				}
			}

			return (
				sanitizeConclusionText(reasons.join(" ")) ||
				"Reason could not be generated from extracted evidence."
			);
		}
		function mapFinalRecommendationDecision(
			rawRecommendation,
			admissionRequired,
		) {
			const rec = String(rawRecommendation || "")
				.trim()
				.toLowerCase();
			const admission = String(admissionRequired || "")
				.trim()
				.toLowerCase();
			...
			if (admission === "yes" || admission === "true" || admission === "1")
				return "ADMISSIBLE";
			if (admission === "no" || admission === "false" || admission === "0")
				return "INADMISSIBLE";
			return "QUERY";
		}
		function extractTextByAliases(extractionPairs, aliases, limit) {
			...
			extractionPairs.forEach(function (pair) {
				...
				const matched = aliasList.some(function (alias) {
					return (
						keyNorm === alias ||
						keyNorm.indexOf(alias) >= 0 ||
						alias.indexOf(keyNorm) >= 0
					);
				});
				if (!matched) return;
				const textValue = sanitizeReportText(
		...
		function isLikelyDateTimeOnly(value) {
			...
			if (
				...
				return true;
			if (
				/^\d{4}[.\-\/]\d{1,2}[.\-\/]\d{1,2}(?:\s+\d{1,2}:\d{2}(?::\d{2})?)?$/.test(
					t,
				)
			)
				return true;
		...
		function extractDiagnosisFromNarrativeBlocks(values) {
			const list = Array.isArray(values) ? values : [];
			for (let i = 0; i < list.length; i += 1) {
				const lines = sanitizeReportText(list[i])
					.split("\n")
					.map(function (x) {
				...
				for (let j = 0; j < lines.length; j += 1) {
					...
					for (let k = j + 1; k < Math.min(lines.length, j + 5); k += 1) {
						...
						if (!cand) continue;
						if (
							/^case\s+history|reason\s+for\s+admission|treatment|operative/i.test(
								cand,
							)
						)
							continue;
		...
		function trimClinicalTextAtStopLabels(value) {
			...
			const stopMatch = text.match(
				/([\s\S]*?)(?=\b(?:major\s+diagnostic\s+finding|clinical\s+findings?|provisional\s+diagnostics?|final\s+diagnosis|patient\s+name|hospital|admit(?:ting|ing)\s*dr|consultant\s+name|reg(?:istration)?\.?\s*no|total|final\s+payment|surgery|procedure\s+performed|details\s+of\s+medication|ipd\s+medicine\s+bill|medicine\s+name|claimed|name\s+of\s+the\s+establishment|full\s+postal\s+address|name\s+of\s+the\s+manager)\b|$)/i,
			);
			return sanitizeReportText(
				stopMatch && stopMatch[1] ? stopMatch[1] : text,
			);
		}
		...
		function extractChiefComplaintsFromNarrative(value) {
			...
			for (let i = 0; i < patterns.length; i += 1) {
				...
				const cleaned = trimClinicalTextAtStopLabels(match[1])
					.replace(/^[\s,;:.\-]+/, "")
					.trim();
		...
		function cleanMajorDiagnosticFindingText(value, chiefComplaintsText) {
			...
			if (chiefText && chiefText !== "-") {
				...
				if (escapedChief)
					text = text.replace(new RegExp(escapedChief, "ig"), " ");
			}
			text = text.replace(
				/chief\s*complaints?(?:\s*at\s*admission)?\s*[:\-]?/gi,
				"",
			);

			text = text.replace(
				/\b(?:patient\s+name|name\s+of\s+the\s+establishment|full\s+postal\s+address|add\s*:|admit(?:ting|ing)\s*dr|consultant(?:\s+name)?|reg(?:istration)?\.?\s*no|ipd\s+medicine\s+bill|medicine\s+name|received\s+with\s+thanks|total\s*(?:sum)?|final\s+payment|claimed|date\s+of\s+admission|date\s+of\s+discharge)\b[\s\S]*$/i,
				"",
			);

			const lineFiltered = String(text || "")
				.split(/\r?\n/)
				.map(function (line) {
				...
				.filter(function (line) {
					if (!line) return false;
					if (
						/^(patient\s+name|hospital\b|name\s+of\s+the\s+establishment|full\s+postal\s+address|add\s*:|admit(?:ting|ing)\s*dr|consultant(?:\s+name)?|reg(?:istration)?\.?\s*no|ipd\s+medicine\s+bill|medicine\s+name|received\s+with\s+thanks|total\b|final\s+payment|claimed\b|date\s+of\s+admission|date\s+of\s+discharge)/i.test(
							line,
						)
					)
						return false;
					if (/^name\s*[:\-]?\s*[a-z]/i.test(line)) return false;
					if (/^(?:dr\.?\s+|doctor\s+|consultant\s+dr\.?\s*)/i.test(line))
						return false;
					if (
						/\b(?:mbbs|md|ms|dm|mch|bams|bhms|bums|dnb|fnb|rmc\s*\d+)\b/i.test(
							line,
						)
					)
						return false;
					if (
						/\b(?:follow\s*up|in\s+case\s+of\s+emergency|call\s+us|mobile\s*no|phone\s*no)\b/i.test(
							line,
						)
					)
						return false;
					const addressTokenHits = (
						line.match(
							/\b(?:road|rd|street|st|lane|ln|nagar|colony|city|district|state|pin|pincode|zip|plot|floor|building|plaza|near|opp|opposite|shop|apartment|apt|flat|unit|society|chsl|complex|tower|wing|sector|block|east|west|north|south|taluka|tehsil|vihar|chowk)\b/gi,
						) || []
					).length;
					const commaCount = (line.match(/,/g) || []).length;
					const clinicalTokenHits = (
						line.match(
							/\b(?:pain|swelling|fever|cough|breathlessness|vomiting|diarrhea|fracture|sepsis|infection|uti|pneumonia|tachycardia|hypotension|hypertension|bp|pulse|spo2|temperature|rbc|wbc|platelet|creatinine|sodium|potassium|diagnosis|edema|tenderness|discharge)\b/gi,
						) || []
					).length;
					if (
						addressTokenHits >= 1 &&
						clinicalTokenHits === 0 &&
						/\b(?:road|rd|street|st|lane|ln|nagar|vihar|chowk|colony|sector|block|city|district|state|near|opp|opposite)\b/i.test(
							line,
						)
					)
						return false;
					if (
						/\bhospital\b/i.test(line) &&
						(addressTokenHits >= 2 || commaCount >= 2)
					)
						return false;
					if (
						addressTokenHits >= 3 &&
						(commaCount >= 1 || /\b\d{6}\b/.test(line))
					)
						return false;
					return true;
				});
		...
		function buildLegacyReportHtml(
			...
			function legacyValue() {
				for (let i = 0; i < arguments.length; i += 1) {
					...
					if (!keyNorm) continue;
					if (!Object.prototype.hasOwnProperty.call(legacyPayload, keyNorm))
						continue;
			...
			const benefGender = firstNonEmpty(
				extractTextByAliases(
					extractionPairs,
					["benef_gender", "patient_gender", "gender", "sex"],
					1,
				)[0],
			...
			const insuredCombined = (function () {
				...
				const genderPart =
					gender && gender !== "-" ? "/" + gender.charAt(0).toUpperCase() : "";
			...
			let hospital = firstNonEmpty(
				...
				currentClaim &&
					Array.isArray(currentClaim.tags) &&
					currentClaim.tags.length > 3
					? currentClaim.tags[3]
					: "",
				"-",
			);
			const narrativeHospital = extractHospitalNameFromNarrative(
				extractionPairs,
				insured,
			);
			if (
				narrativeHospital &&
				(hospital === "-" ||
					isSameEntityText(hospital, insured) ||
					!isLikelyOrgName(hospital))
			) {
			...
			const treatingDoctorFromExtraction = normalizeDoctorNameCandidate(
				extractTreatingDoctorName(extractionPairs),
			);
			let treatingDoctor =
				normalizeDoctorNameCandidate(
					firstNonEmpty(
						treatingDoctorFromExtraction && treatingDoctorFromExtraction !== "-"
							? treatingDoctorFromExtraction
							: "",
						extractTextByAliases(
							extractionPairs,
							[
			...
			const narrativeDoctor = normalizeDoctorNameCandidate(
				extractDoctorNameFromNarrative(extractionPairs, insured, hospital),
			);
			if (
				narrativeDoctor &&
				(treatingDoctor === "-" ||
			...
			let treatingDoctorRegistration = normalizeDoctorRegNoCandidate(
				firstNonEmpty(
					extractTextByAliases(
						extractionPairs,
						[
			...
			const narrativeDoctorRegistration = normalizeDoctorRegNoCandidate(
				extractDoctorRegistrationFromNarrative(extractionPairs),
			);
			if (
				(!isLikelyDoctorRegNo(treatingDoctorRegistration) ||
			...
			const medicineEvidenceText = buildMedicineEvidenceSummary(
				treatmentMedicines,
				extractionPairs,
			);
			const admissionDateImported = firstNonEmpty(
				...
				extractTextByAliases(
					extractionPairs,
					["admission_date", "date_of_admission", "doa", "doa_date"],
					1,
				)[0],
				"-",
			);
			const dischargeDateImported = firstNonEmpty(
				...
				extractTextByAliases(
					extractionPairs,
					["discharge_date", "date_of_discharge", "dod", "dod_date"],
					1,
				)[0],
				"-",
			);
			...
			const diagnosisFromNarrative = extractDiagnosisFromNarrativeBlocks(
				extractTextByAliases(
					extractionPairs,
					["major_diagnostic_finding", "clinical_findings", "summary"],
					8,
				),
			);
			...
			const chiefComplaintsDirect = sanitizeReportText(
				extractTextByAliases(
					extractionPairs,
					[
						"chief_complaints",
			...
			const chiefComplaintsFromNarrative =
				extractChiefComplaintsFromNarrative(majorFindingRawText);
			const chiefComplaints = firstNonEmpty(
				chiefComplaintsDirect,
				chiefComplaintsFromNarrative,
				"-",
			);
			...
			const alcoholHistory = firstNonEmpty(
				extractTextByAliases(
					extractionPairs,
					["alcoholism_history", "alcohol_history", "alcohol"],
					1,
				)[0],
				"-",
			);
			const claimAmountFromExtraction = pickFirstValidValue(
				extractTextByAliases(
					extractionPairs,
					["claimed_amount", "claim_amount", "amount_claimed", "bill_amount"],
					8,
				),
				function (value) {
					const t = String(value || "").trim();
			...
			const allInvestigationRows = normalizeInvestigationRows(
				extractTextByAliases(
					extractionPairs,
					[
						"all_investigation_reports_with_values",
			...
			const derangedRows = allInvestigationRows.filter(function (line) {
				return /\b(high|low|elevated|decreased|abnormal|deranged)\b/i.test(
					line,
				);
			});

			const filteredEvidenceLines = (evidenceLines || []).filter(
				function (line) {
					const t = String(line || "").trim();
					if (!t) return false;
					if (
			...
			const clinicalFindingCandidates = extractTextByAliases(
				extractionPairs,
				[
					"clinical_findings",
					"summary",
					...
					String(block || "")
						...
						.forEach(function (line) {
							const t = sanitizeReportText(line)
								.replace(/^[\-ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢]+\s*/, "")
								.trim();
							if (!t) return;
							if (
								/^(?:patient\s*name|hospital\b|add\s*:|address\b|doa\b|dod\b|doa[_\s-]*date\b|dod[_\s-]*date\b|date\s*:|time\s*:|total\b|claimed\b|name\s+of\s+the\s+manager\b|signatures?\b|admission\s*date\b|date\s+of\s+admission\b|discharge\s*date\b|date\s+of\s+discharge\b|consultant\s+name\b|admit(?:ting|ing)\s*dr\b)/i.test(
									t,
								)
							)
								return;
							if (
								/\b(?:treating\s*doctor|treating_doctor|doctor\s*reg(?:istration)?(?:\s*no|\s*number)?|treating_doctor_registration_number|date\s*of\s*admission|admission\s*date|date\s*of\s*discharge|discharge\s*date)\b/i.test(
									t,
								)
							)
								return;
							if (
								/\b(?:details?\s+of\s+medication|follow\s*up\s+recommendation|follow\s*up(?:\s*to)?|review\s+after|in\s+case\s+of\s+emergency|call\s+us|contact\s*(?:us|no)?|helpline|ipd\s+medicine\s+bill|medicine\s+name|mobile(?:\s*number)?|phone(?:\s*number)?)\b/i.test(
									t,
								)
							)
								return;
							if (
								/(?:\+?91[\s\-]*)?[6-9]\d{2}[\s\-]?\d{3}[\s\-]?\d{4}\b/.test(t)
							)
								return;
							if (
								/\b(?:hospital\s*address|full\s*postal\s*address|address\s*of\s*hospital|hospital\s*addr(?:ess)?)\b/i.test(
									t,
								)
							)
								return;
							...
							const addressTokenHits = (
								t.match(
									/\b(?:road|rd|street|st|lane|ln|nagar|colony|city|district|state|pin|pincode|zip|plot|floor|building|plaza|near|opp|opposite|shop|apartment|apt|flat|unit|society|chsl|complex|tower|wing|sector|block|east|west|north|south|taluka|tehsil|vihar|chowk|village|gaon)\b/gi,
								) || []
							).length;
							const clinicalTokenHits = (
								t.match(
							...
							if (
								...
								return;
							if (
								/\b(?:shop|apartment|apt|flat|unit)\s*no\.?\s*\d+\b/i.test(t) &&
								/\b\d{6}\b/.test(t)
							)
								return;
							if (
							...
							if (
								addressTokenHits >= 2 &&
								(/\d{3,}/.test(t) || /\b(?:pin|pincode|zip)\b/i.test(t))
							)
								return;
			...
			const tprDailySummaryText = buildDailyTprSummary(
				extractionPairs,
				evidenceLines,
			);
			...
			const finalRecommendationDecision = mapFinalRecommendationDecision(
				rawDecisionSignal,
				admissionRequired,
			);
			const admissionDecisionLabel =
				finalRecommendationDecision === "ADMISSIBLE"
					? "Justified"
					: finalRecommendationDecision === "INADMISSIBLE"
						? "Not Justified"
						: "Query";

			const checklistRows = Array.isArray(
				currentChecklistLatest && currentChecklistLatest.checklist,
			)
				? currentChecklistLatest.checklist
				: [];
			const complaintsForConclusion = summarizeForConclusion(
				chiefComplaints,
				180,
			);
			const findingsForConclusion = summarizeForConclusion(
				majorFinding !== "-" ? majorFinding : clinicalFindingsText,
				260,
			);
			const derangedForConclusion = summarizeForConclusion(derangedText, 220);
			const diagnosisForConclusion =
				summarizeForConclusion(diagnosis, 120) || "the reported diagnosis";
			const treatmentForConclusion = buildCompactTreatmentSummary(
				medicineEvidenceText,
				4,
			);
			const tprForConclusion = summarizeForConclusion(tprDailySummaryText, 240);
			const admissionWindowText =
				String(admissionDateResolved.display || "-") +
				" to " +
				String(dischargeDateResolved.display || "-");
			const highEndSignal = summarizeForConclusion(
				extractTextByAliases(
					extractionPairs,
			...
			const checklistReporting =
				currentChecklistLatest &&
				currentChecklistLatest.source_summary &&
				currentChecklistLatest.source_summary.reporting &&
				typeof currentChecklistLatest.source_summary.reporting === "object"
					? currentChecklistLatest.source_summary.reporting
					: {};
			const checklistConclusionText = sanitizeConclusionText(
				checklistReporting.conclusion || "",
			);
			const structuredConclusionText = sanitizeConclusionText(
				extractTextByAliases(
					extractionPairs,
					["detailed_conclusion", "conclusion", "decision"],
					2,
				).join("\n"),
			);
			const checklistRecommendationTextRaw = sanitizeConclusionText(
				checklistReporting.recommendation_text || "",
			);
			const checklistRecommendationText =
				/\b(recommended\s+for\s+rejection|reject(?:ion|ed)?|inadmissible)\b/i.test(
					...
					: checklistRecommendationTextRaw;
			const structuredRecommendationRaw = sanitizeConclusionText(
				extractTextByAliases(
					extractionPairs,
			...
			const structuredRecommendationText =
				/\b(recommended\s+for\s+rejection|reject(?:ion|ed)?|inadmissible)\b/i.test(
			...
			const triggeredConditionNotesRaw = checklistRows
				.filter(function (entry) {
					...
					if (
						/^R\d{3}$/.test(code) &&
						!isRuleCodeRelevantForContext(code, conclusionRuleContext)
					)
						return false;
					return true;
				})
				.reduce(function (out, entry) {
					const note = sanitizeConclusionText(
						entry.note ||
							entry.why_triggered ||
							entry.summary ||
							entry.reason ||
						...
						.replace(/\bOPENAI_MERGED_REVIEW\b/gi, "")
						.replace(/\b[Rr]\d{3}\b\s*[-:]\s*/g, "")
						.replace(/\bDX\d{3}\b\s*[-:]\s*/g, "")
						.replace(/\bMissing evidence\s*:\s*/gi, "");
					const cleanedNote = sanitizeConclusionText(note);
					if (
						cleanedNote &&
						!/^(openai_claim_rules|openai_diagnosis_criteria)\b/i.test(
							cleanedNote,
						) &&
						!/openai_merged_review|merged\s+document\s+ai\s+medical\s+audit/i.test(
							cleanedNote,
						) &&
						!/^(reject|query|approve|admissible|inadmissible)$/i.test(
							cleanedNote,
						)
					) {
					...
					const missingList = Array.isArray(entry.missing_evidence)
						? entry.missing_evidence
						: [];
					missingList.forEach(function (missing) {
						const cleanedMissing = sanitizeConclusionText(
							String(missing || ""),
						);
			...
			const triggeredConditionNotes = triggeredConditionNotesRaw
			...
			const lengthOfStay = computeLengthOfStay(
				admissionDateResolved.iso,
				dischargeDateResolved.iso,
			);
			triggeredReasonText = filterTriggeredReasonTextForContext(
				triggeredReasonText,
				conclusionRuleContext,
			);
			triggeredRuleSummaryText = buildTriggeredRuleSummary(
				checklistRows,
				conclusionRuleContext,
			);
			const rejectDetailedFallback = sanitizeConclusionText(
				"The patient appears to have been managed for " +
					diagnosisForConclusion +
					" during " +
					admissionWindowText +
					" (LOS: " +
					lengthOfStay +
					...
					(complaintsForConclusion
						? "Chief complaints: " +
							summarizeForConclusion(complaintsForConclusion, 120) +
							". "
						: "") +
					(findingsForConclusion
						? "Clinical findings: " +
							summarizeForConclusion(findingsForConclusion, 150) +
							". "
						: "") +
					(derangedForConclusion &&
					!/no deranged investigation values found/i.test(derangedForConclusion)
						? "Deranged investigations: " +
							summarizeForConclusion(derangedForConclusion, 120) +
							". "
						: "") +
					(highEndSignal
						? "Antibiotic clarification: " +
							summarizeForConclusion(highEndSignal, 140) +
							". "
						: "") +
					(triggeredReasonText
						? "Required clarifications: " +
							summarizeForConclusion(triggeredReasonText, 160) +
							". "
						: "") +
			...
			const queryDetailedFallback = sanitizeConclusionText(
				"Current records are insufficient for a final admissibility decision. " +
					...
					(findingsForConclusion
						? "Clinical findings: " +
							summarizeForConclusion(findingsForConclusion, 140) +
							". "
						: "") +
					(highEndSignal
						? "Antibiotic clarification required: " +
							summarizeForConclusion(highEndSignal, 140) +
							". "
						: "") +
					(triggeredReasonText
			...
			if (isWeakConclusionText(conclusionText)) {
				conclusionText = firstNonEmpty(
					...
					structuredConclusionText,
					finalRecommendationDecision === "INADMISSIBLE"
						? rejectDetailedFallback
						: "",
					finalRecommendationDecision === "QUERY" ? queryDetailedFallback : "",
					finalRecommendationDecision !== "ADMISSIBLE"
						? queryDetailedFallback
						: "",
					sanitizeConclusionText(
						stripHtmlTags(currentStatusItem && currentStatusItem.final_status),
					),
			...
			if (checklistConclusionText && structuredConclusionText) {
				conclusionText = mergeRuleWithLearning(
					conclusionText,
					structuredConclusionText,
					"",
					220,
			...
			let recommendationText = firstNonEmpty(
				checklistRecommendationText,
				structuredRecommendationText,
				"",
			);
			...
			const exactReasonText = buildExactReasonText(
			...
			const ruleStyleConclusionText = buildRuleStyleConclusion(
				finalRecommendationDecision,
				{
					age: benefAge,
					gender: benefGender,
			...
			const ruleDrivenConclusionText = buildRuleDrivenConclusion(
				checklistRows,
				{
					age: benefAge,
					gender: benefGender,
			...
			const preferredConclusionText = firstNonEmpty(
				ruleDrivenConclusionText,
				ruleStyleConclusionText,
				"",
			);
			if (preferredConclusionText) {
				conclusionText =
					sanitizeConclusionText(preferredConclusionText) ||
					preferredConclusionText;
			}

			const auditorLearningText = summarizeForConclusion(
				String(
					currentStatusItem && currentStatusItem.auditor_learning
						? currentStatusItem.auditor_learning
						: "",
				).replace(/\bR\d{3}\b\s*[-:]\s*/gi, ""),
				260,
			);
			...
			return (
				'<h1 class="title">HEALTH CLAIM INVESTIGATION REPORT</h1>' +
				'<div class="meta">Generated: ' +
				escapeHtml(generatedMeta) +
				" | Doctor: " +
				escapeHtml(doctorMeta) +
				"</div>" +
				'<table class="t"><tbody>' +
				"<tr><th>COMPANY NAME</th><td>" +
				escapeHtml(companyName) +
				"</td></tr>" +
				"<tr><th>CLAIM NO.</th><td>" +
				escapeHtml(claimId) +
				"</td></tr>" +
				"<tr><th>CLAIM TYPE</th><td>" +
				escapeHtml(claimType) +
				"</td></tr>" +
				"<tr><th>INSURED</th><td>" +
				escapeHtml(insuredCombined) +
				"</td></tr>" +
				"<tr><th>HOSPITAL</th><td>" +
				escapeHtml(hospital) +
				"</td></tr>" +
				"<tr><th>TREATING DOCTOR</th><td>" +
				escapeHtml(treatingDoctorDisplay) +
				"</td></tr>" +
				"<tr><th>TREATING DOCTOR REGISTRATION NUMBER</th><td>" +
				escapeHtml(treatingDoctorRegistration) +
				"</td></tr>" +
				"<tr><th>ADMISSION</th><td>" +
				escapeHtml(admissionDateResolved.display) +
				"</td></tr>" +
				"<tr><th>DISCHARGE</th><td>" +
				escapeHtml(dischargeDateResolved.display) +
				"</td></tr>" +
				"<tr><th>LENGTH OF STAY</th><td>" +
				escapeHtml(lengthOfStay) +
				"</td></tr>" +
				"<tr><th>DIAGNOSIS</th><td>" +
				escapeHtml(diagnosis) +
				"</td></tr>" +
				"<tr><th>CHIEF COMPLAINTS AT ADMISSION</th><td>" +
				nl2brEsc(chiefComplaints) +
				"</td></tr>" +
				"<tr><th>MAJOR DIAGNOSTIC FINDING (ADMISSION / DURING STAY)</th><td>" +
				nl2brEsc(majorFinding) +
				"</td></tr>" +
				"<tr><th>ALCOHOLISM HISTORY</th><td>" +
				escapeHtml(alcoholHistory) +
				"</td></tr>" +
				"<tr><th>CLAIMED AMOUNT</th><td>" +
				escapeHtml(claimedAmount) +
				"</td></tr>" +
				"</tbody></table>" +
				'<div class="sec">CLINICAL FINDINGS</div>' +
				'<table class="t"><tbody><tr><td>' +
				nl2brEsc(clinicalFindingsText) +
				"</td></tr></tbody></table>" +
				'<div class="sec">ALL INVESTIGATION REPORTS</div>' +
				'<table class="t"><tbody><tr><td>' +
				nl2brEsc(allInvestigationText) +
				"</td></tr></tbody></table>" +
				'<div class="sec">DATE-WISE INVESTIGATION REPORTS</div>' +
				'<table class="t"><tbody><tr><td>' +
				nl2brEsc(dateWiseText) +
				"</td></tr></tbody></table>" +
				'<div class="sec">DERANGED INVESTIGATION REPORTS</div>' +
				'<table class="t"><tbody><tr><td>' +
				nl2brEsc(derangedText) +
				"</td></tr></tbody></table>" +
				'<div class="sec">DAILY TPR CHART (MIN/MAX)</div>' +
				'<table class="t"><tbody><tr><td>' +
				nl2brEsc(tprDailySummaryText) +
				"</td></tr></tbody></table>" +
				'<div class="sec">MEDICINE EVIDENCE USED</div>' +
				'<table class="t"><tbody><tr><td>' +
				nl2brEsc(medicineEvidenceText) +
				"</td></tr></tbody></table>" +
				'<div class="sec">CONCLUSION AND RECOMMENDATION</div>' +
				'<table class="t"><tbody>' +
				"<tr><th>Admission Required</th><td>" +
				escapeHtml(admissionDecisionLabel) +
				"</td></tr>" +
				"<tr><th>Final Recommendation</th><td>" +
				escapeHtml(finalRecommendationDecision) +
				"</td></tr>" +
				"<tr><th>Conclusion</th><td>" +
				nl2brEsc(conclusionText) +
				"</td></tr>" +
				"<tr><th>Recommendation</th><td>" +
				nl2brEsc(recommendationText) +
				"</td></tr>" +
				"</tbody></table>"
			);
		}
		...
		function buildReportPairsFromStructuredData(data) {
			...
			if (!data || typeof data !== "object") {
				return {
					extractionPairs: extractionPairs,
			...
			addPair(
				"hospital_name",
				cleanHospitalDisplayName(data.hospital_name, ""),
			);
			addPair("treating_doctor", data.treating_doctor);
			addPair(
				"treating_doctor_registration_number",
				data.treating_doctor_registration_number,
			);
			...
			addPair(
				"high_end_antibiotic_for_rejection",
				data.high_end_antibiotic_for_rejection,
			);

			const invText = String(
				data.investigation_finding_in_details || "",
			).trim();
			if (invText && invText !== "-") {
					...
					function (line) {
						...
						if (t)
							extractionPairs.push({
								key: "all_investigation_reports_with_values",
								value: t,
							});
					},
				);
			...
			let finalRec = "QUERY";
			if (
				/rejection|reject|inadmissible|not\s+justified/.test(recommendationText)
			)
				finalRec = "INADMISSIBLE";
			else if (/payable|admissible|approve/.test(recommendationText))
				finalRec = "ADMISSIBLE";
			...
			const abxSignal = sanitizeReportText(
				data.high_end_antibiotic_for_rejection || "",
			);
			if (abxSignal && abxSignal !== "-")
				evidenceLines.push("High-end antibiotic check: " + abxSignal);
		...
		async function buildLegacyReportHtmlFromLatestData() {
			...
			const claimUuid = String(
				(currentClaim && currentClaim.id) || routeClaimUuid || "",
			).trim();
			if (claimUuid) {
				try {
					...
					const structured = await apiFetch(
							...
							"/structured-data",
						{
							method: "POST",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify({ use_llm: false, force_refresh: true }),
						},
					);
					if (structured && typeof structured === "object") {
						const mapped = buildReportPairsFromStructuredData(structured);
						if (
							Array.isArray(mapped.extractionPairs) &&
							mapped.extractionPairs.length > 0
						) {
							appendLog(
								"Structured fields ready (" +
									String(structured.source || "heuristic") +
									").",
							);
							return buildLegacyReportHtml(
								new Date().toLocaleString(),
								String((me && me.username) || ""),
								mapped.extractionPairs,
								mapped.evidenceLines || [],
							);
						}
					}
				} catch (_structuredErr) {
					appendLog(
						"Structured field build unavailable; falling back to extraction history.",
					);
				}
			}
			...
			await Promise.all(
				(currentDocs || []).map(async function (doc) {
					...
					try {
						...
						const latest =
							Array.isArray(history && history.items) &&
						...
						const entities =
							...
							typeof latest.extracted_entities === "object"
								? latest.extracted_entities
								: {};
						...
						const exclusionText = [
							...
							.join(" ");
						if (entities.kyc_excluded || isKycIdentityDocName(exclusionText))
							return;

						const focusedName = toFlatText(
							entities.name ||
								entities.patient_name ||
								entities.beneficiary ||
								"",
						);
						if (focusedName && !isLikelyOrgName(focusedName))
							extractionPairs.push({ key: "name", value: focusedName });

						const focusedDiagnosis = toFlatText(
							entities.diagnosis || entities.final_diagnosis || "",
						);
						if (focusedDiagnosis)
							extractionPairs.push({
								key: "diagnosis",
								value: focusedDiagnosis,
							});

						const focusedClinical = toFlatText(
							entities.clinical_findings ||
								entities.major_diagnostic_finding ||
								entities.summary ||
								"",
						);
						if (focusedClinical)
							extractionPairs.push({
								...
								value: focusedClinical,
							});

						const focusedConclusion = toFlatText(
							entities.detailed_conclusion ||
								entities.conclusion ||
								entities.rationale ||
								"",
						);
						if (focusedConclusion)
							extractionPairs.push({
								key: "detailed_conclusion",
								value: focusedConclusion,
							});

						const focusedInvestigations =
							entities.all_investigation_reports_with_values ||
							entities.all_investigation_report_lines ||
							entities.investigation_reports ||
							[];
						if (Array.isArray(focusedInvestigations)) {
							focusedInvestigations.forEach(function (row) {
								if (row && typeof row === "object") {
									const line = toFlatText(
										row.line || row.text || row.value || row.result || "",
									);
									if (line)
										extractionPairs.push({
											key: "all_investigation_reports_with_values",
						...
						const refs = Array.isArray(latest.evidence_refs)
							? latest.evidence_refs
							: [];
						refs.forEach(function (entry) {
							if (!entry || typeof entry !== "object") return;
							const snippet = toFlatText(
								entry.snippet || entry.text || entry.value || entry.note || "",
							);
							if (!snippet) return;

							const looksLikeRawJson =
								(snippet.indexOf("{") === 0 || snippet.indexOf("[") === 0) &&
								(snippet.indexOf('\"extracted_entities\"') >= 0 ||
									snippet.indexOf('\"evidence_refs\"') >= 0);
							...
							if (
								/\d/.test(snippet) &&
								/\b(hb|hgb|hemoglobin|wbc|rbc|platelet|mcv|mch|mchc|rdw|creatinine|urea|bun|sodium|potassium|bilirubin|sgot|sgpt|alt|ast|urine|glucose)\b/i.test(
									snippet,
								)
							) {
								extractionPairs.push({
		...
		function renderGeneratedReport() {
			...
			if (!hasGeneratedReport || !currentClaim) {
				...
				generatedReportEl.innerHTML =
					"Click Generate Report to build latest report.";
				return;
			}
		...
		function getCurrentReportHtml() {
			...
			return String(
				latestGeneratedReportHtml ||
					(generatedReportEl ? generatedReportEl.innerHTML : "") ||
					"",
			).trim();
		}
		...
		function openFullReportView() {
			...
			if (fullReportBodyEl) {
				fullReportBodyEl.innerHTML =
					'<div class="sheet"><div id="report-editor" contenteditable="true">' +
					reportHtml +
					"</div></div>";
			}
		...
		function openReportInBrowserTab(reportHtml, targetWindow) {
			...
			const claimLabel = String(
				(currentClaim && currentClaim.external_claim_id) || routeClaimId || "",
			).trim();
			const actorIdText =
				String((me && me.username) || "doctor-ui").trim() || "doctor-ui";
			const title = claimLabel
				? "Claim Report - " + claimLabel
				: "Claim Report";
			const draftKey =
				"qc_report_draft_" +
				String(Date.now()) +
		...
		async function grammarCheckReportHtmlForClaim(reportHtml, onLog) {
			...
			try {
				const body = await apiFetch(
					"/api/v1/claims/" +
						encodeURIComponent(claimUuidText) +
						"/reports/grammar-check",
					{
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
				...
				const correctedHtml = String(
					body && body.corrected_html ? body.corrected_html : "",
				).trim();
				const changed = !!(body && body.changed);
				const correctedSegments = Number(
					(body && body.corrected_segments) || 0,
				);
				...
				if (typeof onLog === "function") {
					if (changed) {
						onLog(
							"Grammar check applied: corrected " +
								String(correctedSegments) +
								"/" +
								String(checkedSegments) +
								" segment(s)." +
								(model ? " Engine: " + model : ""),
						);
					} else {
						onLog(
							"Grammar check completed: no corrections needed." +
								(model ? " Engine: " + model : ""),
						);
					}
				}
			...
			try {
				...
				docRef.write(
					'<!doctype html><html><head><meta charset="UTF-8"><title>Generating Report...</title>' +
						...
						"</style></head><body>" +
						'<div class="card"><div class="row"><div class="spinner"></div><div id="report-gen-status" class="stage">' +
						escapeHtml(stage) +
						"</div></div>" +
						'<div id="report-gen-timer" class="timer">Elapsed: ' +
						escapeHtml(elapsed) +
						"</div></div>" +
						"</body></html>",
				);
		...
		async function checkExistingExtractionCoverage(docs, onProgress) {
			...
			for (let idx = 0; idx < eligibleDocs.length; idx += 1) {
				...
				if (typeof onProgress === "function")
					onProgress(idx + 1, eligibleDocs.length, docName);
		...
		async function saveCurrentReportToDb(reportSource, options) {
			...
			if (!reportHtml) {
				appendLog(
					"Current report HTML is empty. Attempting rebuild before save...",
				);
				try {
					latestGeneratedReportHtml =
						await buildLegacyReportHtmlFromLatestData();
					renderGeneratedReport();
					reportHtml = getCurrentReportHtml();
				} catch (rebuildErr) {
					appendLog(
						"Rebuild before save failed: " +
							String(
			...
			if (!reportHtml) {
				try {
					const loaded = await loadSavedReportBySource(
						preferredReportSource || "doctor",
						true,
					);
			...
			if (!reportHtml) {
				if (!silent)
					setMessage(
						"case-detail-msg",
						"err",
						"Generate report first, then save.",
					);
				return;
			}
			...
			const targetSource =
				String(reportSource || "doctor").toLowerCase() === "system"
					? "system"
					: "doctor";
			if (!silent)
				setMessage(
					"case-detail-msg",
					"",
					"Saving " + targetSource + " report to DB...",
				);
			appendLog("Save Report started." + (silent ? " (silent mode)" : ""));
			try {
				const saved = await apiFetch(
					"/api/v1/claims/" + encodeURIComponent(claimUuid) + "/reports/html",
					{
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
		...
		async function loadSavedReportBySource(source, silentIfMissing, options) {
			const targetSource =
				String(source || "doctor").toLowerCase() === "system"
					? "system"
					: "doctor";
			try {
				const opts = options || {};
				...
				const payload = await apiFetch(
					"/api/v1/user-tools/completed-reports/" +
						encodeURIComponent(claimUuid) +
						"/latest-html?source=" +
						encodeURIComponent(targetSource),
				);
				const html = String(
					payload && payload.report_html ? payload.report_html : "",
				).trim();
				if (!html)
					throw new Error(
				...
				const reportCreatedAtMs = Date.parse(
					String((payload && payload.created_at) || ""),
				);
				const checklistGeneratedAtMs = Date.parse(
					String(
						(currentChecklistLatest && currentChecklistLatest.generated_at) ||
							"",
					),
				);
				const isSavedReportStale =
					Number.isFinite(reportCreatedAtMs) &&
				...
				if (isSavedReportStale) {
					if (!allowStale) {
						appendLog(
							(targetSource === "system" ? "System" : "Doctor") +
								" saved report is older than latest checklist decision. Skipping stale report load.",
						);
						return false;
					}
					appendLog(
						(targetSource === "system" ? "System" : "Doctor") +
							" saved report is older than latest checklist decision. Loading stale report for manual edit/save.",
					);
					if (!silentIfMissing) {
						setMessage(
							"case-detail-msg",
							...
							"Loaded " +
								(targetSource === "system" ? "system" : "doctor") +
								" report (older than latest checklist). You can edit and save it.",
						);
					}
				}
				...
				if (!silentIfMissing) {
					setMessage(
						"case-detail-msg",
						"ok",
						(targetSource === "system" ? "System-generated" : "Doctor-saved") +
							" report loaded.",
					);
				}
				return true;
				...
				if (
					silentIfMissing &&
					msg.toLowerCase().indexOf("no saved report html") >= 0
				)
					return false;
				if (!silentIfMissing) {
					setMessage(
						"case-detail-msg",
						"err",
						msg || "Failed to load saved report.",
					);
				}
				appendLog(
					"Load saved report failed (" +
						targetSource +
						"): " +
						(msg || "unknown error"),
				);
		...
		async function loadDetail() {
			...
			const claim = await apiFetch(
				"/api/v1/claims/" + encodeURIComponent(claimUuid),
			);
			currentClaim = claim;

			const [docsResult, checklistLatest, latestDecide] = await Promise.all([
				apiFetch(
					"/api/v1/claims/" +
						encodeURIComponent(claimUuid) +
						"/documents?limit=200&offset=0",
			...
			statusParams.set(
				"search_claim",
				String(claim.external_claim_id || routeClaimId || ""),
			);
			if (me && me.role === "doctor" && me.username)
				statusParams.set("doctor_filter", me.username);
			statusParams.set("status_filter", "all");
			statusParams.set("limit", "50");
			statusParams.set("offset", "0");

			const statusResult = await apiFetch(
				"/api/v1/user-tools/claim-document-status?" + statusParams.toString(),
			).catch(function () {
				return { items: [] };
			});

			const statusItems = Array.isArray(statusResult && statusResult.items)
				? statusResult.items
				: [];
			const statusItem =
				statusItems.find(function (item) {
			...
			detailRows.push(
				summaryRow(
					"Claim ID",
					asTextCell(claim.external_claim_id || routeClaimId || "-"),
				),
			);
			detailRows.push(
				summaryRow("Claim Date", asTextCell(formatDateOnly(claim.created_at))),
			);
			detailRows.push(
				summaryRow("Beneficiary", asTextCell(claim.patient_name || "-")),
			);
			detailRows.push(
				summaryRow(
					"Patient Identifier",
					asTextCell(claim.patient_identifier || "-"),
				),
			);
			detailRows.push(
				summaryRow(
					"Assigned Doctor",
					asTextCell(claim.assigned_doctor_id || "-"),
				),
			);
			detailRows.push(
				summaryRow(
					"Allotment Date",
					asTextCell(formatDateOnly(statusItem.allotment_date || "")),
				),
			);
			detailRows.push(
				summaryRow(
					"Import DOA",
					asTextCell(formatDateOnly(statusItem.doa_date || "")),
				),
			);
			detailRows.push(
				summaryRow(
					"Import DOD",
					asTextCell(formatDateOnly(statusItem.dod_date || "")),
				),
			);
			detailRows.push(
				summaryRow(
					"Assigned At",
					asTextCell(formatDateTime(statusItem.assigned_at || "")),
				),
			);
			detailRows.push(
				summaryRow(
					"Documents",
					asTextCell(String(statusItem.documents || docsResult.total || 0)),
				),
			);
			detailRows.push(
				summaryRow(
					"Last Upload",
					asTextCell(formatDateTime(statusItem.last_upload || "")),
				),
			);
			detailRows.push(
				summaryRow(
					"Current Status",
					asTextCell(formatStatusText(claim.status || "-")),
				),
			);
			detailRows.push(
				summaryRow(
					"Final Status",
					asTextCell(
						statusItem.final_status ||
							(checklistLatest && checklistLatest.recommendation
								? checklistLatest.recommendation
								: "Pending"),
					),
				),
			);
			detailRows.push(
				summaryRow("Doctor Opinion", asTextCell(statusItem.opinion || "-")),
			);
			detailRows.push(
				summaryRow("Priority", asTextCell(String(claim.priority || "-"))),
			);
			detailRows.push(
				summaryRow("Source Channel", asTextCell(claim.source_channel || "-")),
			);
			detailRows.push(
				summaryRow(
					"Tags",
					asTextCell(Array.isArray(claim.tags) ? claim.tags.join(", ") : "-"),
				),
			);
			...
			const docs = Array.isArray(docsResult && docsResult.items)
				...
				: [];
			currentDocs = docs;
			currentChecklistLatest = checklistLatest || {
				found: false,
				checklist: [],
			};
			try {
				renderCaseBrain(latestDecide || {}, currentChecklistLatest);
			} catch (_err) {
			}
			...
			const docRows = docs
				.map(function (doc) {
					return (
						...
						"</td>" +
						"<td>" +
						statusChip(doc.parse_status || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(doc.uploaded_by || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(formatDateTime(doc.uploaded_at || "")) +
						"</td>" +
						'<td><button type="button" class="btn-soft" data-doc-open="' +
						escapeHtml(doc.id || "") +
						'">Open</button></td>' +
						"</tr>"
					);
			...
			docsEl.innerHTML =
				docRows ||
				'<tr><td colspan="5">No documents found for this claim.</td></tr>';
			...
			const loadedPreferred = await loadSavedReportBySource(
				preferredReportSource,
				true,
				{ allowStale: false },
			);
			...
			appendLog(
				"Case detail loaded for claim " +
					String(claim.external_claim_id || routeClaimId || claimUuid) +
					".",
			);
		}
		...
		async function runPipelineAction(
			...
			try {
				const summary = await runCasePreparationPipeline(
					claimUuid,
					me && me.username ? me.username : "doctor-ui",
					{
						...
						extractionProvider: pipelineOptions.extractionProvider || "auto",
				...
				if (summary.checklistError) {
					setMessage(
						...
						"err",
						label +
							" completed with checklist error. New extractions: " +
							String(summary.extractedCount) +
							", skipped: " +
					...
					setMessage(
						...
						"err",
						label +
							" completed with extraction failures. New extractions: " +
							String(summary.extractedCount) +
							", skipped: " +
							String(summary.skippedCount) +
							", failed: " +
				...
				setMessage(
					"case-detail-msg",
					"err",
					err && err.message ? err.message : label + " failed",
				);
				appendLog(
					label + " failed: " + String(err && err.message ? err.message : err),
				);
			} finally {
				setActionDisabled(false);
			}
		...
		analyzeBtn.addEventListener("click", async function () {
			await runPipelineAction(
				...
				false,
				{ extractionProvider: "auto", allowAutoFallback: true },
			);
		});

		forceBtn.addEventListener("click", async function () {
			await runPipelineAction(true, false, "Force VerifAI Analyzer", false, {
				extractionProvider: "auto",
		...
		reportBtn.addEventListener("click", async function () {
			...
			try {
				...
				renderProgress();
				try {
					const latestChecklist = await apiFetch(
						"/api/v1/claims/" +
							encodeURIComponent(claimUuid) +
							"/checklist/latest",
					);
					if (latestChecklist && latestChecklist.found) {
						...
						appendLog(
							"Latest checklist decision refreshed: " +
								String(latestChecklist.recommendation || "unknown"),
						);
					}
					appendLog(
						"Running fresh checklist evaluation for report conclusion...",
					);
					try {
						await apiFetch(
							"/api/v1/claims/" +
						...
						appendLog(
							"Structured AI data ready (diagnosis extracted) for checklist.",
						);
					} catch (_err) {
						// Non-blocking: checklist endpoint also auto-generates structured data server-side.
					}
					const evaluatedChecklist = await apiFetch(
						"/api/v1/claims/" +
							encodeURIComponent(claimUuid) +
							"/checklist/evaluate",
						{
					...
					if (
						evaluatedChecklist &&
						Array.isArray(evaluatedChecklist.checklist)
					) {
						currentChecklistLatest = evaluatedChecklist;
						appendLog(
							"Checklist evaluation completed for report generation: " +
								String(evaluatedChecklist.recommendation || "unknown"),
						);
					}
				} catch (refreshErr) {
					appendLog(
						"Checklist refresh before report generation failed: " +
							String(
				...
				const coverage = await checkExistingExtractionCoverage(
					docsForCoverage,
					function (index, total, fileName) {
						stageText =
							"Checking existing AI response: " +
							String(index) +
							"/" +
							String(total) +
							" (" +
				...
				appendLog(
					"Existing extraction coverage: " +
						String(coverage.withExtractionCount) +
						"/" +
						String(coverage.eligibleCount) +
						" clinical document(s).",
				);
				...
				if ((coverage.missingDocs || []).length > 0) {
					...
					appendLog(
						"Missing AI response for " +
							String(missing) +
							" document(s). Auto-running extraction pipeline...",
					);
					setMessage(
						"case-detail-msg",
						"",
						"Running AI extraction for " +
							String(missing) +
							" missing document(s)...",
					);
					...
					try {
						...
						const recheck = await checkExistingExtractionCoverage(
							docsForCoverage,
							function (index, total, fileName) {
								stageText =
						...
						appendLog(
							"Post-extraction coverage: " +
								String(recheck.withExtractionCount) +
								"/" +
								String(recheck.eligibleCount) +
								" clinical document(s).",
						);

						if ((recheck.missingDocs || []).length > 0) {
							const stillMissing = Number(
								(recheck.missingDocs || []).length || 0,
							);
							const msg =
								"Extraction completed but " +
								String(stillMissing) +
								" document(s) still missing AI response. You may need to retry or check document quality.";
							setMessage("case-detail-msg", "err", msg);
							appendLog(msg);
							renderReportLoadingTab(
								reportTab,
								"Extraction incomplete.",
								formatElapsedClock(Date.now() - startedAt),
							);
							try {
				...
				appendLog(
					"Existing AI response found for all clinical documents. Reusing extraction data for report.",
				);
				...
				try {
					...
					latestGeneratedReportHtml = String(
						aiReport && aiReport.report_html ? aiReport.report_html : "",
					).trim();
					reportSavedByServer = !!(aiReport && aiReport.saved);
					if (aiReport && aiReport.saved_version_no) {
						appendLog(
							"AI report saved (system). Version: " +
								String(aiReport.saved_version_no) +
								".",
						);
					} else {
					...
					appendLog(
						"AI report generator failed; falling back to legacy report builder. Error: " +
							String(aiErr && aiErr.message ? aiErr.message : aiErr),
					);
					latestGeneratedReportHtml =
						await buildLegacyReportHtmlFromLatestData();
				...
				if (!String(latestGeneratedReportHtml || "").trim()) {
					appendLog(
						"Generated report HTML was empty. Falling back to template report.",
					);
				...
				if (!reportSavedByServer) {
					...
					renderProgress();
					await saveCurrentReportToDb("system", {
						silent: true,
						manageBusy: false,
					});
				}
				...
				const finalHtml = String(
					getCurrentReportHtml() || latestGeneratedReportHtml || "",
				).trim();
				...
				if (opened) {
					...
					setMessage(
						"case-detail-msg",
						"err",
						"Could not open new tab. Please allow popups and click Generate Report again.",
					);
					appendLog("Report open failed: popup blocked or tab unavailable.");
				}
			} catch (err) {
				const errMsg = String(
					err && err.message ? err.message : err || "Report generation failed",
				);
				setMessage("case-detail-msg", "err", errMsg);
		...
		window.addEventListener("message", async function (event) {
			try {
				...
				if (String(payload.claim_uuid || "") !== String(claimUuid || ""))
					return;
				appendLog(
					"Report saved from new tab. Refreshing latest doctor report.",
				);
				await loadSavedReportBySource("doctor", true);
				setMessage(
					"case-detail-msg",
					"ok",
					"Report saved from new tab and refreshed here.",
				);
			} catch (_syncErr) {}
		});
		if (sendBackBtn) {
			sendBackBtn.addEventListener("click", async function () {
				...
				try {
					await apiFetch(
						"/api/v1/claims/" + encodeURIComponent(claimUuid) + "/status",
						{
							method: "PATCH",
							headers: { "Content-Type": "application/json" },
		...
		try {
			...
			setMessage(
				"case-detail-msg",
				"err",
				err.message || "Failed to load case detail.",
			);
			appendLog(
				"Case detail load failed: " +
					String(err && err.message ? err.message : err),
			);
		}
	}
	...
	async function renderUploadDocument() {
		...
		contentPanel.innerHTML =
			...
			'<div class="claim-filter-group"><label for="upload-doc-doctor-filter">Doctor</label><select id="upload-doc-doctor-filter" name="doctor_filter">' +
			doctorFilterOptions +
			"</select></div>" +
			'<div class="claim-filter-action"><button type="submit" class="claim-apply-btn">Apply</button></div>' +
			"</form>" +
		...
		const uploadDocSortButtons = Array.from(
			contentPanel.querySelectorAll("button[data-upload-doc-sort]"),
		);

		const modalEl = document.getElementById("upload-doc-modal");
		const modalSubtitleEl = document.getElementById(
			"upload-doc-modal-subtitle",
		);
		...
		const modalSelectAllBtn = document.getElementById(
			"upload-doc-modal-select-all",
		);
		...
		const modalSelectPageEl = document.getElementById(
			"upload-doc-modal-select-page",
		);
		...
		function getSelectedModalDocIds() {
			return Array.from(
				modalTbody.querySelectorAll("input[data-modal-doc-select]:checked"),
			)
				.map((el) => String(el.value || "").trim())
				.filter(Boolean);
		}
		...
		function updateModalSelectionUi() {
			const allCheckboxes = Array.from(
				modalTbody.querySelectorAll("input[data-modal-doc-select]"),
			);
			...
			if (modalCountEl) {
				modalCountEl.textContent =
					"Total documents: " +
					String(viewState.total || total) +
					" | Source files: " +
					String(viewState.sourceFiles || 0) +
					" | Selected: " +
					String(selected);
			}
			if (modalDeleteBtn) {
		...
		function updateUploadDocSortButtonsUi() {
			uploadDocSortButtons.forEach(function (btn) {
				const key = String(
					btn.getAttribute("data-upload-doc-sort") || "",
				).trim();
				const baseLabel = String(
					btn.getAttribute("data-sort-label") || "",
				).trim();
				...
				btn.classList.toggle("is-active", !!active);
				btn.textContent = active
					? baseLabel + (uploadDocSortState.dir === "asc" ? " ^" : " v")
					: baseLabel;
			});
		}
		...
		function applyUploadDocSort() {
			...
			const decorated = rows.map(function (row, idx) {
				const cell = row.children[cfg.index];
				const value = parseUploadDocSortValue(
					cell ? cell.textContent : "",
					cfg.type,
				);
				return { row: row, idx: idx, value: value };
		...
		async function openDocumentFile(docId) {
			...
			try {
				...
				setMessage(
					"upload-doc-list-msg",
					"err",
					err && err.message ? err.message : "Failed to open document.",
				);
			}
		}
		...
		async function loadModalDocuments() {
			...
			if (!claimKey) {
				modalTbody.innerHTML =
					'<tr><td colspan="8">Select a claim to view documents.</td></tr>';
				viewState.total = 0;
			...
			modalTbody.innerHTML =
				'<tr><td colspan="8">Loading uploaded documents...</td></tr>';
			try {
				const docsResult = await apiFetch(
					"/api/v1/claims/" +
				...
				const docs = Array.isArray(docsResult && docsResult.items)
					? docsResult.items
					: [];
				viewState.total = Number(
					docsResult && docsResult.total ? docsResult.total : docs.length,
				);
				...
				const rows = docs
					.map(function (doc) {
						const meta =
							doc && doc.metadata && typeof doc.metadata === "object"
								? doc.metadata
								: {};
						const mergedCount = Number(
							meta.merge_source_file_count ||
								meta.merge_accepted_file_count ||
								0,
						);
						...
						const outputSizeBytes = Number(
							doc.file_size_bytes || meta.merged_output_size_bytes || 0,
						);
						const sourceSizeBytes = Number(
							meta.merged_source_total_size_bytes || 0,
						);
						const mergeLabel =
							mergedCount > 0
								? "Merged (" + String(mergedCount) + ")"
								: "Single";
						const compressionNote =
							mergedCount > 0 && sourceSizeBytes > 0
								? '<div class="muted">Compressed: ' +
									escapeHtml(formatBytes(sourceSizeBytes)) +
									" -> " +
									escapeHtml(formatBytes(outputSizeBytes)) +
									"</div>"
								: "";
						return (
							"<tr>" +
							'<td class="claim-select-col"><input type="checkbox" data-modal-doc-select value="' +
							escapeHtml(doc.id || "") +
							'"></td>' +
							"<td>" +
							...
							escapeHtml(formatDateTime(doc.uploaded_at || "")) +
							"</td>" +
							'<td><button type="button" class="btn-soft" data-upload-doc-open="' +
							escapeHtml(doc.id || "") +
							'">Open</button></td>' +
							"</tr>"
						);
				...
				modalTbody.innerHTML =
					rows ||
					'<tr><td colspan="8">No uploaded documents found for this claim.</td></tr>';

				modalTbody
					.querySelectorAll("input[data-modal-doc-select]")
					.forEach(function (el) {
						el.addEventListener("change", updateModalSelectionUi);
					});

				modalTbody
					.querySelectorAll("button[data-upload-doc-open]")
					.forEach(function (btn) {
						btn.addEventListener("click", async function () {
							await openDocumentFile(
								String(this.getAttribute("data-upload-doc-open") || ""),
							);
						});
					});
				...
				modalTbody.innerHTML =
					'<tr><td colspan="8">Failed to load uploaded documents.</td></tr>';
				viewState.total = 0;
				viewState.sourceFiles = 0;
				updateModalSelectionUi();
				setModalMessage(
		...
		async function deleteSelectedDocuments() {
			...
			const ok = window.confirm(
				"Delete " +
					...
					" selected document(s)? This cannot be undone.",
			);
			if (!ok) return;
			...
			try {
				const result = await apiFetch(
					"/api/v1/claims/" + encodeURIComponent(claimKey) + "/documents",
					{
						method: "DELETE",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
				...
				const notFound = Number(
					result && result.not_found ? result.not_found : 0,
				);
				...
				setMessage(
					"upload-doc-list-msg",
					"ok",
					"Documents delete completed for claim " +
						String(viewState.claimId || "-") +
						".",
				);
		...
		async function uploadFilesForClaim(claimId, files) {
			...
			if (masterDocs.length > 0 && list.length > masterDocs.length) {
				const keepMasterOnly = window.confirm(
					"You selected ALL DOCUMENTS with other files. For smaller merged PDF, click OK to upload only ALL DOCUMENTS. Click Cancel to keep all files.",
				);
				if (keepMasterOnly) {
					list = masterDocs;
					setMessage(
						"upload-doc-list-msg",
						"",
						"Smart compression: uploading only ALL DOCUMENTS file(s) to avoid duplicate pages.",
					);
				}
			}

			const claimExternalId = String(
				fileInput.getAttribute("data-claim-external-id") ||
					viewState.claimId ||
					"",
			).trim();
			...
			const previewNames = list
				.slice(0, 5)
				.map(function (f) {
					return String(f && f.name ? f.name : "file");
				})
				.join(", ");
			...
			setModalBusy(
				...
				"Uploading and merging " +
					...
					(previewNames
						? " [" +
							previewNames +
							(moreCount > 0 ? ", +" + String(moreCount) + " more" : "") +
							"]"
						: ""),
			);
			setMessage(
			...
			try {
				const mergedResult = await apiFetch(
					"/api/v1/claims/" +
						encodeURIComponent(claimKey) +
						"/documents/merged",
					{
				...
				const sourceCount = Number(
					(mergedResult && mergedResult.source_file_count) || list.length || 0,
				);
				const acceptedCount = Number(
					(mergedResult && mergedResult.accepted_file_count) || 0,
				);
				const skippedCount = Number(
					(mergedResult && mergedResult.skipped_file_count) || 0,
				);
				const mergedDocName = String(
					(mergedResult &&
						mergedResult.document &&
						mergedResult.document.file_name) ||
						"merged_document.pdf",
				);
				const mergeProfile = String(
					(mergedResult &&
						mergedResult.document &&
						mergedResult.document.metadata &&
						mergedResult.document.metadata.merge_profile) ||
						"standard",
				);
				const sourceSizeBytes = Number(
					(mergedResult && mergedResult.merged_source_total_size_bytes) || 0,
				);
				const outputSizeBytes = Number(
					(mergedResult && mergedResult.merged_output_size_bytes) ||
						(mergedResult &&
							mergedResult.document &&
							mergedResult.document.file_size_bytes) ||
						0,
				);
				const savedSizeBytes = Number(
					(mergedResult && mergedResult.merged_saved_size_bytes) ||
						Math.max(0, sourceSizeBytes - outputSizeBytes),
				);
				const lowSavings =
					sourceSizeBytes > 0 && outputSizeBytes >= sourceSizeBytes * 0.98;

				const successText =
					"Merged upload complete. Source: " +
					String(sourceCount) +
					", accepted: " +
					...
					(sourceSizeBytes > 0
						? ", Compressed: " +
							...
							formatBytes(outputSizeBytes) +
							" (saved " +
							formatBytes(savedSizeBytes) +
							")"
						: "") +
					", Mode: " +
					mergeProfile +
					(lowSavings
						? ". Note: Source files are already PDF/compressed, so additional compression is limited."
						: "");
		...
		async function queueAiBatch(claimUuid, buttonEl) {
			...
			function openAiBatchProgressModal() {
				...
				backdrop.innerHTML =
					...
					"</div>" +
					'<p class="muted" id="ai-progress-subtitle">Claim UUID: ' +
					escapeHtml(claimKey) +
					"</p>" +
				...
				const refreshBtn = backdrop.querySelector(
					"#ai-progress-refresh-events",
				);
					...
					try {
						...
						items
							...
							.forEach(function (evt) {
								...
								seenEventIds.add(id);
								const when =
									evt && evt.occurred_at ? String(evt.occurred_at) : "";
								const type =
									evt && evt.event_type ? String(evt.event_type) : "event";
								...
								if (pl && typeof pl === "object") {
									...
									if (compact && compact.length <= 700)
										pushLine("  payload: " + compact);
								}
							});
					} catch (err) {
						pushLine(
							"server_event_refresh_failed: " +
								String(err && err.message ? err.message : err),
						);
					}
				}
				...
				function setProgress(processed, total, label, meta) {
					...
					if (processedEl)
						processedEl.textContent = String(p) + "/" + String(total || 0);
					const m = meta && typeof meta === "object" ? meta : {};
					if (extractedEl)
						extractedEl.textContent = String(
							m.extractedCount != null ? m.extractedCount : "",
						);
					if (skippedEl)
						skippedEl.textContent = String(
							m.skippedCount != null ? m.skippedCount : "",
						);
					if (failedEl)
						failedEl.textContent = String(
							m.failedCount != null ? m.failedCount : "",
						);
					...
					if (stageEl) {
						...
						stageEl.textContent =
							(stage ? "Stage: " + stage + " | " : "") +
							(label ? "Now: " + String(label) : "Running…");
					}
				}
				...
				if (refreshBtn)
					refreshBtn.addEventListener("click", refreshWorkflowEvents);
			...
			try {
				...
				const summary = await runCasePreparationPipeline(
					claimKey,
					String((me && me.username) || "user-ui"),
					{
						force: false,
						preferOpenAI: false,
						strictOpenAI: false,
						onProgress: function (processed, total, label, meta) {
							if (progressModal && progressModal.setProgress)
								progressModal.setProgress(processed, total, label, meta);
						},
						onLog: function (line) {
							if (progressModal && progressModal.log)
								progressModal.log(String(line || ""));
						},
					},
				);
				try {
					if (progressModal && progressModal.refreshWorkflowEvents)
						await progressModal.refreshWorkflowEvents();
					if (progressModal && progressModal.log)
						progressModal.log(
							"AI batch finished. extracted=" +
								String(summary.extractedCount || 0) +
								", skipped=" +
				...
				setMessage(
					"upload-doc-list-msg",
					"ok",
					"Queue AI Batch done. New extractions: " +
						String(summary.extractedCount || 0) +
						", skipped: " +
						String(summary.skippedCount || 0) +
						", failed: " +
				...
				setMessage(
					"upload-doc-list-msg",
					"err",
					err && err.message ? err.message : "Queue AI Batch failed.",
				);
			} finally {
				if (btn) {
					btn.disabled = false;
		...
		async function loadRows(resetPage) {
			...
			const searchClaim = String(
				document.getElementById("upload-doc-search").value || "",
			).trim();
			const allotmentDate = String(
				document.getElementById("upload-doc-allotment-date").value || "",
			).trim();
			const statusFilter = String(
				document.getElementById("upload-doc-status-filter").value || "all",
			).trim();
			const doctorFilter = String(
				document.getElementById("upload-doc-doctor-filter").value || "",
			).trim();
			const documentUploadFilter = String(
				document.getElementById("upload-doc-document-upload-filter").value ||
					"all",
			)
				.trim()
				.toLowerCase();
			...
			if (statusFilter) params.set("status_filter", statusFilter);
			if (doctorFilter) params.set("doctor_filter", doctorFilter);
			if (documentUploadFilter && documentUploadFilter !== "all")
				params.set("document_upload", documentUploadFilter);
			params.set("exclude_completed_uploaded", "true");
			params.set("exclude_withdrawn", "true");
			params.set("limit", String(state.pageSize));
			params.set("offset", String((state.page - 1) * state.pageSize));
			...
			const rows = (result.items || [])
				.map((c) => {
					...
					const sourceFilesCount = Number(
						c.source_files || documentsCount || 0,
					);
					...
					return (
						...
						"</td>" +
						"<td>" +
						escapeHtml(formatAssignedDoctor(c.assigned_doctor_id)) +
						"</td>" +
						"<td>" +
						escapeHtml(formatDateOnly(c.allotment_date)) +
						"</td>" +
						"<td>" +
						escapeHtml(documentsCount > 0 ? "Yes" : "No") +
						"</td>" +
						"<td>" +
						escapeHtml(mergeSummary) +
						"</td>" +
						"<td>" +
						escapeHtml(lastUploadedBy || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(formatDateTime(c.last_upload)) +
						"</td>" +
						'<td><div class="doctor-case-actions">' +
						'<button type="button" class="btn-soft" data-upload-claim="' +
						escapeHtml(c.id || "") +
						'" data-upload-claim-id="' +
						escapeHtml(c.external_claim_id || "") +
						'">Upload</button>' +
						'<button type="button" class="btn-soft" data-view-claim="' +
						escapeHtml(c.id || "") +
						'" data-view-claim-id="' +
						escapeHtml(c.external_claim_id || "") +
						'">View</button>' +
						'<button type="button" class="btn-soft" data-queue-claim="' +
						escapeHtml(c.id || "") +
						'"' +
						(canQueue ? "" : " disabled") +
						">Queue AI Batch</button>" +
						"</div></td>" +
						"</tr>"
					);
				})
			...
			tbody.querySelectorAll("button[data-upload-claim]").forEach((btn) => {
				btn.addEventListener("click", async function () {
					const claimId = String(
						this.getAttribute("data-upload-claim") || "",
					).trim();
					const claimExternalId = String(
						this.getAttribute("data-upload-claim-id") || "",
					).trim();
					if (!claimId) return;
					...
					setModalMessage(
						"",
						"Select files now. Upload will run in this modal.",
					);
					fileInput.click();
				});
			});
			...
			tbody.querySelectorAll("button[data-queue-claim]").forEach((btn) => {
				btn.addEventListener("click", async function () {
					const claimId = String(
						this.getAttribute("data-queue-claim") || "",
					).trim();
		...
		fileInput.addEventListener("change", async function () {
			const claimId = String(
				fileInput.getAttribute("data-claim-id") || "",
			).trim();
		...
		uploadDocSortButtons.forEach(function (btn) {
			btn.addEventListener("click", function () {
				const key = String(
					this.getAttribute("data-upload-doc-sort") || "",
				).trim();
				const cfg = uploadDocSortConfig[key];
				if (!cfg) return;
				if (uploadDocSortState.key === key) {
					uploadDocSortState.dir =
						uploadDocSortState.dir === "asc" ? "desc" : "asc";
				} else {
		...
		try {
			...
			setMessage("upload-doc-list-msg", "err", err.message);
			tbody.innerHTML =
				'<tr><td colspan="7">Failed to load claim document status.</td></tr>';
			updatePaginationUi();
		}
	}
	...
	async function renderUploadExcel() {
		contentPanel.innerHTML =
			'<h2>Upload Excel</h2><p class="muted">Upload case data in xlsx/csv/sql format.</p>' +
		...
		document
			.getElementById("upload-excel-form")
			.addEventListener("submit", async function (e) {
				e.preventDefault();
				const fd = new FormData(e.currentTarget);
				try {
					const result = await apiFetch("/api/v1/user-tools/upload-excel", {
						method: "POST",
						body: fd,
					});
					setMessage(
	...
	async function renderCompletedReports(initialStatusFilter) {
		...
		const canManageUpload = !!(
			me &&
			(me.role === "super_admin" || me.role === "user")
		);
		const canAuditQc = !!(
			me &&
			(me.role === "super_admin" || me.role === "auditor")
		);
		...
		const auditOnlyRole =
			routeRole === "auditor" || !!(me && me.role === "auditor");
		const isAuditorAuditClaimsPage =
			auditOnlyRole && routePage === "audit-claims";
		const isAuditorDashboardPage = auditOnlyRole && routePage === "dashboard";
		const shouldExcludeTaggedForAuditor =
			isAuditorAuditClaimsPage || isAuditorDashboardPage;
		const hideQcAndSystemActions =
			routeRole === "user" && routePage === "completed-not-uploaded";
		...
		const completedReportsFilterStorageKey =
			"qc_completed_reports_filters_v1:" +
			String(routeRole || "-") +
			":" +
			String(routePage || "-");
		...
		function writeCompletedReportFilters(payload) {
			try {
				...
				sessionStorage.setItem(
					completedReportsFilterStorageKey,
					JSON.stringify(data),
				);
			} catch (_err) {}
		}
		...
		const savedStatus = String(savedFilters.status || "")
			.trim()
			.toLowerCase();
		const savedQc = String(savedFilters.qc || "")
			.trim()
			.toLowerCase();
		...
		const state = {
			page: 1,
			pageSize:
				savedPageSize === 10 || savedPageSize === 20 || savedPageSize === 50
					? savedPageSize
					: 20,
			total: 0,
			status: validStatusFilters.includes(savedStatus)
				? savedStatus
				: initialStatusFilter === "uploaded" || initialStatusFilter === "all"
					? initialStatusFilter
					: "pending",
		...
		function isAuditorClaimLocked(row) {
			...
			if (
				!auditorAllotmentGate.enforced ||
				!Number.isFinite(auditorAllotmentGate.minDateTs)
			)
				return false;
		...
		contentPanel.innerHTML =
			...
			"</select></div>" +
			'<div class="claim-filter-group"><label for="completed-reports-doctor-filter">Doctor</label><select id="completed-reports-doctor-filter" name="doctor_filter">' +
			doctorFilterOptions +
			"</select></div>" +
		...
		const statusFilterEl = document.getElementById(
			"completed-reports-status-filter",
		);
		const qcFilterEl = document.getElementById("completed-reports-qc-filter");
		const doctorFilterEl = document.getElementById(
			"completed-reports-doctor-filter",
		);
		const searchFilterEl = document.getElementById("completed-reports-search");
		const allotmentDateEl = document.getElementById(
			"completed-reports-allotment-date",
		);

		const docsModalEl = document.getElementById("completed-doc-modal");
		const docsModalCloseBtn = document.getElementById(
			"completed-doc-modal-close",
		);
		const docsModalSubtitleEl = document.getElementById(
			"completed-doc-modal-subtitle",
		);
		...
		const uploadModalCloseBtn = document.getElementById(
			"completed-upload-modal-close",
		);
		const uploadModalClaimLabelEl = document.getElementById(
			"completed-upload-modal-claim-label",
		);
		const uploadModalMsgEl = document.getElementById(
			"completed-upload-modal-msg",
		);
		const uploadForm = document.getElementById("completed-upload-form");
		const uploadClaimUuidEl = document.getElementById(
			"completed-upload-claim-uuid",
		);
		...
		const uploadSubtaggingEl = document.getElementById(
			"completed-upload-subtagging",
		);
		...
		const qcModalClaimLabelEl = document.getElementById(
			"completed-qc-modal-claim-label",
		);
		...
		const reportEditorModalEl = document.getElementById(
			"completed-report-editor-modal",
		);
		const reportEditorCloseBtn = document.getElementById(
			"completed-report-editor-close",
		);
		const reportEditorClaimLabelEl = document.getElementById(
			"completed-report-editor-claim-label",
		);
		const reportEditorMsgEl = document.getElementById(
			"completed-report-editor-msg",
		);
		const reportEditorBodyEl = document.getElementById(
			"completed-report-editor-body",
		);
		const reportEditorSaveBtn = document.getElementById(
			"completed-report-editor-save",
		);
		...
		if (searchFilterEl)
			searchFilterEl.value = String(savedFilters.search_claim || "").trim();
		if (allotmentDateEl)
			allotmentDateEl.value = String(savedFilters.allotment_date || "").trim();
		if (doctorFilterEl) {
			const doctorSaved = String(savedFilters.doctor_filter || "").trim();
			const hasDoctorOption = !!Array.from(doctorFilterEl.options || []).find(
				function (opt) {
		...
		function normalizeTagging(value) {
			...
			if (normalized === "fraudulent" || normalized === "fraudlent")
				return "Fraudulent";
			return "";
		}
		...
		function normalizeOpinionText(value) {
			...
			let clean = stripped
				.replace(/trigger\s*remarks?\s*:?/gi, " ")
				.replace(/amber\s*flag\s*entities\s*in\s*this\s*claim\s*:?/gi, " ")
				.replace(/red\s*flag\s*entities\s*in\s*this\s*claim\s*:?/gi, " ")
				.replace(/look\s*alike\s*based\s*:?/gi, " ")
				.replace(/market\s*intelligence\s*:?/gi, " ")
				.replace(/hospital\s*not\s*verified\s*:?/gi, " ")
				.replace(/past\s*claims\s*with\s*similar\s*attributes[^|:]*:?/gi, " ")
				.replace(/\s*\|\s*/g, ", ")
				.replace(/\s{2,}/g, " ")
				.trim();
		...
		function setSubtaggingOptions(taggingValue, selectedSubtagging) {
			...
			const options = Array.isArray(taggingSubtaggingMap[key])
				? taggingSubtaggingMap[key]
				: [];
			uploadSubtaggingEl.innerHTML =
				'<option value="">Select Subtagging</option>';
		...
		async function showClaimDocuments(claimUuid, externalClaimId) {
			...
			docsModalSubtitleEl.textContent =
				"Claim ID: " + (String(externalClaimId || "").trim() || "-");
			setDocsModalMessage("", "");
			docsModalTbody.innerHTML =
			...
			try {
				...
				const rows = docs
					.map(function (doc) {
						return (
							...
							escapeHtml(formatDateTime(doc.uploaded_at || "")) +
							"</td>" +
							'<td><button type="button" class="btn-soft" data-completed-open-doc="' +
							escapeHtml(doc.id || "") +
							'">Open</button></td>' +
							"</tr>"
						);
				...
				docsModalTbody.innerHTML =
					rows ||
					'<tr><td colspan="5">No documents uploaded for this claim.</td></tr>';
				docsModalTbody
					.querySelectorAll("button[data-completed-open-doc]")
					.forEach(function (btn) {
						btn.addEventListener("click", async function () {
							await openDocumentFile(
								String(this.getAttribute("data-completed-open-doc") || ""),
							);
						});
					});
			} catch (err) {
				docsModalTbody.innerHTML =
					'<tr><td colspan="5">Failed to load documents.</td></tr>';
				setDocsModalMessage(
					"err",
					err && err.message ? err.message : "Failed to load documents.",
				);
			}
		}
		...
		function openUploadStatusModal(row) {
			...
			uploadModalClaimLabelEl.textContent =
				"Claim ID: " + (externalClaimId || "-");
			uploadStatusEl.value = "uploaded";
			const normalizedTagging = normalizeTagging(
				normalizeDisplayText(row.tagging),
			);
			uploadTaggingEl.value = normalizedTagging;
			setSubtaggingOptions(
				normalizedTagging,
				normalizeDisplayText(row.subtagging),
			);
		...
		function openQcStatusModal(row) {
			...
			qcStatusEl.value =
				String(row.qc_status || "no").toLowerCase() === "yes" ? "yes" : "no";
			setQcModalMessage("", "");
			openQcModal();
		}
		...
		function closeReportEditorModal() {
			...
			if (reportEditorClaimLabelEl)
				reportEditorClaimLabelEl.textContent = "Claim ID: -";
		}

		function openAuditorQcWorkspace(row) {
			const claimUuid = String(
				row && row.claim_uuid ? row.claim_uuid : "",
			).trim();
			const claimId = String(
				row && row.external_claim_id ? row.external_claim_id : "",
			).trim();
			if (!claimUuid) {
			...
			try {
				const payload = await apiFetch(
					"/api/v1/user-tools/completed-reports/" +
						encodeURIComponent(claimUuid) +
						"/latest-html?source=" +
						encodeURIComponent(reportEditorCurrentSource),
					{ timeoutMs: 15000 },
				);
				const html = String(
					payload && payload.report_html ? payload.report_html : "",
				...
				setReportEditorMessage(
					"err",
					err && err.message ? err.message : "Failed to load report HTML.",
				);
			}
		}
		...
		async function saveEditableReport() {
			...
			const oldText = reportEditorSaveBtn
				? reportEditorSaveBtn.textContent
				: "";
			...
			try {
				...
				setMessage(
					"completed-reports-msg",
					"ok",
					"Report updated for claim " +
						String(row.external_claim_id || "-") +
						".",
				);
				await loadRows(false);
			} catch (err) {
				setReportEditorMessage(
					"err",
					err && err.message ? err.message : "Failed to save report.",
				);
			} finally {
				if (reportEditorSaveBtn) {
		...
		function openCaseDetail(row, reportSource) {
			...
			const route = parseRoute();
			const role =
				String((route && route.routeRole) || "user").trim() || "user";
			const params = new URLSearchParams();
			params.set("claim_uuid", claimUuid);
			params.set("claim_id", String(row.external_claim_id || ""));
			const backPage = auditOnlyRole
				? "audit-claims"
				: initialStatusFilter === "uploaded"
					? "completed-uploaded"
					: "completed-not-uploaded";
			...
			params.set(
				"search_claim",
				String(
					document.getElementById("completed-reports-search").value || "",
				).trim(),
			);
			params.set(
				"allotment_date",
				String(
					document.getElementById("completed-reports-allotment-date").value ||
						"",
				).trim(),
			);
			const src =
			...
			window.location.href =
				"/qc/" + encodeURIComponent(role) + "/case-detail?" + params.toString();
		}
		...
		async function exportWord(row) {
			...
			const wrapped =
				'<html><head><meta charset="UTF-8"><title>Claim Report</title></head><body>' +
				html +
				"</body></html>";
			...
			a.download =
				"claim_report_" + String(row.external_claim_id || "report") + ".doc";
		...
		function waitForRenderImages(rootNode, timeoutMs) {
			...
			return Promise.all(
				images.map(function (img) {
					if (img && img.complete && Number(img.naturalWidth || 0) > 0)
		...
		async function exportPdf(row) {
			...
			const fileName =
				"claim_report_" + String(row.external_claim_id || "report") + ".pdf";
			...
			try {
				...
				const renderWidthPx = Math.max(
					500,
					Math.ceil(host && host.scrollWidth ? host.scrollWidth : 520),
				);

				let saved = false;
				try {
					saved = await exportPdfUsingHtmlRenderer(
						host,
						fileName,
						renderWidthPx,
					);
		...
		async function loadRows(resetPage) {
			...
			const searchClaim = String(
				searchFilterEl && searchFilterEl.value ? searchFilterEl.value : "",
			).trim();
			const allotmentDate = String(
				allotmentDateEl && allotmentDateEl.value ? allotmentDateEl.value : "",
			).trim();
			const statusFilterRaw = String(
				statusFilterEl && statusFilterEl.value
					? statusFilterEl.value
					: state.status,
			).trim();
			const qcFilterRaw = String(
				qcFilterEl && qcFilterEl.value ? qcFilterEl.value : state.qc || "no",
			).trim();
			const statusFilter = statusFilterRaw;
			const qcFilter = auditOnlyRole ? "no" : qcFilterRaw;
			const doctorFilter = String(
				doctorFilterEl && doctorFilterEl.value ? doctorFilterEl.value : "",
			).trim();
			...
			if (
				result &&
				result.tagging_subtagging_options &&
				typeof result.tagging_subtagging_options === "object"
			) {
				taggingSubtaggingMap = result.tagging_subtagging_options;
			}

			const rawItems = Array.isArray(result && result.items)
				? result.items
				: [];
			const items = auditOnlyRole
				? sortClaimsByAllotmentDateFirst(rawItems, true)
				: rawItems;
			if (auditOnlyRole) {
				const earliestItem =
					earliestResult &&
					Array.isArray(earliestResult.items) &&
					earliestResult.items.length
						? earliestResult.items[0]
						: null;
				const minDateTs = parseAllotmentDateValue(
					earliestItem && earliestItem.allotment_date,
				);
				auditorAllotmentGate = {
					enforced: Number.isFinite(minDateTs),
					minDateTs: minDateTs,
					minDateLabel: Number.isFinite(minDateTs)
						? formatDateOnly(new Date(minDateTs))
						: "",
				};
			} else {
				auditorAllotmentGate = {
			...
			const rows = items
				.map(function (r) {
					...
					const qcText =
						String(r.qc_status || "no").toLowerCase() === "yes" ? "Yes" : "No";
					const hasReportHtml = !!r.report_html_available;
					const hasDoctorReport =
						!!r.doctor_report_html_available ||
						(hasReportHtml &&
							String(r.latest_report_source || "doctor") !== "system");
					const hasSystemReport = !!r.system_report_html_available;
					const hasAnyReport =
						hasDoctorReport || hasSystemReport || hasReportHtml;
					...
					const auditLockTitle = isLockedForAudit
						? "Process allotment date " +
							String(auditorAllotmentGate.minDateLabel || "-") +
							" claim(s) first."
						: "";
					const primaryButtonsHtml = auditOnlyRole
						? '<button type="button" data-completed-view-html="' +
							escapeHtml(claimUuid) +
							'"' +
							(canOpenDoctorHtml && !isLockedForAudit ? "" : " disabled") +
							(auditLockTitle
								? ' title="' + escapeHtml(auditLockTitle) + '"'
								: "") +
							">View Doctor HTML</button>"
						: '<button type="button" data-completed-view-html="' +
							escapeHtml(claimUuid) +
							'"' +
							(hasDoctorReport ? "" : " disabled") +
							">View Doctor HTML</button>" +
							'<button type="button" data-completed-view-doc="' +
							escapeHtml(claimUuid) +
							'" data-completed-view-claim-id="' +
							escapeHtml(r.external_claim_id || "") +
					...
					if (auditOnlyRole) {
						...
						overflowButtonsHtml +=
							'<button type="button" data-completed-export-word="' +
							escapeHtml(claimUuid) +
							'"' +
							(hasAnyReport ? "" : " disabled") +
							">Export Word</button>";
						overflowButtonsHtml +=
							'<button type="button" data-completed-export-pdf="' +
							escapeHtml(claimUuid) +
							'"' +
							(hasAnyReport ? "" : " disabled") +
							">Export PDF</button>";
						overflowButtonsHtml +=
							'<button type="button" data-completed-pending="' +
							escapeHtml(claimUuid) +
							'">Pending</button>';
						overflowButtonsHtml +=
							'<button type="button" data-completed-withdraw="' +
							escapeHtml(claimUuid) +
							'">Withdraw</button>';
						overflowButtonsHtml +=
							'<button type="button" class="btn-soft" data-completed-open-upload="' +
							escapeHtml(claimUuid) +
							'">Update Upload</button>';
					}

					const moreMenuHtml = overflowButtonsHtml
						? '<details class="action-menu"><summary>More</summary><div class="action-menu-list">' +
							overflowButtonsHtml +
							"</div></details>"
						: "";
					...
					return (
						...
						"</td>" +
						"<td>" +
						escapeHtml(formatAssignedDoctor(r.assigned_doctor_id)) +
						"</td>" +
						"<td>" +
						escapeHtml(formatDateTime(r.completed_at || r.updated_at || "")) +
						"</td>" +
						"<td>" +
						escapeHtml(formatStatusText(r.report_status || "pending")) +
						"</td>" +
						"<td>" +
						escapeHtml(qcText) +
						"</td>" +
						"<td>" +
						escapeHtml(taggingText || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(subtaggingText || "-") +
						"</td>" +
						'<td title="' +
						escapeHtml(opinionText) +
						'">' +
						escapeHtml(opinionDisplay) +
						"</td>" +
						"<td>" +
						escapeHtml(String(r.report_count || 0)) +
						"</td>" +
			...
			tbody.innerHTML =
				rows || '<tr><td colspan="10">No completed reports found.</td></tr>';

			if (!canManageUpload) {
				tbody
					.querySelectorAll("button[data-completed-open-upload]")
					.forEach(function (btn) {
						btn.remove();
					});
			}
			if (!canAuditQc) {
				tbody
					.querySelectorAll(
						"button[data-completed-open-qc], button[data-completed-send-back]",
					)
					.forEach(function (btn) {
						btn.remove();
					});
			...
			if (!canChangeStatus) {
				tbody
					.querySelectorAll(
						"button[data-completed-pending], button[data-completed-withdraw]",
					)
					.forEach(function (btn) {
						btn.remove();
					});
			...
			tbody
				.querySelectorAll("button[data-completed-view-html]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
						const row = rowsByClaim.get(
							String(this.getAttribute("data-completed-view-html") || ""),
						);
						if (!row) return;
						if (auditOnlyRole) {
						...
						const rowHasDoctorReport =
							!!row.doctor_report_html_available ||
							(rowHasReportHtml &&
								String(row.latest_report_source || "doctor") !== "system");
						const rowHasSystemReport =
							!!row.system_report_html_available || rowHasReportHtml;
						try {
			...
			tbody
				.querySelectorAll("button[data-completed-view-system]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
						const row = rowsByClaim.get(
							String(this.getAttribute("data-completed-view-system") || ""),
						);
						if (!row) return;
						try {
							await openEditableReport(row, "system");
			...
			tbody
				.querySelectorAll("button[data-completed-view-doc]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
			...
			tbody
				.querySelectorAll("button[data-completed-export-word]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
						try {
							const row = rowsByClaim.get(
								String(this.getAttribute("data-completed-export-word") || ""),
							);
							if (!row) return;
							...
							setMessage(
								"completed-reports-msg",
								"err",
								err && err.message ? err.message : "Export Word failed.",
							);
						}
					});
				});
			...
			tbody
				.querySelectorAll("button[data-completed-export-pdf]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
						try {
							const row = rowsByClaim.get(
								String(this.getAttribute("data-completed-export-pdf") || ""),
							);
							if (!row) return;
							await exportPdf(row);
						} catch (err) {
							setMessage(
								"completed-reports-msg",
								"err",
								err && err.message ? err.message : "Export PDF failed.",
							);
						}
					});
				});
			...
			tbody
				.querySelectorAll("button[data-completed-pending]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
						const row = rowsByClaim.get(
							String(this.getAttribute("data-completed-pending") || ""),
						);
						if (!row) return;
						...
						try {
							await changeClaimStatus(
								...
								"Claim moved to Pending",
							);
						} catch (err) {
							setMessage(
								"completed-reports-msg",
								"err",
								err && err.message ? err.message : "Pending update failed.",
							);
						}
					});
				});
			...
			tbody
				.querySelectorAll("button[data-completed-withdraw]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
						const row = rowsByClaim.get(
							String(this.getAttribute("data-completed-withdraw") || ""),
						);
						if (!row) return;
						...
						try {
							await changeClaimStatus(
								row,
								"withdrawn",
								"Claim moved to Withdrawn",
							);
						} catch (err) {
							setMessage(
								"completed-reports-msg",
								"err",
								err && err.message ? err.message : "Withdraw failed.",
							);
						}
					});
				});
			...
			tbody
				...
				.forEach(function (btn) {
					btn.addEventListener("click", function () {
						const row = rowsByClaim.get(
							String(this.getAttribute("data-completed-open-upload") || ""),
						);
			...
			tbody
				.querySelectorAll("button[data-completed-open-qc]")
				.forEach(function (btn) {
					btn.addEventListener("click", function () {
						const row = rowsByClaim.get(
							String(this.getAttribute("data-completed-open-qc") || ""),
						);
			...
			tbody
				.querySelectorAll("button[data-completed-send-back]")
				.forEach(function (btn) {
					btn.addEventListener("click", async function () {
						const row = rowsByClaim.get(
							String(this.getAttribute("data-completed-send-back") || ""),
						);
						if (!row) return;
						const opinion = String(
							window.prompt(
						...
						try {
							await apiFetch(
								"/api/v1/claims/" +
									encodeURIComponent(String(row.claim_uuid || "")) +
									"/status",
								{
									method: "PATCH",
									headers: { "Content-Type": "application/json" },
							...
							setMessage(
								"completed-reports-msg",
								"ok",
								"Case sent back to doctor for claim " +
									String(row.external_claim_id || "-") +
									".",
							);
							await loadRows(false);
							...
							setMessage(
								"completed-reports-msg",
								"err",
								err && err.message ? err.message : "Send back failed.",
							);
						}
					});
				});
		...
		form.addEventListener("submit", async function (e) {
			...
			try {
				...
				setMessage(
					...
					err && err.message
						? err.message
						: "Failed to load completed reports.",
				);
			}
		});
		...
		if (uploadForm) {
			uploadForm.addEventListener("submit", async function (e) {
				...
				if (!tagging || !subtagging || !opinion) {
					setUploadModalMessage(
						"err",
						"Tagging, Subtagging and Opinion are mandatory.",
					);
					return;
				}
				...
				try {
					...
					const claimLabel = String(
						result && result.external_claim_id
							? result.external_claim_id
							: uploadModalClaimLabelEl.textContent.replace("Claim ID: ", ""),
					);
					setMessage(
						"completed-reports-msg",
						"ok",
						"Upload status updated to Uploaded for claim " + claimLabel + ".",
					);
					closeUploadModal();
					await loadRows(false);
				} catch (err) {
		...
		if (qcForm) {
			qcForm.addEventListener("submit", async function (e) {
				...
				const qcStatus = String(qcStatusEl.value || "no")
					.trim()
					.toLowerCase();
				if (!claimUuid) {
				...
				try {
					...
					const claimLabel = String(
						result && result.external_claim_id
							? result.external_claim_id
							: qcModalClaimLabelEl.textContent.replace("Claim ID: ", ""),
					);
					setMessage(
						"completed-reports-msg",
						"ok",
						"QC updated for claim " + claimLabel + ".",
					);
					closeQcModal();
					await loadRows(false);
				} catch (err) {
					setQcModalMessage(
						"err",
						err && err.message ? err.message : "Failed to update QC status.",
					);
				} finally {
		...
		completedReportsMessageHandler = async function (event) {
			try {
				...
				if (
					...
					eventType !== "claim-status-updated" &&
					eventType !== "report-saved-from-tab"
				)
					return;
				await loadRows(false);
			} catch (_err) {}
		...
		try {
			...
			setMessage(
				"completed-reports-msg",
				"err",
				err && err.message ? err.message : "Failed to load completed reports.",
			);
			tbody.innerHTML =
				'<tr><td colspan="10">Failed to load completed reports.</td></tr>';
			updatePaginationUi();
		}
	}
	...
	async function renderExportData() {
		contentPanel.innerHTML =
			'<h2>Export Full Data</h2><p class="muted">Export filtered claims as CSV or Excel.</p>' +
		...
		document
			...
			.addEventListener("submit", async function (e) {
				...
				const qs =
					...
					"&allotment_date=" +
					encodeURIComponent(allotmentDate) +
					"&format=" +
					encodeURIComponent(normalizedFormat);

				try {
					const isExcel = normalizedFormat === "excel";
					const fileResult = await apiFetchFile(
						"/api/v1/user-tools/export-full-data" + qs,
					);
					const a = document.createElement("a");
					...
					setMessage(
						"export-msg",
						"ok",
						(isExcel ? "Excel" : "CSV") + " exported successfully.",
					);
	...
	async function renderAllotmentDateWise() {
		contentPanel.innerHTML =
			'<h2>Allotment Date Wise Data</h2><p class="muted">Date-wise pending and completed (completed + uploaded) counts from legacy payload allocation date.</p>' +
		...
		const fromDateInputEl = allotmentFormEl
			? allotmentFormEl.querySelector('input[name="from_date"]')
			: null;
		const toDateInputEl = allotmentFormEl
			? allotmentFormEl.querySelector('input[name="to_date"]')
			: null;
		const allotmentResultsEl = document.getElementById("allotment-results");
		const allotmentDetailEl = document.getElementById(
			"allotment-detail-results",
		);
		...
		if (fromDateInputEl && !String(fromDateInputEl.value || "").trim())
			fromDateInputEl.value = defaultFromDate;
		if (toDateInputEl && !String(toDateInputEl.value || "").trim())
			toDateInputEl.value = defaultToDate;
		...
		function renderCountButton(count, bucket, options) {
			...
			if (opts.allotmentDate)
				attrs.push(
					'data-allotment-date="' +
						escapeHtml(String(opts.allotmentDate || "")) +
						'"',
				);
			if (opts.fromDate)
				attrs.push(
					'data-from-date="' + escapeHtml(String(opts.fromDate || "")) + '"',
				);
			if (opts.toDate)
				attrs.push(
					'data-to-date="' + escapeHtml(String(opts.toDate || "")) + '"',
				);
			return (
				"<button " +
				attrs.join(" ") +
				">" +
				escapeHtml(String(numericCount)) +
				"</button>"
			);
		}
		...
		function detailTitleFor(bucket, allotmentDate, fromDate, toDate) {
			const label =
				bucket === "completed" ? "Completed (Completed + Uploaded)" : "Pending";
			if (allotmentDate) {
				return (
					label +
					" claim list for allotment date " +
					formatDateOnly(allotmentDate)
				);
			}
			const fromLabel = fromDate ? formatDateOnly(fromDate) : "-";
			const toLabel = toDate ? formatDateOnly(toDate) : "-";
			return (
				label + " claim list for date range " + fromLabel + " to " + toLabel
			);
		}

		async function loadDetailList(opts) {
			const options = opts || {};
			const bucket = String(options.bucket || "all")
				.trim()
				.toLowerCase();
			...
			allotmentDetailEl.innerHTML =
				'<section class="claim-status-panel allotment-detail-panel"><p class="muted">Loading full list...</p></section>';

			const result = await apiFetch(
				"/api/v1/user-tools/allotment-date-wise/claims?" + params.toString(),
			);
			const items = Array.isArray(result && result.items) ? result.items : [];
			const total = Number(
				result && result.total ? result.total : items.length,
			);
			const rows = items
				.map(function (it) {
					return (
						"<tr>" +
						"<td>" +
						escapeHtml(formatDateOnly(it.allotment_date || "")) +
						"</td>" +
						"<td>" +
						escapeHtml(it.external_claim_id || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(it.patient_name || "-") +
						"</td>" +
						"<td>" +
						escapeHtml(it.assigned_doctor_id || "-") +
						"</td>" +
						"<td>" +
			...
			allotmentDetailEl.innerHTML =
				'<section class="claim-status-panel allotment-detail-panel">' +
				"<h3>" +
				escapeHtml(detailTitleFor(bucket, allotmentDate, fromDate, toDate)) +
				"</h3>" +
				'<p class="muted">Total Claims: <strong>' +
		...
		function attachAllotmentDetailHandlers() {
			...
			const countButtons = allotmentResultsEl.querySelectorAll(
				".allotment-count-link",
			);
			countButtons.forEach(function (btn) {
				btn.addEventListener("click", async function () {
					const bucket = String(
						this.getAttribute("data-allotment-bucket") || "all",
					)
						.trim()
						.toLowerCase();
					const allotmentDate = String(
						this.getAttribute("data-allotment-date") || "",
					).trim();
					const fromDate = String(
						this.getAttribute("data-from-date") || "",
					).trim();
					...
					try {
						...
						allotmentDetailEl.innerHTML =
							'<section class="claim-status-panel allotment-detail-panel"><p class="msg err">' +
							escapeHtml(
								err && err.message
									? err.message
									: "Failed to load detail list.",
							) +
							"</p></section>";
					}
				});
		...
		async function loadData(from, to) {
			const qs =
				...
				encodeURIComponent(to || "");
			const result = await apiFetch(
				"/api/v1/user-tools/allotment-date-wise" + qs,
			);
			...
			const rows = items
				.map(function (it) {
					...
					return (
						...
						"</td>" +
						"<td>" +
						renderCountButton(it.pending_count, "pending", {
							allotmentDate: rowDate,
						}) +
						"</td>" +
						"<td>" +
						renderCountButton(it.completed_count, "completed", {
							allotmentDate: rowDate,
						}) +
						"</td>" +
						"</tr>"
			...
			allotmentResultsEl.innerHTML =
				'<p class="muted">Total Pending: ' +
				renderCountButton(totalPending, "pending", {
					fromDate: from,
					toDate: to,
				}) +
				" | Total Completed (Completed + Uploaded): " +
				renderCountButton(totalCompleted, "completed", {
					fromDate: from,
					toDate: to,
				}) +
				" | Uploaded Included: <strong>" +
				escapeHtml(String(totalUploaded)) +
				"</strong></p>" +
	...
	async function renderStorageMaintenance() {
		...
		const bucketRows = (data.buckets || [])
			.map(
				(b) =>
					"<tr><td>" +
					escapeHtml(b.bucket || "unknown") +
					"</td><td>" +
					escapeHtml(String(b.count || 0)) +
					"</td></tr>",
			)
			.join("");

		contentPanel.innerHTML =
			'<h2>Storage Maintenance</h2><p class="muted">Read-only summary for document storage state.</p>' +
			'<div class="stats-grid">' +
			'<article class="stat-card"><div class="muted">Total Documents</div><div class="value">' +
			escapeHtml(String(data.total_documents || 0)) +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Total Size</div><div class="value">' +
			escapeHtml(formatBytes(data.total_bytes || 0)) +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Pending Parse</div><div class="value">' +
			escapeHtml(String((data.parse_status_counts || {}).pending || 0)) +
			"</div></article>" +
			'</div><div class="stats-grid" style="margin-top:12px;">' +
			'<article class="stat-card"><div class="muted">Processing</div><div class="value">' +
			escapeHtml(String((data.parse_status_counts || {}).processing || 0)) +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Succeeded</div><div class="value">' +
			escapeHtml(String((data.parse_status_counts || {}).succeeded || 0)) +
			"</div></article>" +
			'<article class="stat-card"><div class="muted">Failed</div><div class="value">' +
			escapeHtml(String((data.parse_status_counts || {}).failed || 0)) +
			"</div></article>" +
			...
			(bucketRows ||
				'<tr><td colspan="2">No bucket metadata found.</td></tr>') +
			"</tbody></table></div>";
	}

	async function renderRuleSuggestions() {
		const result = await apiFetch(
			"/api/v1/admin/rule-suggestions?status_filter=all&limit=300",
		);
		const items = result.items || [];
		...
		const rows = items
			.map((s) => {
				return (
					"<tr>" +
					"<td><code>" +
					escapeHtml(s.claim_id || "") +
					"</code></td>" +
					"<td>" +
					escapeHtml(s.suggestion_type || "new_rule") +
					"</td>" +
					"<td><code>" +
					escapeHtml(s.target_rule_id || s.proposed_rule_id || "-") +
					"</code></td>" +
					"<td>" +
					escapeHtml(s.suggested_name || "-") +
					"</td>" +
					"<td>" +
					statusChip(s.suggested_decision || "QUERY") +
					"</td>" +
					"<td>" +
					escapeHtml(String(s.generator_confidence || 0)) +
					"%</td>" +
					"<td>" +
					...
					"<td>" +
					(s.status === "pending"
						? '<button type="button" data-suggest-approve="' +
							s.id +
							'">Approve</button> <button type="button" class="btn-soft" data-suggest-reject="' +
							s.id +
							'">Reject</button>'
		...
		contentPanel.innerHTML =
			'<h2>Rule Suggestions</h2><p class="muted">Nightly generated suggestions from analysis history.</p>' +
		...
		contentPanel
			.querySelectorAll("button[data-suggest-approve]")
			.forEach((btn) => {
				btn.addEventListener("click", async function () {
					const id = Number(this.getAttribute("data-suggest-approve"));
					try {
						await apiFetch("/api/v1/admin/rule-suggestions/" + id, {
							method: "PATCH",
							headers: { "Content-Type": "application/json" },
		...
		contentPanel
			.querySelectorAll("button[data-suggest-reject]")
			.forEach((btn) => {
				btn.addEventListener("click", async function () {
	...
	async function renderClaimRules() {
		...
		const rows = items
			.map((r) => {
				return (
					...
					"</td>" +
					"<td>" +
					statusChip(r.decision) +
					"</td>" +
					"<td>" +
					escapeHtml(r.severity) +
					"</td>" +
					"<td>" +
					escapeHtml(String(r.priority || 0)) +
					"</td>" +
					"<td>" +
					statusChip(r.is_active ? "active" : "inactive") +
					"</td>" +
					'<td><button type="button" data-rule-edit="' +
					r.id +
					'">Edit</button> ' +
					'<button type="button" class="btn-soft" data-rule-toggle="' +
					r.id +
					'" data-rule-active="' +
					(r.is_active ? "1" : "0") +
					'">' +
					...
					"</button> " +
					'<button type="button" data-rule-delete="' +
					r.id +
					'">Delete</button></td></tr>'
				);
			})
		...
		contentPanel.innerHTML =
			'<h2>Claim Rules</h2><p class="muted">Manage claim checklist rules.</p>' +
		...
		function fillForm(item) {
			...
			form.required_evidence.value = listToTextarea(
		...
		contentPanel.querySelectorAll("button[data-rule-toggle]").forEach((btn) => {
			btn.addEventListener("click", async function () {
				...
				try {
					await apiFetch(
						"/api/v1/admin/claim-rules/" + id + "/toggle?is_active=" + !active,
						{ method: "PATCH" },
					);
		...
		contentPanel.querySelectorAll("button[data-rule-delete]").forEach((btn) => {
			btn.addEventListener("click", async function () {
				...
				try {
					await apiFetch("/api/v1/admin/claim-rules/" + id, {
						method: "DELETE",
					});
		...
		form.addEventListener("submit", async function (e) {
			...
			try {
				if (rowId) {
					await apiFetch(
						"/api/v1/admin/claim-rules/" + encodeURIComponent(rowId),
						{
							method: "PATCH",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify(payload),
					...
					await apiFetch("/api/v1/admin/claim-rules", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify(payload),
					});
	...
	async function renderDiagnosisCriteria() {
		...
		const rows = items
			.map((r) => {
				return (
					"<tr>" +
					"<td><code>" +
					escapeHtml(r.criteria_id) +
					"</code></td>" +
					"<td>" +
					escapeHtml(r.diagnosis_name) +
					"</td>" +
					"<td>" +
					statusChip(r.decision) +
					"</td>" +
					...
					statusChip(r.is_active ? "active" : "inactive") +
					"</td>" +
					'<td><button type="button" data-criteria-edit="' +
					r.id +
					'">Edit</button> ' +
					'<button type="button" class="btn-soft" data-criteria-toggle="' +
					r.id +
					...
					(r.is_active ? "Disable" : "Enable") +
					"</button> " +
					'<button type="button" data-criteria-delete="' +
					r.id +
					'">Delete</button></td></tr>'
				);
			})
		...
		contentPanel.innerHTML =
			'<h2>Diagnosis Criteria</h2><p class="muted">Manage diagnosis checklist criteria.</p>' +
		...
		document
			.getElementById("criteria-reset-btn")
			.addEventListener("click", resetForm);

		contentPanel
			.querySelectorAll("button[data-criteria-edit]")
			.forEach((btn) => {
				btn.addEventListener("click", function () {
		...
		contentPanel
			.querySelectorAll("button[data-criteria-toggle]")
			.forEach((btn) => {
				btn.addEventListener("click", async function () {
					...
					try {
						await apiFetch(
							"/api/v1/admin/diagnosis-criteria/" +
								id +
								"/toggle?is_active=" +
								!active,
							{ method: "PATCH" },
		...
		form.addEventListener("submit", async function (e) {
			...
			try {
				if (rowId) {
					await apiFetch(
						"/api/v1/admin/diagnosis-criteria/" + encodeURIComponent(rowId),
						{
							method: "PATCH",
							headers: { "Content-Type": "application/json" },
							body: JSON.stringify(payload),
					...
					await apiFetch("/api/v1/admin/diagnosis-criteria", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify(payload),
					});
	...
	async function renderMedicines() {
		const state = renderMedicines._state || {
			page: 1,
			pageSize: 50,
			total: 0,
			search: "",
		...
		contentPanel.innerHTML =
			'<h2>Medicines</h2><p class="muted">Medicine components and subclassification lookup.</p>' +
			'<form id="med-filter-form" class="claim-status-toolbar" style="margin-bottom:8px;">' +
			'<div class="claim-filter-group"><label for="med-search">Search</label><input id="med-search" name="search" placeholder="Medicine / component / subclass" value="' +
		...
		async function deleteMedicine(item) {
			...
			const confirmed = window.confirm(
				'Delete medicine "' + (medName || "ID " + String(item.id)) + '"?',
			);
			if (!confirmed) return false;

			try {
				await apiFetch(
					"/api/v1/admin/medicines/" + encodeURIComponent(String(item.id)),
					{ method: "DELETE" },
				);
				...
				setMessage(
					"med-msg",
					"err",
					err && err.message ? err.message : "Failed to delete medicine.",
				);
				return false;
			}
		}
		...
		function openMedicineModal(item) {
			...
			backdrop.innerHTML =
				...
				'<div class="modal-header"><h3 id="med-modal-title">' +
				(isEdit ? "Edit Medicine" : "Create Medicine") +
				'</h3><button type="button" class="btn-soft" id="med-modal-close">Close</button></div>' +
				...
				(isEdit
					? '<button type="button" class="btn-soft" id="med-modal-delete-btn">Delete</button>'
					: "") +
			...
			formEl.medicine_name.value =
				item && item.medicine_name ? item.medicine_name : "";
			formEl.components.value = item && item.components ? item.components : "";
			formEl.subclassification.value =
				item && item.subclassification
					? item.subclassification
					: "Supportive care";
			formEl.is_high_end_antibiotic.checked = !!(
				item && item.is_high_end_antibiotic
			);
			...
			formEl.addEventListener("submit", async function (e) {
				...
				const payload = {
					...
					subclassification:
						String(
							formEl.subclassification.value || "Supportive care",
						).trim() || "Supportive care",
					is_high_end_antibiotic: !!formEl.is_high_end_antibiotic.checked,
				};

				const controls = [saveBtn, cancelBtn, closeBtn, deleteBtn].filter(
					Boolean,
				);
				controls.forEach((el) => {
					el.disabled = true;
				...
				try {
					if (rowId) {
						await apiFetch(
							"/api/v1/admin/medicines/" + encodeURIComponent(rowId),
							{
								method: "PATCH",
								headers: { "Content-Type": "application/json" },
								body: JSON.stringify(payload),
					...
					setMessage(
						"med-modal-msg",
						"err",
						err && err.message ? err.message : "Failed to save medicine.",
					);
					controls.forEach((el) => {
						el.disabled = false;
					});
		...
		function bindActionButtons() {
			...
			contentPanel
				.querySelectorAll("button[data-med-delete]")
				.forEach((btn) => {
					btn.addEventListener("click", async function () {
		...
		async function loadRows(resetPage) {
			...
			try {
				const result = await apiFetch(
					"/api/v1/admin/medicines?" + params.toString(),
				);
				state.total = Number(result && result.total ? result.total : 0);
				currentItems =
				...
				if (!currentItems.length) {
					tbody.innerHTML =
						'<tr><td colspan="6">No medicine rows found.</td></tr>';
					updatePaginationUi();
					return;
				}
				...
				const rows = currentItems
					.map((m) => {
						return (
							...
							"</td>" +
							"<td>" +
							escapeHtml(m.components || "") +
							"</td>" +
							"<td>" +
							escapeHtml(m.subclassification || "") +
							"</td>" +
							"<td>" +
							statusChip(m.is_high_end_antibiotic ? "yes" : "no") +
							"</td>" +
							"<td>" +
							escapeHtml(m.source || "") +
							"</td>" +
							'<td><div class="doctor-case-actions">' +
							'<button type="button" class="btn-soft" data-med-edit="' +
							escapeHtml(String(m.id || "")) +
							'">Edit</button>' +
							'<button type="button" class="btn-soft" data-med-delete="' +
							escapeHtml(String(m.id || "")) +
							'">Delete</button>' +
							"</div></td>" +
							"</tr>"
				...
				currentItems = [];
				tbody.innerHTML =
					'<tr><td colspan="6">Failed to load medicines.</td></tr>';
				updatePaginationUi();
				setMessage(
					"med-msg",
					"err",
					err && err.message ? err.message : "Failed to load medicines.",
				);
			}
		}
	...
	async function renderLegacyMigration() {
		contentPanel.innerHTML =
			'<h2>Legacy Migration Pipeline</h2><p class="muted">One-click migration from TeamRightWorks to VerifAI for claims/data only.</p>' +
		...
		function renderJob(job) {
			if (!job) {
				statusBox.innerHTML =
					"<table><tbody><tr><td>No migration has started yet.</td></tr></tbody></table>";
				return;
			}
			...
			const sampleErrors =
				job.progress && Array.isArray(job.progress.sample_errors)
					? job.progress.sample_errors
					: [];
			const errorRows = sampleErrors
				.slice(0, 10)
				.map((e) => {
					return (
						"<tr><td>" +
						escapeHtml(e.claim_id || "-") +
						"</td><td>" +
						escapeHtml(e.error || "-") +
						"</td><td>" +
						escapeHtml(String(e.http_code || "-")) +
						"</td></tr>"
					);
				})
				.join("");
			...
			statusBox.innerHTML =
				"<table><tbody>" +
				'<tr><th style="width:260px">Job ID</th><td><code>' +
				...
				"</td></tr>" +
				"<tr><th>Phase</th><td>" +
				escapeHtml((job.progress && job.progress.phase) || "-") +
				"</td></tr>" +
				"<tr><th>Message</th><td>" +
				escapeHtml(job.message || "-") +
				"</td></tr>" +
				"<tr><th>Started By</th><td>" +
				escapeHtml(job.started_by || "-") +
				"</td></tr>" +
				"<tr><th>Started At</th><td>" +
				escapeHtml(formatDateTime(job.started_at || "")) +
				"</td></tr>" +
				"<tr><th>Finished At</th><td>" +
				escapeHtml(formatDateTime(job.finished_at || "")) +
				"</td></tr>" +
				"<tr><th>Claims Synced</th><td>Selected: " +
				escapeHtml(String(claims.selected || 0)) +
				", Success: " +
				escapeHtml(String(claims.success || 0)) +
				", Failed: " +
				escapeHtml(String(claims.failed || 0)) +
				", Batches: " +
				escapeHtml(String(claims.batches || 0)) +
				"</td></tr>" +
				"<tr><th>Users Synced</th><td>Candidates: " +
				escapeHtml(String(users.candidates || 0)) +
				", Created: " +
				escapeHtml(String(users.created || 0)) +
				", Updated: " +
				escapeHtml(String(users.updated || 0)) +
				", Skipped: " +
				escapeHtml(String(users.skipped || 0)) +
				", Failed: " +
				escapeHtml(String(users.failed || 0)) +
				"</td></tr>" +
				"<tr><th>Raw Cleanup</th><td>Enabled: " +
				escapeHtml(String(!!rawCleanup.enabled)) +
				", Claims: " +
				escapeHtml(String(rawCleanup.claims_touched || 0)) +
				", Reports Deleted: " +
				escapeHtml(String(rawCleanup.report_versions_deleted || 0)) +
				", Extractions Deleted: " +
				escapeHtml(String(rawCleanup.document_extractions_deleted || 0)) +
				", Docs Reset: " +
				escapeHtml(String(rawCleanup.documents_reset || 0)) +
				"</td></tr>" +
				(job.error
					? "<tr><th>Error</th><td>" + escapeHtml(job.error) + "</td></tr>"
					: "") +
				"</tbody></table>" +
				(errorRows
					? '<h4 style="margin:10px 0 6px">Recent Sync Errors</h4><table><thead><tr><th>Claim ID</th><th>Error</th><th>HTTP</th></tr></thead><tbody>' +
						errorRows +
						"</tbody></table>"
					: "");
		}
		...
		async function refreshStatus(jobId, autoContinue) {
			try {
				...
				const statusPayload = await apiFetch(
					"/api/v1/admin/legacy-migration/status" + qs,
				);
				const job =
					statusPayload && statusPayload.job ? statusPayload.job : null;
				renderJob(job);
				if (
					job &&
		...
		form.addEventListener("submit", async function (e) {
			...
			const includeClaims =
				!!document.getElementById("legacy-sync-claims").checked;
			const rawFilesOnly = !!document.getElementById("legacy-sync-raw").checked;
			const statusFilter = String(
				document.getElementById("legacy-sync-status").value || "completed",
			);
			const batchSize = Number(
				document.getElementById("legacy-sync-batch").value || 200,
			);
			const maxBatches = Number(
				document.getElementById("legacy-sync-max").value || 200,
			);
			...
			if (!includeClaims) {
				setMessage(
					"legacy-sync-msg",
					"err",
					"Select claims/data to start migration.",
				);
				return;
			}
			...
			try {
				const started = await apiFetch("/api/v1/admin/legacy-migration/start", {
					...
					body: JSON.stringify({
						...
						batch_size: Math.max(
							1,
							Math.min(500, Math.floor(batchSize || 200)),
						),
						max_batches: Math.max(
							1,
							Math.min(1000, Math.floor(maxBatches || 200)),
						),
					}),
				});
				...
				setMessage(
					"legacy-sync-msg",
					"ok",
					"Migration started. Job ID: " + String(started.job_id || "-"),
				);
				...
				setMessage(
					"legacy-sync-msg",
					"err",
					err && err.message ? err.message : "Failed to start migration.",
				);
			} finally {
	...
	async function renderAIPrompt() {
		contentPanel.innerHTML =
			'<h2>AI Prompt</h2><p class="muted">Admin prompt configuration page from legacy QC is reserved here.</p>' +
	...
	async function renderWithdrawnClaims() {
		...
		contentPanel.innerHTML =
			...
			'<div class="claim-filter-group"><label for="withdrawn-claims-doctor-filter">Doctor</label><select id="withdrawn-claims-doctor-filter" name="doctor_filter">' +
			doctorFilterOptions +
			"</select></div>" +
			'<div class="claim-filter-action"><button type="submit" class="claim-apply-btn">Apply</button></div>' +
		...
		async function loadRows(resetPage) {
			...
			try {
				...
				if (!items.length) {
					tbody.innerHTML =
						'<tr><td colspan="5">No withdrawn claims found.</td></tr>';
					updatePaginationUi();
					return;
				}
				...
				const rows = items
					.map(function (item) {
						return (
							...
							"</code></td>" +
							"<td>" +
							escapeHtml(formatAssignedDoctor(item.assigned_doctor_id || "")) +
							"</td>" +
							"<td>" +
							escapeHtml(formatDateTime(item.allotment_date || "")) +
							"</td>" +
							"<td>" +
							statusChip(item.status_display || item.status || "withdrawn") +
							"</td>" +
							"<td>" +
				...
				state.total = 0;
				tbody.innerHTML =
					'<tr><td colspan="5">Failed to load withdrawn claims.</td></tr>';
				updatePaginationUi();
				setMessage(
					"withdrawn-claims-msg",
					"err",
					err && err.message ? err.message : "Failed to load withdrawn claims.",
				);
			}
		}
	...
	async function renderPage(activeRole, page) {
		...
		if (page === "completed-not-uploaded")
			return renderCompletedReports("pending");
		if (page === "completed-uploaded")
			return renderCompletedReports("uploaded");
		...
		contentPanel.innerHTML =
			'<h2>Not Found</h2><p class="muted">This QC module path is not mapped yet.</p>';
	}
	...
	async function boot() {
		try {
			...
			const page =
				route.page &&
				(allowedPages.includes(route.page) || isRoleCaseDetailPage)
					? route.page
					: "dashboard";
			const navPage =
				page === "case-detail"
			...
			renderError(
				err && err.message ? err.message : "Failed to load workspace",
			);
		}
	}

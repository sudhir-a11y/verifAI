...
def _load_from_modern_postgres(db: Session) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    try:
...
def get_checklist_catalog(
    ...
    *,
    ...
    try:
        rules, criteria = _load_from_modern_postgres(db)
    ...
    if source in {"legacy_mysql", "seed_fallback"}:
        try:
            sync_counts = _upsert_catalog_to_modern_postgres(db, rules, criteria, source)
        except Exception as exc:
            sync_error = str(exc)

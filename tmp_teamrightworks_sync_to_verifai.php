...
function build_payload_for_claim(PDO $pdo, string $claimId): array
{
    ...
    $excelTagging = trim((string)($case['tagging'] ?? ''));
    $excelSubtagging = trim((string)($case['subtagging'] ?? ''));
    $excelOpinion = trim((string)($case['opinion'] ?? ''));
    if ($excelOpinion === '') {
        $excelOpinion = trim((string)($case['doctor_opinion'] ?? ''));
    }
    if ($excelOpinion === '') {
        $excelOpinion = trim((string)($case['trigger_remarks'] ?? ''));
    }
    $excelReportStatus = trim((string)($case['report_export_status'] ?? ''));
    if ($excelReportStatus === '') {
        $excelReportStatus = trim((string)($case['document_status'] ?? ''));
    }
    $excelQcStatus = trim((string)($case['qc_status'] ?? ''));
    ...
    $payload = [
        ...
        'doctor_opinion' => $excelOpinion !== '' ? $excelOpinion : null,
        'tagging' => $excelTagging !== '' ? $excelTagging : null,
        'subtagging' => $excelSubtagging !== '' ? $excelSubtagging : null,
        'opinion' => $excelOpinion !== '' ? $excelOpinion : null,
        'report_export_status' => $excelReportStatus !== '' ? $excelReportStatus : null,
        'qc_status' => $excelQcStatus !== '' ? $excelQcStatus : null,

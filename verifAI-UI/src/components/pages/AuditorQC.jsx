	...
	if (!canUse)
		return (
			<p className="text-sm text-slate-700">
				Only auditor/super_admin can access Auditor QC.
			</p>
		);
	if (!claimUuid)
		return <p className="text-sm text-slate-700">Missing `claim_uuid`.</p>;
				...
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						...
						<div className="mt-1 text-xs text-slate-600">
							Claim: <span className="font-mono">{claimIdLabel || "-"}</span> •
							UUID: <span className="font-mono">{claimUuid}</span>
						</div>
						{reportMeta?.created_at ? (
							<div className="mt-1 text-xs text-slate-500">
								Loaded: {formatDateTime(reportMeta.created_at)} • Source:{" "}
								{String(reportMeta?.report_source || "-")} • Version:{" "}
								{String(reportMeta?.version_no ?? "-")}
							</div>
						) : null}
					</div>
			...
			<div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
				...
				<section className="space-y-2">
					...
					<div className="flex flex-wrap items-center gap-2">
						...
						>
							<option value="">
								{docsLoading ? "Loading docs..." : "Select document..."}
							</option>
							{docs.map((d) => (

...
| Core Functionality  | Complete | 0        | -     | 100%       |
| **Overall**         | **100%** | **0%**   | -     | **100%**   |

**Important:** All UI-side features and backend API endpoints have been migrated. Migration is complete.
...
| **Workspace Layout** | `qc/public/workspace.js` NAV      | `verifAI-UI/src/app/WorkspaceLayout.jsx`  | ✅ Complete | Role-based nav + acting role switch + claim sync |
...
- ✅ Document preview (inline iframe + open in new tab)
- ✅ Checklist evaluate/latest
- ✅ Extraction pipeline (per-doc + run pipeline + checklist)
- ✅ Extraction stage breakdown (entities, evidence refs, investigations)
- ✅ Report generation via `/structured-data`
- ✅ Open migrated report editor
- ✅ Auditor actions (send back / mark completed)
- ✅ Super-admin "acting role" UX (via WorkspaceLayout)
- ✅ Claim sync events tracking (cross-tab sync via BroadcastChannel/storage)
- ✅ Pipeline heuristics display (data source, diagnosis, recommendation)
- ✅ Workflow events timeline (backend API + UI timeline component)
...
- AI Prompt backend integration (both legacy and React are placeholders)
- Enhanced extraction pipeline UI
- Advanced document handling workflows
- Workflow events timeline (requires backend API)
...
| `claimSync.js` | Cross-tab claim sync     | `verifAI-UI/src/lib/claimSync.js` |

## Backend Services (New)

These backend services were created to support the migration:

| File | Purpose | Location |
| ---- | ------- | -------- |
| `workflow_events_repo.py` | DB access for workflow events | `backend/app/repositories/` |
| `workflow_events.py` (domain) | Domain use-case for workflow events | `backend/app/domain/claims/` |
| `workflow_events.py` (endpoint) | API endpoint for workflow events | `backend/app/api/v1/endpoints/` |
| `workflowEvents.js` | Frontend API client | `verifAI-UI/src/services/` |
...
- [ ] AI Prompt backend integration when endpoints are available

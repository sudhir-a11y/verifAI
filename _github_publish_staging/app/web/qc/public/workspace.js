(function () {
  ...
  async function renderDoctorDashboard() {
    ...
    const doctorId = String((me && me.username) || '').trim();
    ...
    const [allSummary, completedSummary, withdrawnSummary, listPayload, openClaimsPayload] = await Promise.all([
    ...
    contentPanel.innerHTML = '<h2>Dashboard Overview</h2><p class="muted">Quick summary of your assigned and completed workload.</p>'
      ...
      + '</div><h3 style="margin-top:18px">Assigned Cases (Open)</h3>'
  ...
  async function renderUserDashboard() {
    ...
    const assigneeRows = assigneeItems.map(function (item) {
      return '<tr>'
        ...
        + '<td>' + escapeHtml(formatRoleLabel(item && item.role ? item.role : '')) + '</td>'
        + '<td>' + escapeHtml(String(item && item.completed ? item.completed : 0)) + '</td>'
  ...
  async function renderDoctorCaseDetail() {
    ...
    function normalizeInvestigationRows(lines) {
      ...
      function isNoiseLine(value) {
        ...
        if (/\b(?:details?\s+of\s+medication|follow\s*up\s+recommendation|ipd\s+medicine\s+bill)\b/i.test(t)) return true;
        return false;
      }
    ...
    function extractVitalsFromLine(value) {
      ...
      const tempRegex = /\b(?:temp(?:erature)?|t)\b\s*[:=\-]?\s*([0-9]{2,3}(?:\.[0-9]{1,2})?)\s*(?:Ãƒâ€šÃ‚Â°?\s*[CF])?/ig;
    ...
    function buildMedicineEvidenceSummary(primaryMedicineText, extractionPairs) {
      ...
      allTexts.forEach(function (block) {
        String(block || '').split(/\r?\n|;/).forEach(function (line) {
          const cleaned = sanitizeReportText(line).replace(/^[\-ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢\d.()]+\s*/, '').trim();
    ...
    function buildRuleStyleConclusion(decision, context) {
      ...
      const treatments = summarizeForConclusion(info.treatments || '', 220) || 'standard in-patient treatment';
      ...
      return sanitizeConclusionText(
        ...
        + ', treated with following ' + treatments
        + ', and deranged investigation report of ' + deranged + '. '
        + 'Reason for ' + reasonLabel + ': ' + reason + '.'
      );
    }

    function buildTriggeredRuleSummary(checklistRows) {
    ...
    function getDecisionOutcomeForConclusion(finalDecision) {
      ...
      if (d === 'ADMISSIBLE') return { verdict: 'APPROVED', category: 'INFO' };
      if (d === 'INADMISSIBLE') return { verdict: 'REJECTED', category: 'HARD_REJECT' };
      return { verdict: 'QUERIED', category: 'SOFT_QUERY' };
    }
    ...
    function buildRuleDrivenConclusion(checklistRows, context) {
      ...
      const complaints = summarizeForConclusion(info.complaints || '', 180) || 'unspecified complaints';
      const diagnosis = summarizeForConclusion(info.diagnosis || '', 130) || 'unspecified diagnosis';
      const treatments = summarizeForConclusion(info.treatments || '', 180) || 'supportive in-patient treatment';
      ...
      rows.forEach(function (entry) {
        ...
        const fallbackLine = sanitizeConclusionText(entry.note || entry.why_triggered || entry.summary || entry.reason || '')
          .replace(/\bR\d{3}\b\s*[-:]\s*/gi, '')
          .trim();
        if (fallbackLine) ruleLinesOnly.push(fallbackLine);
      });

      if (ruleLinesOnly.length === 0) return introLine;

      const outcome = getDecisionOutcomeForConclusion(info.finalDecision);
      const rawGaps = String(info.triggeredReasonText || '').split(';').map(function (x) {
      ...
      const finalParagraph = sanitizeConclusionText(
        ...
        + 'Based on the submitted evidence and medical necessity assessment, the claim is hereby '
        + outcome.verdict + ' under ' + outcome.category + '.'
      );
    ...
    function buildExactReasonText(decision, context) {
      ...
      const triggerText = summarizeForConclusion(String(info.triggeredReasonText || '').replace(/\bR\d{3}\b\s*[-:]\s*/gi, ''), 220);
      ...
      if (decision === 'INADMISSIBLE') {
        ...
        if (triggerText) {
          reasons.push('Key gaps: ' + triggerText + '.');
        ...
        if (triggerText) {
          reasons.push('Pending clarifications: ' + triggerText + '.');
        } else if (triggeredRuleSummary) {
    ...
    function buildLegacyReportHtml(generatedAt, doctorName, extractionPairs, evidenceLines) {
      ...
      const clinicalFindingsText = (function () {
        ...
        clinicalFindingCandidates.forEach(function (block) {
          String(block || '').split(/\r?\n/).forEach(function (line) {
            const t = sanitizeReportText(line).replace(/^[\-ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢]+\s*/, '').trim();
            if (!t) return;
      ...
      const structuredRecommendationText = /\b(recommended\s+for\s+rejection|reject(?:ion|ed)?|inadmissible)\b/i.test(structuredRecommendationRaw)
        ...
        .filter(function (entry) {
          ...
          const src = String(entry.source || "").trim().toLowerCase();
          return isChecklistRuleSource(src);
        })
      ...
      const triggeredReasonText = triggeredConditionNotes.join('; ');
      const triggeredRuleSummaryText = buildTriggeredRuleSummary(checklistRows);
    ...
    function buildReportPairsFromStructuredData(data) {
      ...
      addPair('hospital_name', data.hospital_name);
    ...
    reportBtn.addEventListener('click', async function () {
      ...
      try {
        ...
        try {
          ...
          const hasChecklistRows = Array.isArray(currentChecklistLatest && currentChecklistLatest.checklist)
            && currentChecklistLatest.checklist.length > 0;
          if (!hasChecklistRows) {
            appendLog('No rule checklist found. Running checklist evaluation for report conclusion...');
            ...
            if (evaluatedChecklist && Array.isArray(evaluatedChecklist.checklist)) {
              ...
              appendLog('Checklist evaluation completed for report generation.');
            }
          }
        } catch (refreshErr) {
  ...
  async function renderMedicines() {
    ...
    contentPanel.innerHTML = '<h2>Medicines</h2><p class="muted">Medicine components and subclassification lookup.</p>'
      ...
      + '</div>'
      + '<h3 id="med-form-title">Create Medicine</h3><form id="med-form"><input type="hidden" name="row_id">'
      ...
      + '<div class="link-row"><button type="submit">Save Medicine</button><button type="button" class="btn-soft" id="med-reset-btn">New</button></div></form>';
    ...
    function resetForm() {
      form.reset();
      form.row_id.value = '';
      form.subclassification.value = 'Supportive care';
      title.textContent = 'Create Medicine';
    }

    function fillForm(item) {
      form.row_id.value = String(item.id || '');
      form.medicine_name.value = item.medicine_name || '';
      form.components.value = item.components || '';
      form.subclassification.value = item.subclassification || 'Supportive care';
      form.is_high_end_antibiotic.checked = !!item.is_high_end_antibiotic;
    ...
    async function loadRows(resetPage) {
      ...
      try {
        ...
        const rows = currentItems.map((m) => {
          return '<tr><td>' + escapeHtml(m.medicine_name || '') + '</td><td>' + escapeHtml(m.components || '') + '</td><td>' + escapeHtml(m.subclassification || '') + '</td><td>' + statusChip(m.is_high_end_antibiotic ? 'yes' : 'no') + '</td><td>' + escapeHtml(m.source || '') + '</td><td><button type="button" data-med-edit="' + m.id + '">Edit</button></td></tr>';
        }).join('');
    ...
    document.getElementById('med-reset-btn').addEventListener('click', resetForm);
    ...
    form.addEventListener('submit', async function (e) {
      ...
      const rowId = String(form.row_id.value || '').trim();
      const payload = {
        medicine_name: String(form.medicine_name.value || '').trim(),
        components: String(form.components.value || '').trim(),
        subclassification: String(form.subclassification.value || 'Supportive care').trim() || 'Supportive care',
        is_high_end_antibiotic: !!form.is_high_end_antibiotic.checked,
      };

      try {
        if (rowId) {
          await apiFetch('/api/v1/admin/medicines/' + encodeURIComponent(rowId), { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
          setMessage('med-msg', 'ok', 'Medicine updated.');
        } else {
          await apiFetch('/api/v1/admin/medicines', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
          setMessage('med-msg', 'ok', 'Medicine created.');
        }
        await loadRows(false);
      } catch (err) {
        setMessage('med-msg', 'err', err.message);
      }
    });

...
- `POST /api/v1/documents/{document_id}/extract` with provider `auto | openai | local | aws_textract`
- `GET /api/v1/documents/{document_id}/extractions`

`auto` now tries AWS Textract first, then OpenAI (if configured), then local extraction.
...
Use structured claim JSON + policy retrieval for claim-vs-policy validation:

- `POST /api/v1/policy-rag/policies/ingest`
- `POST /api/v1/policy-rag/claims/{claim_id}/validate`

Flow:

1. Extract claim documents with AWS Textract (`provider=aws_textract` or `auto`).
2. Ingest policy wording into chunked retrieval store.
3. Build structured claim JSON from latest document extractions.
4. Retrieve top policy chunks (lexical + optional embedding similarity).
5. Run LLM reasoning on `structured_claim + retrieved_policy_chunks` and save decision packet.
...
- `AWS_TEXTRACT_SESSION_TOKEN` (optional)
- `AWS_TEXTRACT_ENDPOINT_URL` (optional)
- `OPENAI_EMBEDDING_MODEL` (optional, default `text-embedding-3-small`)
- `OPENAI_RAG_MODEL` (optional, default `gpt-4o-mini`)

(function () {
  ...
  async function renderDoctorDashboard() {
    ...
    const [allSummary, completedSummary, withdrawnSummary, listPayload] = await Promise.all([
    ...
    const visibleItems = items.filter((c) => c.status !== 'completed' && c.status !== 'withdrawn');
  ...
  async function renderDoctorCaseDetail() {
    ...
    function buildExactReasonText(decision, context) {
      ...
      const triggerText = summarizeForConclusion(info.triggeredReasonText || '', 220);
      ...
      if (triggerText) reasons.push('Checklist observations: ' + triggerText + '.');
    ...
    function buildLegacyReportHtml(generatedAt, doctorName, extractionPairs, evidenceLines) {
      ...
      const rejectDetailedFallback = sanitizeConclusionText(
        'Based on available records, inpatient admission from ' + admissionWindowText + ' (LOS: ' + lengthOfStay + ') for ' + diagnosisForConclusion + ' is not sufficiently justified for reimbursement. '
        + (complaintsForConclusion ? ('Documented complaints: ' + complaintsForConclusion + '. ') : '')
        + (findingsForConclusion ? ('Clinical findings reviewed: ' + findingsForConclusion + '. ') : '')
        + (derangedForConclusion && !/no deranged investigation values found/i.test(derangedForConclusion) ? ('Deranged investigations noted: ' + derangedForConclusion + '. ') : '')
        + (medicineForConclusion && !/no medicine list extracted/i.test(medicineForConclusion) ? ('Medicine evidence: ' + medicineForConclusion + '. ') : '')
        + (tprForConclusion && !/no date-wise tpr/i.test(tprForConclusion) ? ('Daily TPR summary: ' + tprForConclusion + '. ') : '')
        + (triggeredReasonText ? ('Checklist concerns: ' + triggeredReasonText + '. ') : '')
        + 'Therefore, admission and treatment are not justified with current evidence.'
      );
      const queryDetailedFallback = sanitizeConclusionText(
        'Available records are insufficient for final admissibility decision. '
        + (complaintsForConclusion ? ('Documented complaints: ' + complaintsForConclusion + '. ') : '')
        + (findingsForConclusion ? ('Clinical findings reviewed: ' + findingsForConclusion + '. ') : '')
        + (medicineForConclusion && !/no medicine list extracted/i.test(medicineForConclusion) ? ('Medicine evidence: ' + medicineForConclusion + '. ') : '')
        + (tprForConclusion && !/no date-wise tpr/i.test(tprForConclusion) ? ('Daily TPR summary: ' + tprForConclusion + '. ') : '')
        + (triggeredReasonText ? ('Checklist query points: ' + triggeredReasonText + '. ') : '')
        + 'Additional medical records are required for final conclusion.'
      );
